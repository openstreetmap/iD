    ...
    declare("iD.renderer.WayUI", [iD.renderer.EntityUI], {
        ...
        redraw: function() {
            ...
            var coords = _.map(way.nodes, _.bind(function(node) {
                var pt = this.map.locationPoint(node);
                pt.x += this.map.transform.x;
                pt.y += this.map.transform.y;
                return pt;
            }, this));
            ...
            for (i = 0; i < this.styleList.subparts.length; i++) {
                ...
                if (this.styleList.shapeStyles[subpart]) {
                    ...
                    // Stroke
                    if (s.width)  {
                        var line_stroke = this.targetGroup('stroke', s.sublayer)
                            .createPolyline(coords)
                            .setStroke(s.strokeStyler());

                        this.recordSprite(line_stroke);
                    ...
                    if (!isNaN(s.fill_color)) {
                        var fill_stroke = this.targetGroup('fill', s.sublayer)
                            .createPolyline(coords)
                            .setFill(s.fillStyler());

                        this.recordSprite(fill_stroke);
                        drawn = true;
                    }

                    // Casing
                    if (s.casing_width) {
                        var casing_stroke = this.targetGroup('casing')
                            .createPolyline(coords)
                            .setStroke(s.casingStyler());

                        this.recordSprite(casing_stroke);

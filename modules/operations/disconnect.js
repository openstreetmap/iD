...
export function operationDisconnect(context, selectedIDs) {
    ...
    if (_vertexIDs.length > 0) {
        ...
        _vertexIDs.forEach(function (vertexID) {
            ...
            _disconnectingWayIds = _disconnectingWayIds.concat(
                context
                    .graph()
                    .parentWays(context.graph().entity(vertexID))
                    .map((d) => d.id),
            );
        });
        _disconnectingWayIds = utilArrayUniq(_disconnectingWayIds).filter(
            function (id) {
        ...
        _descriptionID +=
            _actions.length === 1 ? 'single_point.' : 'multiple_points.';
        if (_wayIDs.length === 1) {
            _descriptionID +=
                'single_way.' + context.graph().geometry(_wayIDs[0]);
        } else {
            _descriptionID +=
        ...
        _descriptionID +=
            _wayIDs.length === 1 ? 'single_way.' : 'multiple_ways.';
    ...
    operation.disabled = function () {
        ...
        if (
            ...
            return (
                'too_large.' +
                ((_vertexIDs.length ? _vertexIDs : _wayIDs).length === 1
                    ? 'single'
                    : 'multiple')
            );

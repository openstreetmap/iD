...
export function operationExtract(context, selectedIDs) {
    ...
    var _geometries = utilArrayUniq(
        selectedIDs
            .map(function (entityID) {
                return (
                    context.graph().hasEntity(entityID) &&
                    context.graph().geometry(entityID)
                );
            })
            .filter(Boolean),
    );
    ...
    var _actions = selectedIDs
        .map(function (entityID) {
            ...
            _extent = _extent
                ? _extent.extend(entity.extent(graph))
                : entity.extent(graph);
    ...
    operation.disabled = function () {
        if (
            ...
            selectedIDs.some(function (entityID) {
                return (
                    context.graph().geometry(entityID) === 'vertex' &&
    ...
    operation.tooltip = function () {
        ...
        if (disableReason) {
            return t.append(
                'operations.extract.' + disableReason + '.' + _amount,
            );
        } else {
            return t.append(
                'operations.extract.description.' + _geometryID + '.' + _amount,
            );
        }
    };

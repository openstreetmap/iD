...
export function operationDelete(context, selectedIDs) {
    ...
    var operation = function () {
        ...
        if (selectedIDs.length === 1) {
            ...
            if (geometry === 'vertex') {
                ...
                if (i === 0) {
                    ...
                    var a = geoSphericalDistance(
                        entity.loc,
                        context.entity(nodes[i - 1]).loc,
                    );
                    var b = geoSphericalDistance(
                        entity.loc,
                        context.entity(nodes[i + 1]).loc,
                    );
                    i = a < b ? i - 1 : i + 1;
        ...
        if (nextSelectedID && nextSelectedLoc) {
            if (context.hasEntity(nextSelectedID)) {
                context.enter(
                    modeSelect(context, [nextSelectedID]).follow(true),
                );
            } else {
    ...
    operation.annotation = function () {
        return selectedIDs.length === 1
            ? t(
                  'operations.delete.annotation.' +
                      context.graph().geometry(selectedIDs[0]),
              )
            : t('operations.delete.annotation.feature', {

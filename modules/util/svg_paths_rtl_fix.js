...
import { BaselineSplitter, WordShaper } from 'alif-toolkit';
...
export function fixRTLTextForSvg(inputText) {
    let ret = '', rtlBuffer = [];
    let arabicRegex = /[\u0600-\u06FF]/g;
    let arabicTashkil = /[\u0610-\u061A\u064B-\u065F\u0670\u06D6-\u06DC\u06DF-\u06E4\u06E7\u06E8\u06EA-\u06ED]/;
    let thaanaVowel = /[\u07A6-\u07B0]/;
    let hebrewSign = /[\u0591-\u05bd\u05bf\u05c1-\u05c5\u05c7]/;
    ...
    if (!arabicRegex.test(inputText)) {
        ...
        let myWord = BaselineSplitter(WordShaper(inputText));
        myWord.forEach((baseline) => {
          if (baseline === ' ') {
            ret = baseline + rtlBuffer.reverse().join('') + ret;
            rtlBuffer = [];
          } else if (baseline.match(/^\d+$/)) {
            ret = rtlBuffer.reverse().join('') + ret + baseline;
            rtlBuffer = [];
          } else {
            rtlBuffer.push(baseline);
          }
        });
        ret = rtlBuffer.reverse().join('') + ret;
        return ret;
    }
}

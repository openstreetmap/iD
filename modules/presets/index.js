...
export { presetCategory, presetCollection, presetField, presetPreset };
...
export function presetIndex() {
    ...
    const POINT = presetPreset('point', {
        name: 'Point',
        tags: {},
    ...
    _this.merge = (d) => {
        ...
        if (d.presets) {
            Object.keys(d.presets).forEach((presetID) => {
                ...
                if (p) {
                    // add or replace
                    const isAddable =
                        !_addablePresetIDs || _addablePresetIDs.has(presetID);
        ...
        _this.collection = Object.values(_presets).concat(
            Object.values(_categories),
        );
            ...
            Object.keys(d.defaults).forEach((geometry) => {
                ...
                if (Array.isArray(def)) {
                    ...
                    _defaults[geometry] = presetCollection(
                        def
                            .map((id) => _presets[id] || _categories[id])
                            .filter(Boolean),
                    );
                } else {
                    // remove
        ...
        if (
            d.featureCollection &&
            Array.isArray(d.featureCollection.features)
        ) {
    ...
    _this.matchTags = (tags, geometry, loc) => {
        ...
        if (
            bestMatch &&
            bestMatch.locationSetID &&
            bestMatch.locationSetID !== '+[Q2]' &&
            Array.isArray(loc)
        ) {
            ...
            if (!validHere[bestMatch.locationSetID]) {
                ...
                for (let i = 0; i < matchCandidates.length; i++) {
                    ...
                    if (
                        !candidateScore.candidate.locationSetID ||
    ...
    _this.defaults = (geometry, n, startWithRecents, loc, extraPresets) => {
        ...
        let result = presetCollection(
            utilArrayUniq(
                recents.concat(defaults).concat(extraPresets || []),
            ).slice(0, n - 1),
        );
    ...
    _this.getGenericRibbonItems = () => {
        return ['point', 'line', 'area'].map((id) =>
            RibbonItem(_this.item(id), 'generic'),
        );
    };

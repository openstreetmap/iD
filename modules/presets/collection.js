...
export function presetCollection(collection) {
    ...
    _this.matchAnyGeometry = (geometries) => {
        return presetCollection(
            _this.collection.filter((d) =>
                geometries.some((geom) => d.matchGeometry(geom)),
            ),
        );
    };
    ...
    _this.search = (value, geometry, loc) => {
        ...
        function sortPresets(nameProp, aliasesProp) {
            return function sortNames(a, b) {
                ...
                if (aliasesProp) {
                    ...
                    aCompare = findMatchingAlias(
                        [aCompare].concat(a[aliasesProp]()),
                    );
                    bCompare = findMatchingAlias(
                        [bCompare].concat(b[aliasesProp]()),
                    );
                }
        ...
        const searchable = pool.filter(
            (a) => a.searchable !== false && a.suggestion !== true,
        );
        ...
        const leadingNames = searchable
            .filter(
                (a) =>
                    leading(a.searchName()) || a.searchAliases().some(leading),
            )
        ...
        const leadingNamesStripped = searchable
            .filter(
                (a) =>
                    leading(a.searchNameStripped()) ||
                    a.searchAliasesStripped().some(leading),
            )
        ...
        const leadingTerms = searchable.filter((a) =>
            (a.terms() || []).some(leading),
        );

        const leadingSuggestionTerms = suggestions.filter((a) =>
            (a.terms() || []).some(leading),
        );

        // matches value to preset.tags values
        const leadingTagValues = searchable.filter((a) =>
            Object.values(a.tags || {})
                .filter((val) => val !== '*')
                .some(leading),
        );
        ...
        const similarName = searchable
            ...
            .filter(
                (a) =>
                    a.dist +
                        Math.min(
                            value.length - a.preset.searchName().length,
                            0,
                        ) <
                    3,
            )
        ...
        const similarTerms = searchable.filter((a) => {
            return (a.terms() || []).some((b) => {
                return (
                    utilEditDistance(value, b) +
                        Math.min(value.length - b.length, 0) <
                    3
                );
            });
        });
        ...
        if (value.includes('=')) {
            leadingTagKeyValues = searchable
                .filter(
                    (a) =>
                        a.tags &&
                        Object.keys(a.tags).some(
                            (key) => key + '=' + a.tags[key] === value,
                        ),
                )
                .concat(
                    searchable.filter(
                        (a) =>
                            a.tags &&
                            Object.keys(a.tags).some((key) =>
                                leading(key + '=' + a.tags[key]),
                            ),
                    ),
                );
        }

        let results = leadingNames
            .concat(
                leadingSuggestions,

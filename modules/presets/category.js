...
export function presetCategory(categoryID, category, allPresets) {
    ...
    _this.matchAllGeometry = (geometries) =>
        _this.members.collection.some((preset) =>
            preset.matchAllGeometry(geometries),
        );

    _this.matchScore = () => -1;

    _this.name = () =>
        t(`_tagging.presets.categories.${categoryID}.name`, {
            default: categoryID,
        });
    _this.nameLabel = () =>
        t.append(`_tagging.presets.categories.${categoryID}.name`, {
            default: categoryID,
        });
    ...
    _this.searchNameStripped = () => {
        if (!_searchNameStripped) {
            ...
            // remove diacritics
            _searchNameStripped = _searchNameStripped.replace(
                /[\u0300-\u036f]/g,
                '',
            );
        }

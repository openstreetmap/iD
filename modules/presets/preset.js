...
export function presetPreset(presetID, preset, addable, allFields, allPresets) {
    ...
    _this.name = () => {
        return resolveReference('originalName').t('name', {
            default: _this.originalName || presetID,
        });
    };

    _this.nameLabel = () => {
        return resolveReference('originalName').t.append('name', {
            default: _this.originalName || presetID,
        });
    };
    ...
    _this.searchName = () => {
        if (!_searchName) {
            _searchName = (
                _this.suggestion ? _this.originalName : _this.name()
            ).toLowerCase();
        }
        return _searchName;
    };
    ...
    _this.searchAliasesStripped = () => {
        if (!_searchAliasesStripped) {
            ...
            _searchAliasesStripped =
                _searchAliasesStripped.map(stripDiacritics);
        }
    ...
    _this.setTags = (tags, geometry, skipFieldDefaults, loc) => {
        ...
        if (!addTags.hasOwnProperty('area')) {
            ...
            if (geometry === 'area') {
                ...
                for (let k in addTags) {
                    if (
                        (_this.geometry.indexOf('line') === -1 &&
                            k in osmAreaKeys) ||
                        (k in osmAreaKeysExceptions &&
                            addTags[k] in osmAreaKeysExceptions[k])
                    ) {
                        needsAreaTag = false;
                        break;
                    }
    ...
    function resolveFields(which, loc) {
        ...
        fieldIDs.forEach((fieldID) => {
            ...
            if (match !== null) {
                ...
                resolved = resolved.concat(
                    inheritFields(allPresets[match[1]], which, loc),
                );
                ...
                console.log(
                    `Cannot resolve "${fieldID}" found in ${_this.id}.${which}`,
                        ...
                        parent =
                            allPresets[
                                candidateIDs.find((candidateID) => {
                                    ...
                                    return (
                                        validHere[candidate.locationSetID] &&
                                        isEqual(candidate.tags, parent.tags)
                                    );
                                })
                            ];
                    }
        ...
        if (loc) {
            ...
            resolved = resolved.filter(
                (field) =>
                    !field.locationSetID || validHere[field.locationSetID],
            );
        }
        ...
        function shouldInherit(f) {
            ...
            if (
                f.key &&
                (_this.originalFields.some(
                    (originalField) => f.key === allFields[originalField]?.key,
                ) ||
                    _this.originalMoreFields.some(
                        (originalField) =>
                            f.key === allFields[originalField]?.key,
                    ))
            ) {

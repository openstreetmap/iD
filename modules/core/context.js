...
export function coreContext() {
    ...
    context.zoomToEntities = (entityIDs, zoomTo) => {
        ...
        let loadedEntities = [];
        const throttledZoomTo = _throttle(
            () => _map.zoomTo(loadedEntities),
            500,
        );
        ...
        _map.on('drawn.zoomToEntity', () => {
            if (!entityIDs.every((entityID) => context.hasEntity(entityID)))
                return;
    ...
    context.cleanTagKey = (val) =>
        utilCleanOsmString(val, context.maxCharsForTagKey());
    context.cleanTagValue = (val) =>
        utilCleanOsmString(val, context.maxCharsForTagValue());
    context.cleanRelationRole = (val) =>
        utilCleanOsmString(val, context.maxCharsForRelationRole());
    ...
    context.selectedIDs = () =>
        (_mode && _mode.selectedIDs && _mode.selectedIDs()) || [];
    ...
    context.init = () => {
        ...
        function initializeDependents() {
            ...
            if (context.initialHashParams.locale) {
                localizer.preferredLocaleCodes(
                    context.initialHashParams.locale,
                );
            }

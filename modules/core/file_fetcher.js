...
export function coreFileFetcher() {
    ...
    let _fileMap = {
        ...
        oci_defaults:
            ociCdnUrl.replace('{version}', ociVersion) +
        ...
        presets_package:
            presetsCdnUrl.replace('{presets_version}', presetsVersion) +
            'package.json',
        ...
        wmf_sitematrix:
            wmfSitematrixCdnUrl.replace('{version}', '0.2') +
            'data/wikipedia.min.json',
    };
    ...
    _this.get = (which) => {
        ...
        if (url.includes('{presets_version}')) {
            return _this.get('presets_package').then((result) => {
                ...
                return getUrl(
                    url.replace('{presets_version}', presetsVersion),
                    which,
                );
            });
        } else {
    ...
    function getUrl(url, which) {
        ...
        if (!prom) {
            _inflight[url] = prom = (
                window.VITEST ? import(`../${url}`) : fetch(url)
            )
                .then((response) => {
                    ...
                    if (response.status === 204 || response.status === 205)
                        return; // No Content, Reset Content

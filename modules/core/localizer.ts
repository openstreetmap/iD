...
export class coreLocalizer {
    ...
    ensureLoaded() {
        ...
        const filesToFetch: LocaleIndexKey[] = [];
        ...
        return (this._loadPromise = Promise.all([
            ...
            .then(([languages, locales, ...indexes]) => {
                ...
                const loadStringsPromises: Promise<string>[] = [];

                indexes.forEach((index, i) => {
                    // Will always return the index for `en` if nothing else
                    const fullCoverageIndex = this._localeCodes.findIndex(
                        (locale) => index[locale] && index[locale].pct === 1,
                    );
                    ...
                    this._localeCodes.slice(0, fullCoverageIndex + 1).forEach((code) => {
                        ...
                        if (index[code]) {
                            loadStringsPromises.push(this.loadLocale(code, scopeId, directory));
                        }
                    });
                });
    ...
    private _updateForCurrentLocale() {
        ...
        const currentData =
            this._dataLocales[this._localeCode] || this._dataLocales[this._languageCode];
    ...
    loadLocale(locale: string, scopeId: string, directory: string) {
        ...
        if (this._localeStrings[scopeId] && this._localeStrings[scopeId][locale]) {
            // already loaded
    ...
    tInfo(origStringId: string, replacements?: Replacements, locale?: string): TInfo | TInfo[] {
        ...
        let localeStringTree: TranslationStrings =
            this._localeStrings &&
            this._localeStrings[scopeId] &&
            this._localeStrings[scopeId][stringsKey];
        ...
        if (Array.isArray(localeString)) {
            ...
            return localeString.map(
                (_, idx) => this.tInfo(`${origStringId}.${idx}`, replacements, locale) as TInfo,
            );
        }
        ...
        if (localeString !== undefined) {
            if (replacements) {
                if (typeof localeString === 'object' && Object.keys(localeString).length) {
                    ...
                    const number = Object.values(replacements).find(
    ...
    t_append(
        ...
        const ret: LocalizedTextRenderer = <T extends HTMLElement>(selection: d3.Selection<T>) => {
            ...
            const texts = [replacements?.prefix, ...info.texts, replacements?.suffix].filter(
                Boolean,
            );
    ...
    t_addOrUpdate(
        ...
        const ret: LocalizedTextRenderer = <T extends HTMLElement>(selection: d3.Selection<T>) => {
            ...
            const span = selection.selectAll<HTMLSpanElement, any>('span').data(
                texts.map((_, i) => i),
                (d) => stringId + d,
            );
            span.exit().remove();
    ...
    languageName(code: string, options?: { localOnly?: boolean }) {
        if (this._languageNames && this._languageNames[code]) {
        ...
        if (langInfo) {
            if (langInfo.nativeName) {
                ...
                if (this._languageNames && this._languageNames[base]) {
                    // base language name in locale language
                    const scriptCode = langInfo.script;
                    const script =
                        (this._scriptNames && this._scriptNames[scriptCode]) || scriptCode;
                    // e.g. "Serbian (Cyrillic)"
                    return this.t('translate.language_and_code', {
                        language: this._languageNames[base],
                        code: script,
                    });
                } else if (this._dataLanguages[base] && this._dataLanguages[base].nativeName) {
                    // e.g. "српски (sr-Cyrl)"
                    return this.t('translate.language_and_code', {
                        language: this._dataLanguages[base].nativeName,
                        code: code,
                    });
                }
            }
...
const _t = (stringId: string, replacements?: ReplacementsSimple, locale?: string) =>
    _mainLocalizer.t(stringId, replacements, locale);

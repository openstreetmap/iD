...
export function coreGraph(other, mutable) {
    ...
    if (other instanceof coreGraph) {
        ...
        this.entities = Object.assign(
            Object.create(base.entities),
            other.entities,
        );
        this._parentWays = Object.assign(
            Object.create(base.parentWays),
            other._parentWays,
        );
        this._parentRels = Object.assign(
            Object.create(base.parentRels),
            other._parentRels,
        );
    } else {
...
coreGraph.prototype = {
    ...
    _updateCalculated: function (oldentity, entity, parentWays, parentRels) {
        ...
        if (type === 'way') {
            ...
            var oldentityMemberIDs = oldentity
                ? oldentity.members.map(function (m) {
                      return m.id;
                  })
                : [];
            var entityMemberIDs = entity
                ? entity.members.map(function (m) {
                      return m.id;
                  })
                : [];

            if (oldentity && entity) {
                removed = utilArrayDifference(
                    oldentityMemberIDs,
                    entityMemberIDs,
                );
                added = utilArrayDifference(
                    entityMemberIDs,
                    oldentityMemberIDs,
                );
            } else if (oldentity) {

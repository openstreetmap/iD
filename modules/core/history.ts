...
import type { EntityId, NodeId, OsmEntity, RelationId, WayId } from '../osm';
...
import type { LocalizedTextRenderer } from './localizer';
...
export type CreateOperation = (context: iD.Context, selectedIDs: EntityId[]) => Operation;
...
interface SerialisedStack extends Omit<Stack, 'graph'> {
    modified?: EntityId[];
    deleted?: EntityId[];
}

interface IntroGraphEntity {
    id: EntityId;
    tags?: Tags;
    loc?: Vec2;
    nodes?: NodeId[];
...
type EventMap = {
    reset: [];
    change: [difference?: coreDifference]
    merge: [entities: OsmEntity[]];
    restore: [];
    undone: [stack: Stack, previousStack: Stack];
    redone: [stack: Stack, previousStack: Stack];
    storage_error: [];
}
...
export class coreHistory extends EventDispatcher<EventMap> {
    ...
    private change(previous: coreGraph): coreDifference {
        ...
        perform(...args: ActionList): coreDifference | Promise<coreDifference>;
        perform() {
        ...
        toIntroGraph() {
            ...
            var baseEntities: { [entityId: EntityId]: IntroGraphEntity } = {};
            ...
            function copyIntroEntity(source: OsmEntity) {
                var copy = utilObjectOmit(source, ['type', 'user', 'v', 'version', 'visible']) as IntroGraphEntity;
                ...
                if (match !== null) {
                    ...
                    var permID;
                    do { permID = (nrw + (++nextID[nrw])) as NodeId & WayId & RelationId; }
        ...
        fromJSON(h: SerialisedHistory, loadChildNodes?: boolean) {
            ...
            if (h.version === 2 || h.version === 3) {
                ...
                this._stack = h.stack.map((d) => {
                    var entities: { [key: EntityId]: OsmEntity | undefined } = {}, entity;
                ...
                this._stack = h.stack.map((d) => {
                    var entities: { [key: EntityId]: OsmEntity | undefined } = {};

                    // @ts-expect-error -- intentional, this is the legacy format from 13 years ago
        ...
        async restore() {
            if (this._lock.locked()) {
                this._hasUnresolvedRestorableChanges = false;
                var json = await asyncPrefs.get<SerialisedHistory>('saved_history');

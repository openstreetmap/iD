...
export function coreHistory(context) {
    ...
    var _hasUnresolvedRestorableChanges =
        lock.lock() && !!prefs(getKey('saved_history'));
    ...
    var history = {
        ...
        photoOverlaysUsed: function (sources) {
            if (sources) {
                ...
                _stack.slice(1, _index + 1).forEach(function (state) {
                    if (
                        state.photoOverlaysUsed &&
                        Array.isArray(state.photoOverlaysUsed)
                    ) {
                        state.photoOverlaysUsed.forEach(
        ...
        toIntroGraph: function () {
            ...
            function copyIntroEntity(source) {
                var copy = utilObjectOmit(source, [
                    'type',
                    'user',
                    'v',
                    'version',
        ...
        toJSON: function () {
            ...
            var s = _stack.map(function (i) {
                ...
                if (i.photoOverlaysUsed)
                    x.photoOverlaysUsed = i.photoOverlaysUsed;
                    ...
                    if (loadChildNodes) {
                        ...
                        if (missing.length && osm) {
                            ...
                            var childNodesLoaded = function (err, result) {
                                if (!err) {
                                    var visibleGroups = utilArrayGroupBy(
                                        result.data,
                                        'visible',
                                    );
                                    ...
                                    if (visibles.length) {
                                        var visibleIDs = visibles.map(
                                            function (entity) {
                                                return entity.id;
                                            },
                                        );
                                        var stack = _stack.map(
                                            function (state) {
                                                return state.graph;
                                            },
                                        );
                                        missing = utilArrayDifference(
                                            missing,
                                            visibleIDs,
                                        );
                                        _stack[0].graph.rebase(
                                            visibles,
                                            stack,
                                            true,
                                        );

...
export function coreValidator(context) {
    ...
    const _errorOverrides = parseHashParam(
        context.initialHashParams.validationError,
    );
    const _warningOverrides = parseHashParam(
        context.initialHashParams.validationWarning,
    );
    const _suggestionOverrides = parseHashParam(
        context.initialHashParams.validationSuggestion,
    );
    const _disableOverrides = parseHashParam(
        context.initialHashParams.validationDisable,
    );
    ...
    function revalidateUnsquare(cache) {
        ...
        const buildings = context
            .history()
            .tree()
            .intersects(geoExtent([-180, -90], [180, 90]), cache.graph) // everywhere
            .filter(
                (entity) =>
                    entity.type === 'way' &&
    ...
    validator.getIssues = (options) => {
        ...
        if (_headCache.graph && _headCache.graph !== _baseCache.graph) {
            Object.values(_headCache.issuesByIssueID).forEach((issue) => {
                ...
                const userModified = (issue.entityIds || []).some((id) =>
                    _completeDiff.hasOwnProperty(id),
                );
        ...
        function filter(issue) {
            ...
            if (
                opts.includeDisabledRules === 'only' &&
                !_disabledRules[issue.type]
            )
                return false;
            ...
            if (
                opts.includeIgnored === 'only' &&
                !_ignoredIssueIDs.has(issue.id)
            )
                return false;
            ...
            if ((issue.entityIds || []).some((id) => !context.hasEntity(id)))
                return false;
    ...
    validator.focusIssue = (issue) => {
        ...
        if (issue.entityIds && issue.entityIds.length) {
            ...
            if (selectID && selectID.charAt(0) === 'r') {
                ...
                let nodeID = ids.find(
                    (id) => id.charAt(0) === 'n' && graph.hasEntity(id),
                );

                if (!nodeID) {
                    // relation has no downloaded nodes to focus on
                    const wayID = ids.find(
                        (id) => id.charAt(0) === 'w' && graph.hasEntity(id),
                    );
                    if (wayID) {
    ...
    validator.getSharedEntityIssues = (entityIDs, options) => {
        const orderedIssueTypes = [
            ...
            'missing_tag',
            'missing_role', // - missing data first
            'outdated_tags',
            'mismatched_geometry', // - identity issues
            'crossing_ways',
            'almost_junction', // - geometry issues where fixing them might solve connectivity issues
            'disconnected_way',
            'impossible_oneway', // - finally connectivity issues
        ];
        ...
        return allIssues
            .filter((issue) =>
                (issue.entityIds || []).some((entityID) =>
                    forEntityIDs.has(entityID),
                ),
            )
            .sort((issue1, issue2) => {
    ...
    validator.validate = () => {
        ...
        Object.values({
            ...incrementalDiff.created(),
            ...incrementalDiff.deleted(),
        })
        ...
        Object.values(incrementalDiff.modified())
            ...
            .forEach(({ baseEntity, headEntity }) => {
                ...
                const symDiff = utilArrayDifference(
                    utilArrayUnion(bm, hm),
                    utilArrayIntersection(bm, hm),
                );
        ...
        function runValidation(key) {
            ...
            function applySeverityOverrides(issue) {
                ...
                for (i = 0; i < _errorOverrides.length; i++) {
                    if (
                        _errorOverrides[i].type.test(type) &&
                        _errorOverrides[i].subtype.test(subtype)
                    ) {
                ...
                for (i = 0; i < _warningOverrides.length; i++) {
                    if (
                        _warningOverrides[i].type.test(type) &&
                        _warningOverrides[i].subtype.test(subtype)
                    ) {
                ...
                for (i = 0; i < _suggestionOverrides.length; i++) {
                    if (
                        _suggestionOverrides[i].type.test(type) &&
                        _suggestionOverrides[i].subtype.test(subtype)
                    ) {
                ...
                for (i = 0; i < _disableOverrides.length; i++) {
                    if (
                        _disableOverrides[i].type.test(type) &&
                        _disableOverrides[i].subtype.test(subtype)
                    ) {
                        return false;
                    }
                }
    ...
    function validateEntitiesAsync(entityIDs, cache) {
        // Enqueue the work
        const jobs = Array.from(entityIDs)
            .map((entityID) => {
    ...
    function revalidateProvisionalEntities(cache) {
        ...
        const handle = window.setTimeout(() => {
            ...
            validateEntitiesAsync(
                Array.from(cache.provisionalEntityIDs),
                cache,
            );
        }, RETRY);
...
function validationCache(which) {
    ...
    cache.uncacheIssuesOfType = (type) => {
        const issuesOfType = Object.values(cache.issuesByIssueID).filter(
        ...
        (entityIDs || []).forEach((entityID) => {
            ...
            if (entityIssueIDs) {
                entityIssueIDs.forEach((issueID) => {
                    const issue = cache.issuesByIssueID[issueID];
                    if (issue) {
                        (issue.entityIds || []).forEach((relatedID) =>

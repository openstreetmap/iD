...
export function rendererTileLayer(context) {
    ...
    function render(selection) {
        ...
        function load(d3_event, d) {
            _cache[d.url] = true;
            d3_select(this).on('error', null).on('load', null);
            render(selection);
        }
        ...
        function imageTransform(d) {
            ...
            return (
                'translate(' +
                (((d[0] * ts + d.source.offset()[0] * Math.pow(2, _zoom)) *
                    _tileSize) /
                    d.tileSize -
                    _tileOrigin[0]) +
                'px,' +
                (((d[1] * ts + d.source.offset()[1] * Math.pow(2, _zoom)) *
                    _tileSize) /
                    d.tileSize -
                    _tileOrigin[1]) +
                'px) ' +
                'scale(' +
                (scale * _tileSize) / d.tileSize +
                ',' +
                (scale * _tileSize) / d.tileSize +
                ')'
            );

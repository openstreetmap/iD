...
export function rendererBackgroundSource(data) {
    ...
    var _template = source.encrypted
    ...
    source.label = function () {
        var id_safe = source.id.replace(/\./g, '<TX_DOT>');
        return t.append('imagery.' + id_safe + '.name', {
            default: escape(_name),
        });
    };

    source.hasDescription = function () {
        var id_safe = source.id.replace(/\./g, '<TX_DOT>');
        var descriptionText = localizer.tInfo(
            'imagery.' + id_safe + '.description',
    ...
    source.url = function (coord) {
        ...
        if (source.type === 'wms') {
            var tileToProjectedCoords = function (x, y, z) {
                ...
                var lat = Math.atan(
                    Math.sinh(Math.PI * (1 - (2 * y) / zoomSize)),
                );
            ...
            var maxXminY = tileToProjectedCoords(
                coord[0] + 1,
                coord[1] + 1,
                coord[2],
            );
    ...
    source.validZoom = function (z, underzoom) {
        ...
        return (
            source.zoomExtent[0] - underzoom <= z &&
    ...
    source.isHidden = function () {
        return (
            source.id === 'DigitalGlobe-Premium-vintage' ||
...
rendererBackgroundSource.Bing = function (data, dispatch) {
    ...
    data.template =
        'https://ecn.t{switch:0,1,2,3}.tiles.virtualearth.net/tiles/a{u}.jpeg?g=1&pr=odbl&n=z';

    var bing = rendererBackgroundSource(data);
    var key = utilAesDecrypt(
    ...
    */
    ...
    var url =
        'https://dev.virtualearth.net/REST/v1/Imagery/Metadata/AerialOSM?include=ImageryProviders&uriScheme=https&key=' +
        key;
    var cache = {};
    var inflight = {};
    var providers = [];
    ...
    d3_json(url)
        .then(function (json) {
            ...
            let subDomainNumbers = subDomains
                .map((subDomain) => {
            ...
            template = template
                .replace('{subdomain}', `t{switch:${subDomainNumbers}}`)
                .replace('{quadkey}', '{u}');
            ...
            providers = imageryResource.imageryProviders.map(
                function (provider) {
                    return {
    ...
    bing.getMetadata = function (center, tileCoord, callback) {
        ...
        var url =
            'https://dev.virtualearth.net/REST/v1/Imagery/BasicMetadata/AerialOSM/' +
            centerPoint +
            '?zl=' +
            zoom +
            '&key=' +
...
rendererBackgroundSource.Esri = function (data) {
    ...
    esri.fetchTilemap = function (center) {
        ...
        if (_prevCenter && geoSphericalDistance(center, _prevCenter) < 5000)
            return;
        _prevCenter = center;
        ...
        d3_json(url)
            .then(function (result) {
                ...
                result = result.features
                    .map((f) => f.attributes)
                    .filter(
                        (a) => a.MinMapLevel <= zoom && a.MaxMapLevel >= zoom,
                    )[0];
...
rendererBackgroundSource.Custom = function (template) {
    ...
    source.imageryUsed = function () {
        ...
        if (cleaned.indexOf('?') !== -1) {
            ...
            ['access_token', 'connectId', 'token', 'Signature'].forEach(
                function (param) {
                    if (qs[param]) {
                        qs[param] = '{apikey}';
                    }

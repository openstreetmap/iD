...
export function rendererMap(context) {
    ...
    var _zoomerPannerFunction =
        'PointerEvent' in window ? utilZoomPan : d3_zoom;
    ...
    var _zoomerPanner = _zoomerPannerFunction()
        ...
        .on('start.map', function (d3_event) {
            _pointerDown =
                d3_event &&
                (d3_event.type === 'pointerdown' ||
                    (d3_event.sourceEvent &&
                        d3_event.sourceEvent.type === 'pointerdown'));
        })
        .on('end.map', function () {
            _pointerDown = false;
        });
    ...
    function map(selection) {
        ...
        _doubleUpHandler.on('doubleUp.map', function (d3_event, p0) {
            ...
            if (
                typeof d3_event.target.__data__ === 'object' &&
                // or area fills
                !d3_select(d3_event.target).classed('fill')
            )
                return;
    ...
    function drawEditable(difference, extent) {
        ...
        if (map.isInWideSelection()) {
            ...
            if (extent) {
                data = context
                    .history()
                    .intersects(map.extent().intersection(extent));
                set = new Set(
    ...
    function zoomIn(delta) {
        setCenterZoom(
            map.center(),
            Math.trunc(map.zoom() + 0.45) + delta,
            150,
            true,
        );
    }
    ...
    function paneWidth() {
        const openPane = context
            .container()
            .select('.map-panes .map-pane.shown');
        if (!openPane.empty()) {

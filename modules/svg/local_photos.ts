...
export function svgLocalPhotos(projection: Projection, context: coreContext, dispatch: Dispatch<object>) {
   ...
   function ensureViewerLoaded(context: coreContext) {
    ...
    function setStyles(hovered?: LocalPhoto | null) {
        ...
        context.container().selectAll<HTMLElement, LocalPhoto>('.layer-local-photos .viewfield-group')
        ...
        for (const file of files) {
            try {
                ...
                if (sameName.length === 0) {
                    ...
                    // @ts-expect-error -- TODO: this appears to be a bug, `value` never exists on `string | ArrayBuffer`
    ...
    drawPhotos.setFiles = function(fileList: File[], callback: (photos: LocalPhoto[]) => void) {
     ...
     */
    drawPhotos.fileList = function(fileList: File[], callback: (photos: LocalPhoto[]) => void) {

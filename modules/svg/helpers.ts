...
export type SegmentFeature = Feature<
    LineString,
    {
        nope?: true;
        target: true;
...
export function svgSegmentWay(
    ...
    function getWaySegments() {
        ...
        const features: { passive: SegmentFeature[]; active: SegmentFeature[] } = {
            passive: [],
            active: [],
        };
        ...
        for (let i = 0; i < way.nodes.length; i++) {
            ...
            if (start.type !== undefined) {
                if (start.node.id === activeID || end.node.id === activeID) {
                    // push nothing
                } else if (isActiveWay && (start.type === 2 || end.type === 2)) {
                    // one adjacent vertex

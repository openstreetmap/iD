...
import { isAddressPoint, textWidth } from './labels';
...
export function svgPoints(projection, context) {
    ...
    function addressShieldWidth(d, selection) {
        const width = textWidth(
            d.tags['addr:housenumber'] || d.tags['addr:housename'] || '',
            10,
    ...
    function drawTargets(selection, graph, entities, filter) {
        ...
        targets
            ...
            .attr('x', (d) =>
                d.properties.isAddr
                    ? -addressShieldWidth(d.properties.entity, selection) / 2
                    : -10,
            )
            .attr('y', (d) => (d.properties.isAddr ? -8 : -26))
            .attr('width', (d) =>
                d.properties.isAddr
                    ? addressShieldWidth(d.properties.entity, selection)
                    : 20,
            )
    ...
    function drawPoints(selection, graph, entities, filter) {
        ...
        var drawLayer = selection.selectAll(
            '.layer-osm.points .points-group.points',
        );

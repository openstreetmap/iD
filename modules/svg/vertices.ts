...
import { osmIdManager, type EntityId, type NodeId, type OsmEntity } from '../osm';
...
export function svgVertices(projection: Projection, context: iD.Context) {
    ...
    function draw(selection: d3.Selection<SVGGElement>, graph: coreGraph, vertices: osmNode[], sets: Selected, filter: (node: osmNode) => boolean) {
        ...
        function updateAttributes(selection: d3.Selection<SVGGElement>) {
        ...
        groups = groups
            ...
            .classed('moved', function(d) {
                return !!base.entities[d.id] && !deepEqual(graph.entity(d.id).loc, base.entity(d.id).loc);
            })
    ...
    function drawTargets(selection: d3.Selection<SVGGElement>, graph: coreGraph, entities: osmNode[], filter: (node: osmNode) => boolean) {
    ...
    function drawVertices(selection: d3.Selection, graph: coreGraph, entities: osmNode[], filter: (node: osmNode) => boolean, extent: geoExtent, fullRedraw: boolean) {
        ...
        var drawLayer = selection.selectAll<SVGGElement, unknown>('.layer-osm.points .points-group.vertices');
        var touchLayer = selection.selectAll<SVGGElement, unknown>('.layer-touch.points');

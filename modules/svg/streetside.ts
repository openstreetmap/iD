    ...
    function filterBubbles(bubbles: Bubble[], skipDateFilter = false) {
        ...
        if (fromDate && !skipDateFilter) {
            ...
            bubbles = bubbles.filter(function(bubble) {
                return bubble.captured_at && bubble.captured_at.getTime() >= fromTimestamp;
            });
        }
        if (toDate && !skipDateFilter) {
            var toTimestamp = new Date(toDate).getTime();
            bubbles = bubbles.filter(function(bubble) {
                return bubble.captured_at && bubble.captured_at.getTime() <= toTimestamp;
            });
        }
        if (usernames) {
            bubbles = bubbles.filter(function(bubble) {
    ...
    function filterSequences(sequences: BubbleSequence[], skipDateFilter = false) {
        ...
        if (fromDate && !skipDateFilter) {
            ...
            sequences = sequences.filter(function(sequences) {
                return sequences.properties.captured_at && sequences.properties.captured_at?.getTime() >= fromTimestamp;
            });
        }
        if (toDate && !skipDateFilter) {
            var toTimestamp = new Date(toDate).getTime();
            sequences = sequences.filter(function(sequences) {
                return sequences.properties.captured_at && sequences.properties.captured_at.getTime() <= toTimestamp;
            });
        }
        if (usernames) {
            sequences = sequences.filter(function(sequences) {

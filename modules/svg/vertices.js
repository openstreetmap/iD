...
export function svgVertices(projection, context) {
    ...
    function draw(selection, graph, vertices, sets, filter) {
        ...
        groups = groups
            ...
            .classed('moved', function (d) {
                return (
                    base.entities[d.id] &&
                    !deepEqual(
                        graph.entities[d.id].loc,
                        base.entities[d.id].loc,
                    )
                );
            })
        ...
        var dgroups = groups.selectAll('.viewfieldgroup').data(function data(
            d,
        ) {
    ...
    function drawTargets(selection, graph, entities, filter) {
        ...
        var targets = selection
            .selectAll('.vertex.target-allowed')
            .filter(function (d) {
        ...
        targets
            ...
            .attr('class', function (d) {
                return (
                    'node vertex target target-allowed ' + targetClass + d.id
                );
            })
    ...
    function renderAsVertex(entity, graph, wireframe, zoom) {
        ...
        return (
            ...
            (geometry === 'point' &&
                (wireframe ||
                    (zoom >= 18 &&
                        entity.directions(graph, projection).length)))
        );
    }
    ...
    function drawVertices(
        ...
        var filterRendered = function (d) {
            return (
                d.id in _currPersistent ||
                d.id in _currSelected ||
                d.id in _currHover ||
                filter(d)
            );
        };
    ...
    drawVertices.drawSelected = function (selection, graph, extent) {
        ...
        if (context.map().isInWideSelection()) {
            ...
            _currSelected = getSiblingAndChildVertices(
                context.selectedIDs(),
                graph,
                wireframe,
                zoom,
    ...
    drawVertices.drawHover = function (selection, graph, target, extent) {
        ...
        if (entity) {
            _currHover = getSiblingAndChildVertices(
                [entity.id],
                graph,
                wireframe,
                zoom,

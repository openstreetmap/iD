...
export function svgDefs(context: iD.Context) {
    let _defsSelection = d3_select<SVGDefsElement, unknown>(null!);
    ...
    function drawDefs(selection: d3.Selection) {
        ...
        const alphaSlope5 = filters.filter('#alpha-slope5').append('feComponentTransfer');
    ...
    function addSprites(ids: string[], overrideColors: boolean) {
        ...
        const spritesheets = _defsSelection.selectAll('.spritesheet').data(_spritesheetIds);

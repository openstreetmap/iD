...
export type LayerDispatch = Dispatch<
    object,
    {
        change: [];
        photoDatesChanged: [];
...
export function svgLayers(projection: Projection, context: iD.Context) {
    const dispatch = d3_dispatch('change', 'photoDatesChanged');
    let svg = d3_select<SVGSVGElement, 0>(null!);
    let _layers: SvgLayerItem[] = [
    ...
    function drawLayers(selection: d3.Selection) {
        svg = selection.selectAll<SVGSVGElement, 0>('.surface').data([0]);
        ...
        const groups = svg.selectAll<SVGGElement, SvgLayerItem>('.data-layer').data(_layers);
    ...
    drawLayers.layer = function (id: string) {
        const obj = _layers.find(function (o) {
            return o.id === id;
        });
    ...
    drawLayers.only = function (what: string[]) {
        const arr = [...what];
        const all = _layers.map(function (layer) {
            return layer.id;
        });

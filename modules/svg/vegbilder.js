...
export function svgVegbilder(projection, context, dispatch) {
    ...
    function filterImages(images, skipDateFilter) {
        ...
        if (fromDateString && !skipDateFilter) {
            ...
            images = images.filter(
                (image) => image.captured_at.getTime() >= fromDate.getTime(),
            );
        }
        ...
        if (toDateString && !skipDateFilter) {
            ...
            images = images.filter(
                (image) => image.captured_at.getTime() <= toDate.getTime(),
            );
        }
    ...
    function filterSequences(sequences, skipDateFilter) {
        ...
        if (fromDateString && !skipDateFilter) {
            ...
            sequences = sequences.filter(
                ({ images }) =>
                    images[0].captured_at.getTime() >= fromDate.getTime(),
            );
        }
        ...
        if (toDateString && !skipDateFilter) {
            ...
            sequences = sequences.filter(
                ({ images }) =>
                    images[images.length - 1].captured_at.getTime() <=
                    toDate.getTime(),
            );
        }
    ...
    drawImages.validHere = function (extent, zoom) {
        return zoom >= minZoom - 2 && getService().validHere(extent);
    };

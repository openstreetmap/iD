...
import { osmIdManager, type osmWay, type WayId } from '../osm';
...
export function svgAreas(projection: Projection, context: iD.Context) {
    ...
    function drawAreas(selection: d3.Selection, graph: coreGraph, entities: osmWay[], filter: (way: osmWay) => boolean) {
        ...
        const drawLayer = selection.selectAll<SVGGElement, osmWay>('.layer-osm.areas');
        const touchLayer = selection.selectAll<SVGGElement, osmWay>('.layer-touch.areas');

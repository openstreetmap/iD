...
export function uiFieldWikidata(field, context) {
    ...
    function wiki(selection) {
        ...
        enter
            ...
            .on('click', function (d3_event) {
                d3_event.preventDefault();
                d3_select(this.parentNode).select('input').node().select();
    ...
    function change() {
        ...
        wikidata.entityByQID(_qid, function (err, entity) {
            ...
            if (_wikipediaKey) {
                ...
                for (var i in langs) {
                    ...
                    if (entity.sitelinks[siteID]) {
                        foundPreferred = true;
                        newWikipediaValue =
                            lang + ':' + entity.sitelinks[siteID].title;
                        // use the first match
                        break;
                    }
                }
                ...
                if (!foundPreferred) {
                    ...
                    var wikiSiteKeys = Object.keys(entity.sitelinks).filter(
                        function (site) {
            ...
            context.replace(function actionUpdateWikipediaTags(graph) {
    ...
    function setLabelForEntity() {
        ...
        utilGetSetValue(_searchInput, label.value).attr('lang', label.language);
    }
    ...
    wiki.tags = function (tags) {
        ...
        _searchInput
            .attr(
                'title',
                isMixed ? tags[field.key].filter(Boolean).join('\n') : null,
            )
        ...
        wikidata.entityByQID(_qid, function (err, entity) {
            ...
            _selection
                .select('.preset-wikidata-description')
            ...
            _selection
                .select('.preset-wikidata-identifier')
                .style('display', function () {
        ...
        function unrecognized() {
            ...
            if (_qid && _qid !== '') {
                _wikiURL =
                    'https://wikidata.org/wiki/Special:Search?search=' + _qid;
            } else {
                _wikiURL = '';
            }
        }

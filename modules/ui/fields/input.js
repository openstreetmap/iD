...
export function uiFieldText(field, context) {
  ...
  // ADDED: inline regex validation
  function updatePatternValidation() {
    if (!field.pattern || !wrap || wrap.empty() || !_tags) return;

    const vals = getVals(_tags);
    ...
    let invalid = false;
    try {
      invalid = !new RegExp(field.pattern).test(value);
    } catch {
      return;
    }

    const info = wrap
      .selectAll('.form-field-pattern-info')
      .data(invalid ? [0] : []);
    ...
    enter.append('span').call(svgIcon('#iD-icon-info'));

    enter.append('span').text(t('inspector.invalid_format'));

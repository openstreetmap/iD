...
export function uiFieldCombo(field, context) {
    ...
    var _showTagInfoSuggestions =
        field.type !== 'manyCombo' && field.autoSuggestions !== false;
    var _allowCustomValues =
        field.type !== 'manyCombo' && field.customValues !== false;
    ...
    function getOptions(allOptions) {
        ...
        if (allOptions !== true) {
            ...
            options = []
                .concat(field.options, stringsField.options)
                .filter(Boolean);
        }
        const result = options.map(function (v) {
            ...
            return {
                ...
                display: addComboboxIcons(
                    stringsField.t.append(labelId, { default: v }),
                    v,
                ),
                klass: stringsField.hasTextForStringId(labelId)
                    ? ''
                    : 'raw-option',
            };
        });
    ...
    function setTaginfoValues(q, callback) {
        var queryFilter = (d) =>
            ...
            d.key.toLowerCase().includes(q.toLowerCase());
        if (hasStaticValues()) {
        ...
        var hasCountryPrefix =
            _isNetwork &&
            _countryCode &&
            _countryCode.indexOf(q.toLowerCase()) === 0;
        if (hasCountryPrefix) {
            query = _countryCode + ':';
        }
        ...
        services.taginfo[fn](params, function (err, data) {
            ...
            data = data.filter(
                (d) => field.type !== 'typeCombo' || d.value !== 'yes',
            );
            ...
            var deprecatedValues =
                deprecatedTagValuesByKey(_dataDeprecated)[field.key];
            if (deprecatedValues) {
            ...
            const additionalOptions = (
                field.options ||
                stringsField.options ||
                []
            )
                .filter(
                    (v) =>
                        !data.some(
                            (dv) => dv.value === (_isMulti ? field.key + v : v),
                        ),
                )
                .map((v) => ({ value: v }));
    ...
    function setPlaceholder(values) {
        if (_isMulti || _isSemi) {
            ...
            _staticPlaceholder =
                field.placeholder() || placeholders.slice(0, 3).join(', ');
        }
    ...
    function change() {
        ...
        if (_isMulti || _isSemi) {
            ...
            if (_isMulti) {
                ...
                t[field.key] = context.cleanTagValue(
    ...
    function combo(selection) {
        ...
        if (_isNetwork) {
            ...
            var countryCode =
                extent && countryCoder.iso1A2Code(extent.center());
    ...
    combo.tags = function (tags) {
        ...
        var showsValue = (value) =>
            !isMixed &&
            value &&
            !(field.type === 'typeCombo' && value === 'yes');
        var isRawValue = (value) =>
            showsValue(value) &&
        ...
        if (_isMulti || _isSemi) {
            ...
            if (_isMulti) {
                ...
                if (field.key) {
                    ...
                    maxLength =
                        ...
                        utilUnicodeCharsCount(field.key);
                } else {
                    ...
                    tags[field.key].forEach(function (tagVal) {
                        ...
                        if (!commonValues) {
                            ...
                            commonValues = commonValues.filter((value) =>
                                thisVals.includes(value),
                            );
                        }
                    });
                    allValues = allValues.filter(Boolean);
                } else {
                    allValues = (tags[field.key] || '')
                        .split(';')
                        .filter(Boolean);
            ...
            var hideAdd =
                maxLength <= 0 || (!_allowCustomValues && !_comboData.length);
            _container
                .selectAll('.chiplist .input-wrap')
                .style('display', hideAdd ? 'none' : null);

            var allowDragAndDrop =
                _isSemi && // only semiCombo values are ordered
                !Array.isArray(tags[field.key]);

            // Render chips
            var chips = _container
                .selectAll('.chip')
            ...
            if (!_isSemi) {
                ...
                chips
                    ...
                    .property(
                        'indeterminate',
                        (d) => d.isMixed || !['yes', 'no'].includes(d.state),
                    )
            ...
            utilGetSetValue(
                _input,
                !isMixed ? displayValue(tags[field.key]) : '',
            )
                .data([tags[field.key]])
        ...
        selection.call(
            d3_drag()
                ...
                .on('drag', function (d3_event) {
                    ...
                    if (
                        !d3_select(this).classed('dragging') &&
                    ...
                    d3_select(this).classed('dragging', true);

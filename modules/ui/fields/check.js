...
export {
    uiFieldCheck as uiFieldDefaultCheck,
    uiFieldCheck as uiFieldOnewayCheck,
};
...
export function uiFieldCheck(field, context) {
    ...
    function checkImpliedYes() {
        ...
        if (field.id === 'oneway') {
            ...
            if (
                entity.type === 'way' &&
                !!utilCheckTagDictionary(
                    entity.tags,
                    omit(osmOneWayTags, 'oneway'),
                )
            ) {
                _impliedYes = true;
                texts[0] = t.html(
                    '_tagging.presets.fields.oneway_yes.options.undefined',
                );
            }
        }
    }
    ...
    var check = function (selection) {
        ...
        input.on('click', function (d3_event) {
            ...
            if (Array.isArray(_tags[field.key])) {
                ...
                t[field.key] =
                    values[(values.indexOf(_value) + 1) % values.length];
            }
            ...
            if (
                t[field.key] === 'reversible' ||
                t[field.key] === 'alternating'
            ) {
                t[field.key] = values[0];
            }
        ...
        if (field.type === 'onewayCheck') {
            ...
            reverser.call(reverserSetText).on('click', function (d3_event) {
                ...
                d3_select(this).call(reverserSetText);
            });
        }
    };
    ...
    check.tags = function (tags) {
        ...
        function isChecked(val) {
            return (
                val !== 'no' && val !== '' && val !== undefined && val !== null
            );
        }
        ...
        input
            .property(
                'indeterminate',
                isMixed || (field.type !== 'defaultCheck' && !_value),
            )
            .property('checked', isChecked(_value));

        text.html(
            isMixed ? t.html('inspector.multiple_values') : textFor(_value),

...
export function uiFieldWikipedia(field, context) {
    ...
    const langCombo = uiCombobox(context, 'wikipedia-lang').fetcher(
        (value, callback) => {
    ...
    const titleCombo = uiCombobox(context, 'wikipedia-title').fetcher(
        (value, callback) => {
            if (!value) {
                value = '';
            ...
            const searchfn =
                value.length > 7 ? wikipedia.search : wikipedia.suggestions;
    ...
    function wiki(selection) {
        let wrap = selection.selectAll('.form-field-input-wrap').data([0]);
        ...
        let langContainer = wrap.selectAll('.wiki-lang-container').data([0]);
        ...
        let titleContainer = wrap.selectAll('.wiki-title-container').data([0]);
        ...
        let link = titleContainer.selectAll('.wiki-link').data([0]);
    ...
    function change(skipWikidata) {
        ...
        const m = value.match(
            /https?:\/\/([-a-z]+)\.wikipedia\.org\/(?:wiki|\1-[-a-z]+)\/([^#]+)(?:#(.+))?/,
        );
        ...
        if (langInfo) {
            ...
            utilGetSetValue(_langInput, nativeLangName).attr(
                'lang',
                langInfo[2],
            );
        ...
        if (value) {
            syncTags.wikipedia = context.cleanTagValue(
                language()[2] + ':' + value,
            );
        } else {
        ...
        wikidata.itemsByTitle(language()[2], value, (err, data) => {
            ...
            context.replace(function actionUpdateWikidataTags(graph) {
    ...
    function updateForTags(tags) {
        const value =
            typeof tags[field.key] === 'string' ? tags[field.key] : '';
        ...
        let anchor = m && m[3];
        const tagLangInfo =
            tagLang && _dataWikipedia.find((d) => tagLang === d[2]);

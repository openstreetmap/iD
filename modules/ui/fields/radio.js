...
export function uiFieldRadio(field, context) {
    ...
    function radio(selection) {
        ...
        enter
            ...
            .attr('value', function (d) {
                return strings.t('options.' + d, { default: d });
            })
            .attr('checked', false);

        enter.append('span').each(function (d) {
            strings.t.append('options.' + d, { default: d })(d3_select(this));
        });
    ...
    function structureExtras(selection, tags) {
        ...
        var extrasWrap = selection
            .selectAll('.structure-extras-wrap')
            .data(selected ? [0] : []);

        extrasWrap.exit().remove();

        extrasWrap = extrasWrap
            .enter()
            .append('div')
        ...
        var typeItem = list
            .selectAll('.structure-type-item')
            .data(typeField ? [typeField] : [], function (d) {
                return d.id;
            });
        ...
        if (typeField) {
            typeItem
                .selectAll('.structure-input-type-wrap')
        ...
        var layerItem = list
            .selectAll('.structure-layer-item')
        ...
        if (layerField) {
            layerItem
                .selectAll('.structure-input-layer-wrap')
    ...
    radio.tags = function (tags) {
        ...
        radios.property('checked', function (d) {
            return (
                isOptionChecked(d) &&
                (field.key ||
                    field.options.filter(isOptionChecked).length === 1)
            );
        });
        ...
        labels
            .classed('active', function (d) {
                ...
                return (
                    ...
                    !!(
                        typeof tags[d] === 'string' &&
                        tags[d].toLowerCase() !== 'no'
                    )
                );
            })
            ...
            .attr('title', function (d) {
                return isMixed(d)
                    ? t('inspector.unshared_value_tooltip')
                    : null;
            });

    ...
    access.tags = function (tags) {
        ...
        utilGetSetValue(items.selectAll('.preset-input-access'), function (d) {
            ...
            .classed('mixed', function (accessField) {
                return (
                    (tags[accessField] && Array.isArray(tags[accessField])) ||
            ...
            .attr('title', function (accessField) {
                return (
                    tags[accessField] &&
                    Array.isArray(tags[accessField]) &&
                    tags[accessField].filter(Boolean).join('\n')
                );
            })
        ...
        function getPlaceholder(tags, accessField) {
            ...
            if (
                tags.vehicle &&
                (accessField === 'bicycle' || accessField === 'motor_vehicle')
            ) {
                return tags.vehicle;
            }
            if (tags.access) {
            ...
            for (const key in placeholdersByTag) {
                if (tags[key]) {
                    if (
                        placeholdersByTag[key][tags[key]] &&

...
export function uiFieldCheck(field: any, context: iD.Context) {
    ...
    function checkImpliedYes() {
        ...
        if (field.id === 'oneway') {
            ...
            if (
                entity.type === 'way' &&
                !!utilCheckTagDictionary(entity.tags, omit(osmOneWayTags, 'oneway'))
            ) {
                _impliedYes = true;
    ...
    function reverserSetText(selection: d3.Selection<HTMLButtonElement>) {
        ...
        selection
            .selectAll<HTMLElement, any>('.reverser-span')
            .text('')
    ...
    const check = function (selection: d3.Selection) {
        ...
        label = selection.selectAll<HTMLLabelElement, any>('.form-field-input-wrap').data([0]);

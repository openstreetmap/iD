...
export function uiPresetList(context) {
    ...
    function presetList(selection) {
        ...
        function initialKeydown(d3_event) {
            ...
            if (
                search.property('value').length === 0 &&
        ...
        function keypress(d3_event) {
            ...
            if (
                d3_event.keyCode === 13 && // ↩ Return
                value.length
            ) {
                list.selectAll('.preset-list-item:first-child').each(
                    function (d) {
                        d.choose.call(this);
                    },
                );
        ...
        function inputevent() {
            ...
            if (value.length) {
                ...
                var entityPresets = _entityIDs.map((entityID) =>
                    presetManager.match(
                        context.graph().entity(entityID),
                        context.graph(),
                    ),
                );
                results = presetManager.defaults(
                    entityGeometries()[0],
                    36,
                    !context.inIntro(),
                    _currLoc,
        ...
        context
            .features()
            .on('change.preset-list', updateForFeatureHiddenState);
    }

    function drawList(list, presets) {
        presets = presets.matchAllGeometry(entityGeometries());
        var collection = presets.collection.reduce(function (
            collection,
            preset,
        ) {
            ...
            if (preset.members) {
                if (
                    preset.members.collection.filter(function (preset) {
        ...
        var items = list
            .selectChildren('.preset-list-item')
    ...
    function itemKeydown(d3_event) {
        ...
        var parentItem = d3_select(
            item.node().parentNode.closest('.preset-list-item'),
        );
        ...
        if (d3_event.keyCode === utilKeybinding.keyCodes['↓']) {
            ...
            if (nextItem.empty()) {
                ...
                nextItem = item.select(
                    '.subgrid .preset-list-item:first-child',
                );
            }
            if (!nextItem.empty()) {
            ...
            if (previousItem.empty()) {
                ...
                previousItem.select('.preset-list-button').classed('expanded')
            ) {
                // select the last subitem of the sublist of the previous item
                previousItem = previousItem.select(
                    '.subgrid .preset-list-item:last-child',
                );
            }
            ...
            if (!previousItem.empty()) {
                ...
                var search = d3_select(
                    this.closest('.preset-list-pane'),
    ...
    function CategoryItem(preset) {
        ...
        function item(selection) {
            ...
            function click() {
                ...
                d3_select(this)
                    ...
                    .attr(
                        'title',
                        !isExpanded ? t('icons.collapse') : t('icons.expand'),
                    );
                d3_select(this)
        ...
        item.choose = function () {
            ...
            if (shown) {
                ...
                shown = true;
                var members =
                    preset.members.matchAllGeometry(entityGeometries());
                ...
                box.transition()
                    ...
                    .style(
                        'max-height',
                        200 + members.collection.length * 190 + 'px',
                    )
    ...
    function PresetItem(preset) {
        function item(selection) {
            ...
            var nameparts = [preset.nameLabel(), preset.subtitleLabel()].filter(
                Boolean,
            );
        ...
        item.choose = function () {
            ...
            context.perform(function (graph) {
                for (var i in _entityIDs) {
                    ...
                    var oldPreset = presetManager.match(
    ...
    function updateForFeatureHiddenState() {
        ...
        var button = context
            .container()
            .selectAll('.preset-list .preset-list-button');
        ...
        button.each(function (item, index) {
            ...
            for (var i in geometries) {
                hiddenPresetFeaturesId = context
                    .features()
                    .isHiddenPreset(item.preset, geometries[i]);
            ...
            var isHiddenPreset =
                ...
                (_currentPresets.length !== 1 ||
                    item.preset !== _currentPresets[0]);

            d3_select(this).classed('disabled', isHiddenPreset);

            if (isHiddenPreset) {
                var isAutoHidden = context
                    .features()
                    .autoHidden(hiddenPresetFeaturesId);
                d3_select(this).call(
                    uiTooltip()
                        .title(() =>
                            t.append(
                                'inspector.hidden_preset.' +
                                    (isAutoHidden ? 'zoom' : 'manual'),
                                {
                                    features: t(
                                        'feature.' +
                                            hiddenPresetFeaturesId +
    ...
    function entityGeometries() {
        ...
        for (var i in _entityIDs) {
            ...
            if (
                geometry === 'vertex' &&
                entity.isOnAddressLine(context.graph())
            ) {
                geometry = 'point';
            }

...
export function uiDisclosure(context, key, expandedDefault) {
    ...
    var disclosure = function (selection) {
        if (_expanded === undefined || _expanded === null) {
            ...
            _expanded =
                preference === null ? !!expandedDefault : preference === 'true';
        }

        var hideToggle = selection.selectAll('.hide-toggle-' + key).data([0]);

        // enter
        var hideToggleEnter = hideToggle
            .enter()
            .append('h3')
            .append('a')
            .attr('role', 'button')
        ...
        var wrap = selection.selectAll('.disclosure-wrap').data([0]);

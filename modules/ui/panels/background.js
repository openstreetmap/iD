...
export function uiPanelBackground(context) {
    ...
    function redraw(selection) {
        ...
        var toggleTiles = context.getDebug('tile')
            ? 'hide_tiles'
            : 'show_tiles';
    ...
    function getMetadata(selection) {
        var tile = context
            .container()
            .select('.layer-background img.tile-center'); // tile near viewport center
        if (tile.empty()) return;

        var sourceId = _currSource.id;
        var d = tile.datum();
        var zoom =
            (d && d.length >= 3 && d[2]) || Math.floor(context.map().zoom());
        var center = context.map().center();

        // update zoom
        _metadata.zoom = String(zoom);
        selection
            .selectAll('.background-info-list-zoom')
            .classed('hide', false)
        ...
        background
            .baseLayerSource()
            .getMetadata(center, d, function (err, result) {
                if (err || _currSource.id !== sourceId) return;

                // update vintage
                var vintage = result.vintage;
                _metadata.vintage =
                    (vintage && vintage.range) ||
                    t('info_panels.background.unknown');
                selection
                    .selectAll('.background-info-list-vintage')
                    .classed('hide', false)
                ...
                _metadataKeys.forEach(function (k) {
                    ...
                    _metadata[k] = val;
                    selection
                        .selectAll('.background-info-list-' + k)
                        .classed('hide', !val)

...
export function uiPanelMeasurement(context) {
    ...
    function redraw(selection) {
            ...
            if (selected.length) {
                ...
                for (var i in selected) {
                    ...
                    if (geometry === 'line' || geometry === 'area') {
                        ...
                        length += radiansToMeters(
                            d3_geoLength(toLineString(feature)),
                        );
                        centroid = d3_geoPath(context.projection).centroid(
                            entity.asGeoJSON(graph),
                        );
                        centroid =
                            centroid && context.projection.invert(centroid);
                        if (
                            !centroid ||
                            !isFinite(centroid[0]) ||
                            !isFinite(centroid[1])
                ...
                if (selected.length === 1 && selected[0].type === 'node') {
                    ...
                    totalNodeCount = utilGetAllNodes(
                        selectedIDs,
                        context.graph(),
                    ).length;
                }
        ...
        if (geometry) {
            list.append('li')
                .call(
                    t.append('info_panels.measurement.geometry', {
                        suffix: ':',
                    }),
                )
                .append('span')
                .html(
                    closed
                        ? t.html('info_panels.measurement.closed_' + geometry)
                        : t.html('geometry.' + geometry),
                );
        }
        ...
        if (totalNodeCount) {
            list.append('li')
                .call(
                    t.append('info_panels.measurement.node_count', {
                        suffix: ':',
                    }),
                )
                .append('span')
        ...
        if (typeof distance === 'number') {
            list.append('li')
                .call(
                    t.append('info_panels.measurement.distance', {
                        suffix: ':',
                    }),
                )
                .append('span')
        ...
        if (location) {
            coordItem = list
                .append('li')
                .call(
                    t.append('info_panels.measurement.location', {
                        suffix: ':',
                    }),
                );
        ...
        if (centroid) {
            coordItem = list
                .append('li')
                .call(
                    t.append('info_panels.measurement.centroid', {
                        suffix: ':',
                    }),
                );
        ...
        if (center) {
            coordItem = list
                .append('li')
                .call(
                    t.append('info_panels.measurement.center', { suffix: ':' }),
                );

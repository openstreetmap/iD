...
export function uiChangesetEditor(context) {
    ...
    function render(selection) {
        ...
        if (!_fieldsArr) {
            ...
            _fieldsArr = [
                uiField(context, presets.field('comment'), null, {
                    show: true,
                    revert: false,
                }),
        ...
        if (initial) {
            ...
            const sourceField = _fieldsArr.find(
                (field) => field.id === 'source',
            );
            ...
            if (osm) {
                osm.userChangesets(function (err, changesets) {
                    ...
                    sourceField.impl.setCustomOptions(
                        utilArrayUniqBy(recentSources, 'title'),
                    );
                });
            }
        }
        ...
        const strLen = utilUnicodeCharsCount(
            utilCleanOsmString(_tags.comment, Number.POSITIVE_INFINITY),
        );
        if (strLen > maxChars || !true) {
            warnings.push({
                id: 'message too long',
                msg: t.append('commit.changeset_comment_length_warning', {
                    maxChars: maxChars,
                }),
            });
        }

        var commentWarning = selection
            .select('.form-field-comment')
            .selectAll('.comment-warning')
        ...
        var commentEnter = commentWarning
            .enter()

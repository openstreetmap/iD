...
export function uiCommit(context) {
    ...
    function initChangeset() {
        ...
        if (photoOverlaysUsed.length) {
            ...
            tags.source = context.cleanTagValue(
    ...
    function loadDerivedChangesetTags() {
        ...
        var imageryUsed = context.cleanTagValue(
            context.history().imageryUsed().join(';'),
        );
        ...
        if (services.keepRight) {
            ...
            if (krClosed.length) {
                tags['closed:keepright'] = context.cleanTagValue(
                    krClosed.join(';'),
                );
            }
        }
        if (services.osmose) {
            ...
            for (itemType in osmoseClosed) {
                tags['closed:osmose:' + itemType] = context.cleanTagValue(
                    osmoseClosed[itemType].toString(),
                );
            }
        }
        ...
        function addIssueCounts(issues, prefix) {
            ...
            for (var issueType in issuesByType) {
                ...
                if (issuesOfType[0].subtype) {
                    var issuesBySubtype = utilArrayGroupBy(
                        issuesOfType,
                        'subtype',
                    );
                    for (var issueSubtype in issuesBySubtype) {
                        var issuesOfSubtype = issuesBySubtype[issueSubtype];
                        tags[prefix + ':' + issueType + ':' + issueSubtype] =
                            context.cleanTagValue(
                                issuesOfSubtype.length.toString(),
                            );
                    }
                } else {
                    tags[prefix + ':' + issueType] = context.cleanTagValue(
                        issuesOfType.length.toString(),
                    );
                }
            }
        }
    ...
    function render(selection) {
        ...
        headerTitle.append('div').append('h2').call(t.append('commit.title'));
        ...
        changesetSection = changesetSection
            .enter()
            .append('div')
        ...
        osm.userDetails(function (err, user) {
            ...
            prose.html(
                t.html('commit.upload_explanation_with_user', {
                    user: { html: userLink.html() },
                }),
            );
        });
        ...
        // Enter
        var requestReviewEnter = requestReview
            .enter()
            .append('div')
        ...
        labelEnter.append('span').call(t.append('commit.request_review'));

        // Update
        requestReview = requestReview.merge(requestReviewEnter);

        var requestReviewInput = requestReview
            .selectAll('input')
        ...
        buttonSection
            .selectAll('.cancel-button')
            .on('click.cancel', function () {
                dispatch.call('cancel', this);
            });

        buttonSection
            .selectAll('.save-button')
        ...
        if (uploadBlockerTooltipText) {
            buttonSection.selectAll('.save-button').call(
                uiTooltip()
        ...
        tagSection.call(
            rawTagEditor.tags(Object.assign({}, context.changeset.tags)).render, // shallow copy
        );

        var changesSection = body
            .selectAll('.commit-changes-section')
            .data([0]);
        ...
        function toggleRequestReview() {
            ...
            tagSection.call(
                rawTagEditor.tags(Object.assign({}, context.changeset.tags)) // shallow copy
                    .render,
            );
        }
    }
    ...
    function getUploadBlockerMessage() {
        ...
        if (errors.length) {
            return t.append('commit.outstanding_errors_message', {
                count: errors.length,
            });
        } else {
            var hasChangesetComment =
                context.changeset &&
                context.changeset.tags.comment &&
                context.changeset.tags.comment.trim().length;
    ...
    function updateChangeset(changed, onInput) {
        ...
        if (!onInput) {
            ...
            var commentOnly =
                changed.hasOwnProperty('comment') && changed.comment !== '';
        ...
        if (_userDetails && _userDetails.changesets_count !== undefined) {
            var changesetsCount =
                parseInt(_userDetails.changesets_count, 10) + 1; // #4283

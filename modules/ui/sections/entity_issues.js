...
export function uiSectionEntityIssues(context) {
    ...
    function reloadIssues() {
        _issues = context
            .validator()
            .getSharedEntityIssues(_entityIDs, { includeDisabledRules: true });
    }
    ...
    function renderDisclosureContent(selection) {
        ...
        var containers = selection
            .selectAll('.issue-container')
            .data(_issues, function (d) {
                return d.key;
            });
        ...
        // Enter
        var containersEnter = containers
            .enter()
            .append('div')
        ...
        textEnter.each(function (d) {
            d3_select(this).call(
                svgIcon(validationIssue.ICONS[d.severity], 'issue-icon'),
            );
        });
        ...
        containersEnter
            ...
            .each(function (d) {
                if (typeof d.reference === 'function') {
                    d3_select(this).call(d.reference);
                } else {
                    d3_select(this).call(
        ...
        var fixes = fixLists.selectAll('.issue-fix-item').data(
            function (d) {
        ...
        var buttons = fixesEnter
            ...
            .on('click', function (d3_event, d) {
                ...
                if (
                    d.issue.dateLastRanFix &&
                    new Date() - d.issue.dateLastRanFix < 1000
                )
                    return;
                ...
                utilHighlightEntities(
                    d.issue.entityIds.concat(d.entityIds),
                    false,
                    context,
                );

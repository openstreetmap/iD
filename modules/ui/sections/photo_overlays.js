...
export function uiSectionPhotoOverlays(context) {
    ...
    var settingsLocalPhotos = uiSettingsLocalPhotos(context).on(
        'change',
        localPhotosChanged,
    );
    ...
    function renderDisclosureContent(selection) {
        var container = selection
            .selectAll('.photo-overlay-container')
            .data([0]);
    ...
    function drawPhotoItems(selection) {
        ...
        function layerEnabled(d) {
            return (
                layerSupported(d) &&
                (d.layer.enabled() || _savedLayers.includes(d.id))
            );
        }
        ...
        var li = ul.selectAll('.list-item-photos').data(data, (d) => d.id);
        ...
        var liEnter = li
            ...
            .attr('class', function (d) {
                ...
                if (
                    d.id === 'mapillary-signs' ||
                    d.id === 'mapillary-map-features'
                ) {
                    classes += ' indented';
                }
                return classes;
    ...
    function drawPhotoTypeItems(selection) {
        ...
        var li = ul
            .selectAll('.list-item-photo-types')
        ...
        var labelEnter = liEnter.append('label').each(function (d) {
            d3_select(this).call(
                uiTooltip()
                    .title(() =>
                        t.append('photo_overlays.photo_type.' + d + '.tooltip'),
                    )
                    .placement('top'),
            );
        });
    ...
    function drawDateSlider(selection) {
        var ul = selection.selectAll('.layer-list-date-slider').data([0]);
        ...
        var li = ul
            .selectAll('.list-item-date-slider')
            .data(
                context.photos().shouldFilterDateBySlider()
                    ? ['date-slider']
                    : [],
            );
        ...
        var labelEnter = liEnter.append('label').each(function () {
            d3_select(this).call(
                uiTooltip()
                    .title(() =>
                        t.append('photo_overlays.age_slider_filter.tooltip'),
                    )
                    .placement('top'),
            );
        });
        ...
        let sliderWrap = labelEnter.append('div').attr('class', 'slider-wrap');
        ...
        sliderWrap
            ...
            .style(
                'direction',
                localizer.textDirection() === 'rtl' ? 'ltr' : 'rtl',
            )
            .call(utilNoAuto)
            .on('change', function () {
        ...
        selection.select('input.from-date').each(function () {
        ...
        sliderWrap
            ...
            .style('display', () =>
                dateSliderValue('to') === 0 ? 'none' : null,
            )
        ...
        selection.select('input.to-date').each(function () {
            this.value = 1 - dateSliderValue('to');
        });

        selection.select('.date-slider-label').call(
            dateSliderValue('from') === 1
                ? t.addOrUpdate('photo_overlays.age_slider_filter.label_all')
                : t.addOrUpdate('photo_overlays.age_slider_filter.label_date', {
                      date: new Date(
                          now -
                              Math.pow(dateSliderValue('from'), 1.45) *
                                  10 *
                                  365.25 *
                                  86400 *
                                  1000,
                      ).toLocaleDateString(localizer.localeCode()),
                  }),
        );
        ...
        const ticks = selection
            .select('datalist#photo-overlay-date-range')
            .selectAll('option')
        ...
        ticks
            ...
            .merge(ticks)
            .attr('value', (d) => d);
        const ticksInverted = selection
            .select('datalist#photo-overlay-date-range-inverted')
            .selectAll('option')
            .data([...dateTicks].concat([1, 0]));
        ticksInverted.exit().remove();
        ticksInverted
            .enter()
            .append('option')
            .merge(ticksInverted)
    ...
    function dateSliderValue(which) {
        const val =
            which === 'from'
                ? context.photos().fromDate()
                : context.photos().toDate();
        if (val) {
    ...
    function setYearFilter(value, updateUrl, which) {
        ...
        if (value < 1 && value > 0) {
            const date = new Date(
                now - Math.pow(value, 1.45) * 10 * 365.25 * 86400 * 1000,
            )
                .toISOString()
                .substring(0, 10);
    ...
    function drawUsernameFilter(selection) {
        ...
        var li = ul
            .selectAll('.list-item-username-filter')
            .data(
                context.photos().shouldFilterByUsername()
                    ? ['username-filter']
                    : [],
            );
        ...
        var labelEnter = liEnter.append('label').each(function () {
            d3_select(this).call(
                uiTooltip()
                    .title(() =>
                        t.append('photo_overlays.username_filter.tooltip'),
                    )
                    .placement('top'),
            );
        });
    ...
    function drawLocalPhotos(selection) {
        ...
        localPhotosLabelEnter.call(t.append('local_photos.header'));
    ...
    context
        .layers()
        .on(
            'photoDatesChanged.uiSectionPhotoOverlays',
            function (service, dates) {

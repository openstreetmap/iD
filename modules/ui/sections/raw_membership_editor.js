...
export function uiSectionRawMembershipEditor(context) {
    ...
    function getSharedParentRelations() {
        ...
        for (var i = 0; i < _entityIDs.length; i++) {
            ...
            if (i === 0) {
                ...
                parents = utilArrayIntersection(
                    parents,
                    context.graph().parentRelations(entity),
                );
            }
    ...
    function getMemberships() {
        ...
        memberships.forEach(function (membership) {
            membership.domId = utilUniqueDomId(
                'membership-' + membership.relation.id,
            );
            var roles = [];
        ...
        const newlyAddedRelations = memberships.filter((membership) =>
            recentlyAdded.has(membership.relation.id),
        );
    ...
    function changeRole(d3_event, d) {
        ...
        var newRole = context.cleanRelationRole(
            d3_select(this).property('value'),
        );
    ...
    function fetchNearbyRelations(q, callback) {
        ...
        function baseDisplayLabel(entity) {
            ...
            var presetName =
                (matched && matched.name()) || t('inspector.relation');
            var entityName = utilDisplayName(entity) || '';

            return (selection) => {
                selection.append('b').text(presetName + ' ');
                selection
                    .append('span')
                    .classed(
                        'has-colour',
                        entity.tags.colour && isColourValid(entity.tags.colour),
                    )
        ...
        if (
            explicitRelation &&
            explicitRelation.type === 'relation' &&
            explicitRelation.id !== entityID
        ) {
            ...
            result.push({
                ...
                value:
                    baseDisplayValue(explicitRelation) +
                    ' ' +
                    explicitRelation.id,
    ...
    function renderDisclosureContent(selection) {
        ...
        var items = list
            .selectAll('li.member-row-normal')
        ...
        labelLink
            ...
            .text(function (d) {
                ...
                return (
                    (matched && matched.name()) || t.html('inspector.relation')
                );
            });
        ...
        labelLink
            ...
            .classed(
                'has-colour',
                (d) =>
                    d.relation.tags.colour &&
                    isColourValid(d.relation.tags.colour),
            )
            .style('border-color', (d) => d.relation.tags.colour)
            .text(function (d) {
                const matched = presetManager.match(
                    d.relation,
                    context.graph(),
                );
        ...
        items.selectAll('button.members-download').classed('hide', (d) => {
            const graph = context.graph();
            return d.relation.members.every((m) => graph.hasEntity(m.id));
        });

        const dupeLabels = new WeakSet(
            Object.values(
                utilArrayGroupBy(
                    items.selectAll('.label-text').nodes(),
                    'textContent',
                ),
            )
                .filter((v) => v.length > 1)
        ...
        items.select('.label-text').each(function () {
            ...
            if (dupeLabels.has(this)) {
                ...
                label.attr(
                    'title',
                    (d) => `${entityName.text()} ${d.relation.id}`,
                );
            } else {
        ...
        wrapEnter
            ...
            .attr('title', function (d) {
                return Array.isArray(d.role)
                    ? d.role.filter(Boolean).join('\n')
                    : d.role;
            })
            .attr('placeholder', function (d) {
                return Array.isArray(d.role)
                    ? t('inspector.multiple_roles')
                    : t('inspector.role');
            })
        ...
        var newMembership = list
            .selectAll('.member-row-new')
            .data(_showBlank ? [0] : []);
        ...
        // Enter
        var newMembershipEnter = newMembership
            .enter()
            .append('li')
        ...
        var addRow = selection.selectAll('.add-row').data([0]);
        ...
        addRelationButton.call(
            uiTooltip()
                .title(() => t.append('inspector.add_to_relation'))
                .placement(
                    localizer.textDirection() === 'ltr' ? 'right' : 'left',
                ),
        );
        ...
        function acceptEntity(d) {
            ...
            var role = context.cleanRelationRole(
                list
                    .selectAll('.member-row-new .member-role')
                    .property('value'),
            );
        ...
        function bindTypeahead(d) {
            ...
            role.call(
                uiCombobox(context, 'member-role')

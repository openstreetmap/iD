...
export function uiSectionValidationStatus(context) {
    ...
    function renderIgnoredIssuesReset(selection) {
        ...
        var resetIgnored = selection
            .selectAll('.reset-ignored')
        ...
        // enter
        var resetIgnoredEnter = resetIgnored
            .enter()
            .append('div')
        ...
        resetIgnored
            .select('a')
            .html(
                t.html('inspector.title_count', {
                    title: { html: t.html('issues.reset_ignored') },
    ...
    function setNoIssuesText(selection) {
            ...
            for (var type in cases) {
                ...
                if (hiddenIssues.length) {
                    selection
                        ...
                        .html('')
                        .call(
                            t.append('issues.no_issues.hidden_issues.' + type, {
        ...
        if (
            opts.what === 'edited' &&
            context.history().difference().summary().length === 0
        ) {

...
import { uiTaskHeader } from './note_header';
...
export function uiTaskingEditor(context) {
    ...
    function taskingEditor(selection) {
        // quick links
        var choices = [{
            id: 'zoom_to',
            label: 'inspector.zoom_to.title',
            tooltip: function() {
                return uiTooltipHtml(t('inspector.zoom_to.tooltip_task'), t('inspector.zoom_to.key'));
            },
        ...
        footer.enter()
            ...
            .attr('class', 'footer')
            .merge(footer)
            // .call(uiViewOnOSM(context).what(_task))
            // .call(uiNoteReport(context).note(_task));


        // rerender the task editor on any auth change
        var osm = services.osm;
        if (osm) {
            osm.on('change.note-save', function() {
                selection.call(taskingEditor);
            });
        }
    }
    ...
    function noteSaveSection(selection) {
        var isSelected = (_task && _task.id === context.selectedNoteID());
        var noteSave = selection.selectAll('.note-save')
            .data((isSelected ? [_task] : []), function(d) { return d.status + d.id; });
        ...
        //     _task = _task.update({ newCategory: val });
        //     var osm = services.osm;
        //     if (osm) {
        //         osm.replaceNote(_task);  // update note cache
        //     }
        //     noteSave
        ...
        noteSaveEnter
            ...
            .text(function() {
                return _task.isNew() ? t('note.newDescription') : t('note.newComment');
            });
    ...
    function clickSave(d) {
        ...
        var osm = services.osm;
        if (osm) {
            osm.postTaskCreate(d, function(err, task) {
    ...
    function clickStatus(d) {
        ...
        if (osm) {
            ...
            osm.postTaskUpdate(d, setStatus, function(err, task) {
                dispatch.call('change', task);
            });
        }
    }
    ...
    function clickComment(d) {
        ...
        var osm = services.osm;
        if (osm) {
            osm.postTaskUpdate(d, d.status, function(err, task) {
                dispatch.call('change', task);
            });
        }
    }
    ...
    taskingEditor.task = function(val) {
        if (!arguments.length) return _task;
        _task = val;
        return taskingEditor;
    };


    return utilRebind(taskingEditor, dispatch, 'on');
}

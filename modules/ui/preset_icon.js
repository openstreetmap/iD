...
export function uiPresetIcon() {
    ...
    function renderPointBorder(container, drawPoint) {
        let pointBorder = container
            .selectAll('.preset-icon-point-border')
            .data(drawPoint ? [0] : []);
    ...
    function renderCategoryBorder(container, category) {
        let categoryBorder = container
            .selectAll('.preset-icon-category-border')
            .data(category ? [0] : []);
        ...
        svgEnter
            ...
            .attr(
                'd',
                'M9.5,7.5 L25.5,7.5 L28.5,12.5 L49.5,12.5 C51.709139,12.5 53.5,14.290861 53.5,16.5 L53.5,43.5 C53.5,45.709139 51.709139,47.5 49.5,47.5 L10.5,47.5 C8.290861,47.5 6.5,45.709139 6.5,43.5 L6.5,12.5 L9.5,7.5 Z',
            );
    ...
    function renderCircleFill(container, drawVertex) {
        let vertexFill = container
            .selectAll('.preset-icon-fill-vertex')
            .data(drawVertex ? [0] : []);
    ...
    function renderSquareFill(container, drawArea, tagClasses) {
        let fill = container
            .selectAll('.preset-icon-fill-area')
            .data(drawArea ? [0] : []);
        ...
        ['fill', 'stroke'].forEach((klass) => {
            fillEnter
                ...
                .attr(
                    'd',
                    `M${c1} ${c1} L${c1} ${c2} L${c2} ${c2} L${c2} ${c1} Z`,
                )
        ...
        fill.selectAll('path.stroke').attr(
            'class',
            `area stroke ${tagClasses}`,
        );
    ...
    function renderLine(container, drawLine, tagClasses) {
        ...
        line.selectAll('path.stroke').attr(
            'class',
            `line stroke ${tagClasses}`,
        );
        line.selectAll('path.casing').attr(
            'class',
            `line casing ${tagClasses}`,
        );
    }

    function renderRoute(container, drawRoute, p) {
        let route = container
            .selectAll('.preset-icon-route')
            .data(drawRoute ? [0] : []);
        ...
        if (drawRoute) {
            let routeType =
                p.tags.type === 'waterway' ? 'waterway' : p.tags.route;
            ...
            for (let i in segmentPresetIDs) {
                ...
                const segmentTagClasses = svgTagClasses().getClassesString(
                    segmentPreset.tags,
                    '',
                );
                route
                    .selectAll(`path.stroke.segment${i}`)
                    .attr(
                        'class',
                        `segment${i} line stroke ${segmentTagClasses}`,
                    );
                route
                    .selectAll(`path.casing.segment${i}`)
                    .attr(
                        'class',
                        `segment${i} line casing ${segmentTagClasses}`,
                    );
            }
        }
    }
    ...
    function renderSvgIcon(
        ...
        let icon = container.selectAll('.preset-icon').data(picon ? [0] : []);
        ...
        icon.attr('class', 'preset-icon ' + (geom ? geom + '-geom' : ''))
            .classed('category', category)
            .classed('framed', isFramed)
            .classed('preset-icon-iD', isiDIcon);

        icon.selectAll('svg').attr(
            'class',
            'icon ' +
                picon +
                ' ' +
                (!isiDIcon && geom !== 'line' ? '' : tagClasses),
        );

        icon.selectAll('use').attr('href', '#' + picon);
    }

    function renderImageIcon(container, imageURL) {
        let imageIcon = container
            .selectAll('img.image-icon')
            .data(imageURL ? [0] : []);
    ...
    function render() {
        ...
        if (
            ...
            ((p.tags.type === 'route' &&
                ...
                p.tags.type === 'waterway')
        ) {
            geom = 'route';
        }

        const showThirdPartyIcons =
        ...
        const isFramed =
            drawVertex || drawArea || drawLine || drawRoute || isCategory;
        ...
        let container = selection.selectAll('.preset-icon-container').data([0]);

...
export function uiPhotoviewer(context) {
    ...
    function photoviewer(selection) {
        ...
        function setPhotoTagButton() {
            ...
            const isActiveForService =
                addPhotoIdButton.has(service) &&
            ...
            function renderAddPhotoIdButton(service, shouldDisplay) {
                const button = selection
                    .selectAll('.set-photo-from-viewer')
                ...
                const buttonEnter = button
                    ...
                    .call(
                        uiTooltip()
                            .title(() =>
                                t.append(
                                    'inspector.set_photo_from_viewer.enable',
                                ),
                            )
                            .placement('right'),
                    );
                ...
                buttonEnter.merge(button).on('click', function (e) {
                    e.preventDefault();
                    e.stopPropagation();
                    ...
                    const action = (graph) =>
                        context.selectedIDs().reduce((graph, entityID) => {
                            ...
                            const action = actionChangeTags(entityID, {
                                ...tags,
                                [activeServiceId]: image.id,
                            });
                            return action(graph);
                        }, graph);
                ...
                if (service === 'panoramax') {
                    const panoramaxControls = selection.select(
                        '.panoramax-wrapper .pnlm-zoom-controls.pnlm-controls',
                    );

                    panoramaxControls.style(
                        'margin-top',
                        shouldDisplay ? '36px' : '6px',
                    );
                }
                ...
                if (
                    entities
                        .map((entity) => entity.tags[service])
                    ...
                    entities
                        .map((entity) =>
                            entity.extent(context.graph()).center(),
                        )
                        .every(
                            (loc) =>
                                geoSphericalDistance(loc, activeImage.loc) >
                                100,
                        )
                ) {
                ...
                if (disabled) {
                    button.call(
                        uiTooltip()
                            .title(() =>
                                t.append(
                                    `inspector.set_photo_from_viewer.disable.${reason}`,
                                ),
                            )
                            .placement('right'),
                    );
        ...
        function buildResizeListener(target, eventName, dispatch, options) {
            ...
            function startResize(d3_event) {
                ...
                if (resizeOnX) {
                    ...
                    const viewerMargin = parseInt(
                        d3_select('.photoviewer').style('margin-left'),
                        10,
                    );
                    var newWidth = clamp(
                        startWidth + d3_event.clientX - startX,
                        minWidth,
                        mapWidth - viewerMargin * 2,
                    );
                ...
                dispatch.call(
                    eventName,
                    target,
                    subtractPadding(utilGetDimensions(target, true), target),
                );
            }
    ...
    photoviewer.onMapResize = function () {
        ...
        if (
            photoDimensions[0] > mapDimensions[0] ||
            photoDimensions[1] >
                mapDimensions[1] - menuHeight - viewerMargin * 2
        ) {
            ...
            dispatch.call(
                'resize',
                photoviewer,
                subtractPadding(setPhotoDimensions, photoviewer),
            );
        }
    };
    ...
    function subtractPadding(dimensions, selection) {
        return [
            dimensions[0] -
                parseFloat(selection.style('padding-left')) -
                parseFloat(selection.style('padding-right')),
            dimensions[1] -
                parseFloat(selection.style('padding-top')) -
                parseFloat(selection.style('padding-bottom')),
        ];
    }

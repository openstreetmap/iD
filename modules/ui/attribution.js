...
export function uiAttribution(context) {
    ...
    function render(selection, data, klass) {
        let div = selection.selectAll(`.${klass}`).data([0]);

        div = div.enter().append('div').attr('class', klass).merge(div);

        let attributions = div
        ...
        let copyright = attributions
            .selectAll('.copyright-notice')
            .data((d) => {
                let notice = d.copyrightNotices(
                    context.map().zoom(),
                    context.map().extent(),
                );
    ...
    function update() {
        ...
        _selection.call(
            render,
            overlays.filter((s) => s.validZoom(z)),

...
export function uiKeepRightDetails(context) {
    ...
    function issueDetail(d) {
        ...
        if (
            localizer.hasTextForStringId(
                `QA.keepRight.errorTypes.${itemType}.title`,
            )
        ) {
            return t.html(
                `QA.keepRight.errorTypes.${itemType}.description`,
                replacements,
            );
        } else {
            return t.html(
                `QA.keepRight.errorTypes.${parentIssueType}.description`,
                replacements,
            );
        }
    }
    ...
    function keepRightDetails(selection) {
        ...
        let relatedEntities = [];
        descriptionEnter
            .selectAll('.error_entity_link, .error_object_link')
            .attr('href', '#')
            .each(function () {

...
export function uiFeatureList(context) {
    ...
    function featureList(selection) {
        ...
        header.append('h2').call(t.append('inspector.feature_list'));
        ...
        function features() {
            ...
            const locationMatch =
                sexagesimal.pair(q.toUpperCase()) || dmsMatcher(q);

            const coordResult = [];
            if (locationMatch) {
                const latLon = [
                    Number(locationMatch[0]),
                    Number(locationMatch[1]),
                ];
            ...
            const idMatch =
                !locationMatch &&
                q.match(
                    /(?:^|\W)(node|way|relation|note|[nwr])\W{0,2}0*([1-9]\d*)(?:\W|$)/i,
                );

            const idResult = [];
            if (idMatch) {
                var elemType =
                    idMatch[1] === 'note' ? idMatch[1] : idMatch[1].charAt(0);
            ...
            for (var id in allEntities) {
                ...
                var type =
                    (matched && matched.name()) || utilDisplayType(entity.id);
                var extent = entity.extent(graph);
                var distance = extent
                    ? geoSphericalDistance(visibleCenter, extent.center())
                    : 0;
            ...
            (_geocodeResults || []).forEach(function (d) {
                if (d.osm_type && d.osm_id) {
                    ...
                    var type =
                        (matched && matched.name()) || utilDisplayType(id);
        ...
        function drawList() {
            ...
            var resultsIndicator = list
                .selectAll('.no-results-item')
                .data([0])
                .enter()
                .append('button')
            ...
            list.selectAll('.no-results-item').style(
                'display',
                value.length && !results.length ? 'block' : 'none',
            );

            list.selectAll('.geocode-item').style(
                'display',
                value && _geocodeResults === undefined ? 'block' : 'none',
            );
            ...
            label.each(function (d) {
                d3_select(this).call(
                    svgIcon('#iD-icon-' + d.geometry, 'pre-text'),
                );
            });
            ...
            label
                ...
                .classed(
                    ...
                    (d) =>
                        d.entity &&
                        d.entity.type === 'relation' &&
                        d.entity.tags.colour &&
                        isColourValid(d.entity.tags.colour),
                )
                .style(
                    'border-color',
                    (d) =>

...
export function uiSuccess(context) {
        ...
        return Promise.all([
            ...
            if (ociResources.length) {
                ...
                return locationManager
                    .mergeLocationSets(ociResources)
                    .then(() => {
                        _oci = {
    ...
    function success(selection) {
        let header = selection.append('div').attr('class', 'header fillL');

        header.append('h2').call(t.append('success.just_edited'));
        ...
        summary
            .append('h3')
            .call(
                t.append('success.thank_you' + (_location ? '_location' : ''), {
                    where: _location,
                }),
            );
        ...
        summaryDetail.append('div').html(
            t.html('success.changeset_id', {
                changeset_id: {
                    html: `<a href="${changesetURL}" target="_blank">${_changeset.id}</a>`,
                },
            }),
        );
        ...
        if (showDonationMessage !== false) {
            ...
            supporting.append('h3').call(t.append('success.supporting.title'));

            supporting.append('p').call(t.append('success.supporting.details'));
        ...
        ensureOSMCommunityIndex().then((oci) => {
            ...
            oci.resources.forEach((resource) => {
                ...
                resource.resolved = resolveStrings(
                    resource,
                    oci.defaults,
                    localizer,
                );
    ...
    function showCommunityLinks(selection, resources) {
        ...
        communityLinks.append('h3').call(t.append('success.like_osm'));
    ...
    function showCommunityDetails(d) {
        ...
        if (
            d.resolved.extendedDescriptionHTML ||
            (d.languageCodes && d.languageCodes.length)
        ) {
            selection.append('div').call(
                uiDisclosure(context, `community-more-${d.id}`, false)
                    .expanded(false)
        ...
        if (nextEvents.length) {
            selection
                .append('div')
                .call(
                    uiDisclosure(context, `community-events-${d.id}`, false)
                        .expanded(false)
                        .updatePreference(false)
        ...
        function showMore(selection) {
            let more = selection.selectAll('.community-more').data([0]);
        ...
        function showNextEvents(selection) {
            ...
            itemEnter
                ...
                .text((d) => {
                    ...
                    if (d.i18n && d.id) {
                        name = t(
                            `community.${communityID}.events.${d.id}.name`,
                            { default: name },
                        );
                    }
                    return name;
            ...
            itemEnter
                ...
                .text((d) => {
                    ...
                    if (d.i18n && d.id) {
                        where = t(
                            `community.${communityID}.events.${d.id}.where`,
                            { default: where },
                        );
                    }
                    return where;
            ...
            itemEnter
                ...
                .text((d) => {
                    ...
                    if (d.i18n && d.id) {
                        description = t(
                            `community.${communityID}.events.${d.id}.description`,
                            { default: description },
                        );
                    }
                    return description;

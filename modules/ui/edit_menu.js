...
export function uiEditMenu(context) {
    ...
    var editMenu = function (selection) {
        var isTouchMenu =
            _triggerType.includes('touch') || _triggerType.includes('pen');
        ...
        var buttonsEnter = buttons
            ...
            .on('mouseenter.highlight', function (d3_event, d) {
                if (!d.relatedEntityIds || d3_select(this).classed('disabled'))
                    return;
        ...
        buttonsEnter.each(function (d) {
            ...
            d3_select(this)
                ...
                .call(
                    svgIcon(
                        (d.icon && d.icon()) || '#iD-operation-' + d.id,
                        'operation',
                    ),
                );
        });
    ...
    function updatePosition() {
        ...
        if (_menuTop) {
            ...
            if (
                anchorLoc[1] + _menuHeight >
                viewport.height - _vpBottomMargin
            ) {
        ...
        var _verticalOffset = parseFloat(
            utilGetDimensions(d3_select('.top-toolbar-wrap'))[1],
        );
        ...
        function displayOnLeft(viewport) {
            if (localizer.textDirection() === 'ltr') {
                if (
                    anchorLoc[0] + _menuSideMargin + _menuWidth >
                    viewport.width - _vpSideMargin
                ) {

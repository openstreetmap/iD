...
export function behaviorDrawWay(context, wayID, mode, startGraph) {
    ...
    function allowsVertex(d) {
        return (
            d.geometry(context.graph()) === 'vertex' ||
    ...
    var drawWay = function (surface) {
        ...
        _annotation = t(
            (_origWay.nodes.length === (_origWay.isClosed() ? 2 : 1)
    ...
    drawWay.off = function (surface) {
        ...
        d3_select(window).on('keydown.drawWay', null).on('keyup.drawWay', null);
    ...
    function attemptAdd(d, loc, doAdd) {
        ...
        checkGeometry(true /* includeDrawNode */);
        if (
            (d && d.properties && d.properties.nope) ||
        ...
        attemptAdd(d, node.loc, function () {
            context.replace(function actionReplaceDrawNode(graph) {
                ...
                return graph.replace(
                    graph.entity(wayID).addNode(node.id, _nodeIndex),
                );
            }, _annotation);
        });
    };
    ...
    function followMode() {
        ...
        try {
            ...
            const isDrawingArea =
                _origWay.nodes[0] === _origWay.nodes.slice(-1)[0];

            const [secondLastNodeId, lastNodeId] = _origWay.nodes.slice(
                isDrawingArea ? -3 : -2,
            );
            ...
            if (
                !lastNodeId ||
                !secondLastNodeId ||
                !historyGraph.hasEntity(lastNodeId) ||
                !historyGraph.hasEntity(secondLastNodeId)
            ) {
                context
                    .ui()
                    .flash.duration(4000)
            ...
            const lastNodesParents = historyGraph
                .parentWays(historyGraph.entity(lastNodeId))
                .filter((w) => w.id !== wayID);
            const secondLastNodesParents = historyGraph
                .parentWays(historyGraph.entity(secondLastNodeId))
                .filter((w) => w.id !== wayID);

            const featureType = getFeatureType(lastNodesParents);

            if (
                lastNodesParents.length !== 1 ||
                secondLastNodesParents.length === 0
            ) {
                context
                    .ui()
                    .flash.duration(4000)
            ...
            // multiple parents.
            if (
                !secondLastNodesParents.some(
                    (n) => n.id === lastNodesParents[0].id,
                )
            ) {
                context
                    .ui()
                    ...
                    .label(
                        t.append(
                            `operations.follow.error.intersection_of_different_ways.${featureType}`,
                        ),
                    )();
                return;
            }
            ...
            // in descending order
            const isDescendingPastZero =
                indexOfLast === way.nodes.length - 2 && indexOfSecondLast === 0;

            let nextNodeIndex =
                indexOfLast +
                (indexOfLast > indexOfSecondLast && !isDescendingPastZero
                    ? 1
                    : -1);
            // if we're following a closed way and we pass the first/last node, the  next index will be -1
            if (nextNodeIndex === -1)
                nextNodeIndex =
                    indexOfSecondLast === 1 ? way.nodes.length - 2 : 1;

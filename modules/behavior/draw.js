...
export function behaviorDraw(context) {
    ...
    const keybinding = utilKeybinding('draw');

    const _hover = behaviorHover(context)
        .altDisables(true)
        .ignoreVertex(true)
        .on('hover', context.ui().sidebar.hover);
    const _edit = behaviorEdit(context);

    const _closeTolerance = 4;
    const _tolerance = 12;
    ...
    function datum(d3_event) {
        ...
        const isNote = mode && (mode.id.indexOf('note') !== -1);
    ...
    function pointerdown(d3_event) {
        ...
        const pointerLocGetter = utilFastMouse(this);
        _downPointer = {
    ...
    function pointerup(d3_event) {
        ...
        const downPointer = _downPointer;
        _downPointer = null;
        ...
        const t2 = +new Date();
        const p2 = downPointer.pointerLocGetter(d3_event);
        const dist = geoVecLength(downPointer.downLoc, p2);
    ...
    function pointermove(d3_event) {
        if (_downPointer &&
            ...
            const p2 = _downPointer.pointerLocGetter(d3_event);
            const dist = geoVecLength(_downPointer.downLoc, p2);
            if (dist >= _closeTolerance) {
    ...
    function click(d3_event, loc) {
        ...
        if (target && target.type === 'node' && allowsVertex(target)) {   // Snap to a node
            ...
            const locLatLng = context.projection.invert(loc);
    ...
    function space(d3_event) {
        ...
        const currSpace = context.map().mouse();
        if (_disableSpace && _lastSpace) {
            const dist = geoVecLength(_lastSpace, currSpace);
            if (dist > _tolerance) {
                _disableSpace = false;
            }
        }

...
export function actionStraightenWay(selectedIDs: EntityId[], projection: Projection): Action {
    ...
    function allNodes(graph: coreGraph): osmNode[] {
        let startNodes: NodeId[] = [];
        let endNodes: NodeId[] = [];
        let remainingWays: NodeId[][] = [];
        const selectedWays = selectedIDs.filter(function (w): w is WayId {
            return graph.entity(w).type === 'way';
        });
        const selectedNodes = selectedIDs.filter(function (n): n is NodeId {
        ...
        let currNode = utilArrayDifference(startNodes, endNodes).concat(
            utilArrayDifference(endNodes, startNodes),
        )[0];

        // Create nested function outside of loop to avoid "function in loop" lint error
        const getNextWay = function (currNode: NodeId, remainingWays: NodeId[][]) {

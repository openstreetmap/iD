...
import { type EntityId, type NodeId, type RelationId } from '../osm';
...
export interface ActionSplit extends Action {
    ...
    waysForNode(nodeId: EntityId, graph: coreGraph): osmWay[];
...
//
export function actionSplit(nodeIds: NodeId[], newWayIds?: EntityId[]): ActionSplit {
    // accept single ID for backwards-compatibility
    if (typeof nodeIds === 'string') nodeIds = [nodeIds];

    var _wayIDs: EntityId[];
    ...
    function split(graph: coreGraph, nodeId: NodeId, wayA: osmWay, newWayId: EntityId | undefined, otherNodeIds: NodeId[]) {
    ...
    function waysForNodes(nodeIds: EntityId[], graph: coreGraph) {
    ...
    function waysForNode(nodeId: EntityId, graph: coreGraph): osmWay[] {

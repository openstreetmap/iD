...
interface MemberWithIndex extends RelationMember {
    index?: number;
}

export function actionAddMember(
    ...
    function addWayMember(relation: osmRelation, graph: coreGraph) {
        ...
        for (i = 0; i < joined.length; i++) {
            ...
            for (k = 0; k < segment.length; k++) {
                item = segment[k];
                const way = graph.entity<osmWay>(item.id);
        ...
        const wayMembers: RelationMember[] = [];

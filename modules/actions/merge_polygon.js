...
export function actionMergePolygon(ids, newRelationId) {
    ...
    var action = function (graph) {
        ...
        var polygons = entities.multipolygon
            .reduce(function (polygons, m) {
        ...
        if (entities.multipolygon.length > 0) {
            var oldestID = utilOldestID(
                entities.multipolygon.map((entity) => entity.id),
            );
            relation = entities.multipolygon.find(
                (entity) => entity.id === oldestID,
            );
        } else {
    ...
    action.disabled = function (graph) {
        ...
        if (!entities.multipolygon.length) {
            ...
            sharedMultipolygons = sharedMultipolygons.filter(
                function (relation) {
                    return (
                        relation.members.length === entities.closedWay.length
                    );
                },
            );

...
export function actionCircularize(wayId: WayId, projection: Projection) {
    const action: Action = function (graph, t) {
        ...
        const keyNodes = nodes.filter(
            (n) => graph.parentWays(n).length > 1 || n.hasInterestingTags(),
        );
        ...
        for (i = 0; i < keyPoints.length; i++) {
            ...
            const nearNodes: Record<string, number> = {};
            const inBetweenNodes: NodeId[] = [];
            let totalAngle, eachAngle;
            ...
            const distance = geoVecLength(centroid, keyPoints[i]) || 1e-4;
            keyPoints[i] = [
    ...
    function getMaxAngle(centroid: Vec2, radius: number) {
        ...
        const numberOfPoints = Math.min(
            MAX_VERTICES,
            Math.max(MIN_VERTICES, Math.round((radiusM * Math.PI) / MAX_SEGMENT_LENGTH) * 2),
        );
    ...
    function makeConvex(graph: coreGraph) {
        ...
        for (i = 0; i < hull.length - 1; i++) {
            ...
            for (j = 1; j < indexRange; j++) {
                const point = geoVecInterp(hull[i], hull[i + 1], j / indexRange);
                const node = nodes[(j + startIndex) % nodes.length].move(projection.invert(point));
                graph = graph.replace(node);
            }
        }
        return graph;
    ...
    action.disabled = function (graph) {
        ...
        for (i = 0; i < hull.length; i++) {
            actualPoint = hull[i];
            const actualDist = geoVecLength(actualPoint, centroid);
        ...
        for (i = 0; i < hull.length; i++) {
            ...
            const startAngle = Math.atan2(
                actualPoint[1] - centroid[1],
                actualPoint[0] - centroid[0],
            );

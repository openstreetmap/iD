...
import type { osmWay, EntityId, NodeId, osmRelation, WayId } from '../osm';
import type { Action } from '../core/history';
import type { coreGraph } from '../core';

export interface ActionJoin extends Action {
    resultingWayNodesLength(graph: coreGraph): number;
}
...
//
export function actionJoin(ids: WayId[]): ActionJoin {
    ...
    const action: ActionJoin = function (graph) {
        const ways = ids.map(graph.entity<osmWay>, graph);
    ...
    action.disabled = function (graph) {
        ...
        const joined = osmJoinWays(ids.map(graph.entity<osmWay>, graph), graph);
        if (joined.length > 1) {
            return 'not_adjacent';
        }
        ...
        const sortedParentRelations = function (id: EntityId) {
            return graph
        ...
        for (i = 0; i < ids.length - 1; i++) {
            for (let j = i + 1; j < ids.length; j++) {
                const path1 = graph.childNodes(graph.entity(ids[i])).map(function (e) {
                    return e.loc;
                });
                const path2 = graph.childNodes(graph.entity(ids[j])).map(function (e) {
                    return e.loc;
                });
            ...
            parents.forEach(function (parent) {
                if (
                    ...
                    parent.members.some(function (m) {
                        return nodeIds.indexOf(m.id as NodeId) >= 0;
                    })
                ) {
                    relation = parent;
                }
    ...
    function canSumTags(key: TagKey, tagsA: Tags, tagsB: Tags) {
        return (
            osmSummableTags.has(key) && Number.isFinite(+tagsA[key]) && Number.isFinite(+tagsB[key])
        );
    }

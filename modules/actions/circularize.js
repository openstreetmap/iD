...
export function actionCircularize(wayId, projection, maxAngle) {
    ...
    var action = function (graph, t) {
        ...
        if (keyNodes.length === 1) {
            ...
            var oppositeIndex = Math.floor(
                (index + nodes.length / 2) % nodes.length,
            );
        ...
        for (i = 0; i < keyPoints.length; i++) {
            ...
            keyPoints[i] = [
                centroid[0] +
                    ((keyPoints[i][0] - centroid[0]) / distance) * radius,
                centroid[1] +
                    ((keyPoints[i][1] - centroid[1]) / distance) * radius,
            ];
    ...
    action.makeConvex = function (graph) {
        ...
        for (i = 0; i < hull.length - 1; i++) {
            ...
            for (j = 1; j < indexRange; j++) {
                ...
                var node = nodes[(j + startIndex) % nodes.length].move(
    ...
    action.disabled = function (graph) {
        ...
        for (i = 0; i < hull.length; i++) {
            ...
            var endAngle = Math.atan2(
                nextPoint[1] - centroid[1],
                nextPoint[0] - centroid[0],
            );

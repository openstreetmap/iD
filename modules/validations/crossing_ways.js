...
export function validationCrossingWays(context) {
    ...
    function getFeatureType(entity, graph) {
        ...
        if (
            hasTag(tags, 'highway') &&
            osmRoutableHighwayTagValues[tags.highway]
        )
            return 'highway';
    ...
    function isLegitCrossing(tags1, featureType1, tags2, featureType2) {
        ...
        if (
            taggedAsIndoor(tags1) &&
            taggedAsIndoor(tags2) &&
            level1 !== level2
        ) {
        ...
        if (
            featureType1 === 'waterway' &&
            featureType2 === 'highway' &&
            tags2.man_made === 'pier'
        )
            return true;
        if (
            featureType2 === 'waterway' &&
            featureType1 === 'highway' &&
            tags1.man_made === 'pier'
        )
            return true;
    ...
    function tagsForConnectionNodeIfAllowed(
        ...
        if (featureTypes === 'aeroway-highway') {
            const isServiceRoad =
                entity1.tags.highway === 'service' ||
                entity2.tags.highway === 'service';
            const isPath =
                entity1.tags.highway in osmPathHighwayTagValues ||
                entity2.tags.highway in osmPathHighwayTagValues;
            // only significant roads get the aeroway=aircraft_crossing tag
            return isServiceRoad || isPath
                ? {}
        ...
        if (featureType1 === featureType2) {
            if (featureType1 === 'highway') {
                var entity1IsPath =
                    osmPathHighwayTagValues[entity1.tags.highway];
                var entity2IsPath =
                ...
                if (
                    ...
                    if (
                        ...
                        pathFeature.tags.highway === 'footway' &&
                        pathFeature.tags.footway === 'sidewalk'
                    ) {
                        return {};
                    }
                    if (
                        [
                            ...
                            'traffic_signals',
                            'uncontrolled',
                        ].indexOf(pathFeature.tags.crossing) !== -1
                    ) {
                        ...
                        if ('crossing:markings' in pathFeature.tags) {
                            tags['crossing:markings'] =
                                pathFeature.tags['crossing:markings'];
                        }
                        return tags;
                    }
            ...
            if (featureTypes.indexOf('highway') !== -1) {
                if (featureTypes.indexOf('railway') !== -1) {
                    ...
                    var isTram =
                        entity1.tags.railway === 'tram' ||
                        entity2.tags.railway === 'tram';
                ...
                if (featureTypes.indexOf('waterway') !== -1) {
                    ...
                    if (
                        hasTag(entity1.tags, 'tunnel') &&
                        hasTag(entity2.tags, 'tunnel')
                    )
                        return null;
                    if (
                        hasTag(entity1.tags, 'bridge') &&
                        hasTag(entity2.tags, 'bridge')
                    )
                        return null;

                    if (
                        highwaysDisallowingFords[entity1.tags.highway] ||
    ...
    function findCrossingsByWay(way1, graph, tree) {
        ...
        for (i = 0; i < way1Nodes.length - 1; i++) {
            ...
            for (j = 0; j < segmentInfos.length; j++) {
                ...
                if (!way2) continue;
                taggedFeature2 = getFeatureWithFeatureTypeTagsForWay(
                    way2,
                    graph,
                );
                ...
                oneOnly =
                    way1FeatureType === 'building' ||
                    way2FeatureType === 'building';
    ...
    function createIssue(crossing, graph) {
        ...
        crossing.wayInfos.sort(function (way1Info, way2Info) {
            ...
            if (type1 === type2) {
                return (
                    utilDisplayLabel(way1Info.way, graph) >
                    utilDisplayLabel(way2Info.way, graph)
                );
        ...
        var featureTypes = [
            crossing.wayInfos[0].featureType,
            crossing.wayInfos[1].featureType,
        ];

        var connectionTags = tagsForConnectionNodeIfAllowed(
            entities[0],
            entities[1],
            graph,
        );
        ...
        var isCrossingIndoors =
            taggedAsIndoor(entities[0].tags) &&
            taggedAsIndoor(entities[1].tags);
        var isCrossingTunnels =
            allowsTunnel(featureType1) &&
            hasTag(entities[0].tags, 'tunnel') &&
            allowsTunnel(featureType2) &&
            hasTag(entities[1].tags, 'tunnel');
        var isCrossingBridges =
            allowsBridge(featureType1) &&
            hasTag(entities[0].tags, 'bridge') &&
            allowsBridge(featureType2) &&
            hasTag(entities[1].tags, 'bridge');
        ...
        if (
            connectionTags &&
            (isCrossingIndoors || isCrossingTunnels || isCrossingBridges)
        ) {
        ...
        var uniqueID =
            crossing.crossPoint[0].toFixed(4) +
            ',' +
            crossing.crossPoint[1].toFixed(4);
        ...
        return new validationIssue({
            ...
            dynamicFixes: function (context) {
                ...
                if (
                    !mode ||
                    mode.id !== 'select' ||
                    mode.selectedIDs().length !== 1
                )
                    return [];

                var selectedIndex =
                    this.entityIds[0] === mode.selectedIDs()[0] ? 0 : 1;
                var selectedFeatureType = this.data.featureTypes[selectedIndex];
                var otherFeatureType =
                    this.data.featureTypes[selectedIndex === 0 ? 1 : 0];

                var fixes = [];

                if (connectionTags) {
                    fixes.push(makeConnectWaysFix(this.data.connectionTags));
                    let lessLikelyConnectionTags =
                        tagsForConnectionNodeIfAllowed(
                            entities[0],
                            entities[1],
                            graph,
                            true,
                    ...
                    if (
                        ...
                        !isEqual(connectionTags, lessLikelyConnectionTags)
                    ) {
                        fixes.push(
                            makeConnectWaysFix(lessLikelyConnectionTags),
                        );
                    }
                }
                ...
                if (isCrossingIndoors) {
                    fixes.push(
                        new validationIssueFix({
                            icon: 'iD-icon-layers',
                            title: t.append(
                                'issues.fix.use_different_levels.title',
                            ),
                        }),
                    );
                } else if (
        ...
        function showReference(selection) {
            selection
                ...
                .call(
                    t.append(
                        'issues.crossing_ways.' + crossingTypeID + '.reference',
                    ),
                );
        }
    }
        ...
        return new validationIssueFix({
            ...
            onClick: function (context) {
                ...
                var action = function actionAddStructure(graph) {
                    ...
                    var structLengthMeters =
                        crossedWay &&
                        isFinite(crossedWay.tags.width) &&
                        Number(crossedWay.tags.width);
                    if (!structLengthMeters) {
                        // if no explicit width is set, approximate the width based on the tags
                        structLengthMeters =
                            crossedWay && crossedWay.impliedLineWidthMeters();
                    }
                    if (structLengthMeters) {
                    ...
                    var a1 =
                        geoAngle(edgeNodes[0], edgeNodes[1], projection) +
                        Math.PI;
                    var a2 =
                        geoAngle(
                            graph.entity(crossedEdge[0]),
                            graph.entity(crossedEdge[1]),
                            projection,
                        ) + Math.PI;
                    ...
                    structLengthMeters =
                        (structLengthMeters / 2 / Math.sin(crossingAngle)) * 2;
                    ...
                    structLengthMeters = Math.min(
                        Math.max(structLengthMeters, 4),
                        50,
                    );
                    ...
                    var projectedAngle = geoVecAngle(
                        projEdgeNode1,
                        projEdgeNode2,
                    );

                    var projectedCrossingLoc = geomToProj(crossingLoc);
                    var linearToSphericalMetersRatio =
                        geoVecLength(projEdgeNode1, projEdgeNode2) /
                        geoSphericalDistance(
                            edgeNodes[0].loc,
                            edgeNodes[1].loc,
                        );

                    function locSphericalDistanceFromCrossingLoc(
                        angle,
                        distanceMeters,
                    ) {
                        var lengthSphericalMeters =
                            distanceMeters * linearToSphericalMetersRatio;
                        return projToGeom([
                            projectedCrossingLoc[0] +
                                Math.cos(angle) * lengthSphericalMeters,
                            projectedCrossingLoc[1] +
                                Math.sin(angle) * lengthSphericalMeters,
                        ]);
                    }

                    var endpointLocGetter1 = function (lengthMeters) {
                        return locSphericalDistanceFromCrossingLoc(
                            projectedAngle,
                            lengthMeters,
                        );
                    };
                    ...
                    function determineEndpoint(edge, endNode, locGetter) {
                        ...
                        var crossingToEdgeEndDistance = geoSphericalDistance(
                            crossingLoc,
                            endNode.loc,
                        );

                        if (
                            crossingToEdgeEndDistance - idealLengthMeters >
                            minEdgeLengthMeters
                        ) {
                        ...
                        var splitAction = actionSplit([newNode.id]).limitWays(
                            resultWayIDs,
                        ); // only split selected or created ways

                        // do the split
                        graph = splitAction(graph);
                        if (splitAction.getCreatedWayIDs().length) {
                            resultWayIDs.push(
                                splitAction.getCreatedWayIDs()[0],
                            );
                        }
                    ...
                    var structEndNode1 = determineEndpoint(
                        edge,
                        edgeNodes[1],
                        endpointLocGetter1,
                    );
                    var structEndNode2 = determineEndpoint(
                        [edgeNodes[0].id, structEndNode1.id],
                        edgeNodes[0],
                        endpointLocGetter2,
                    );

                    var structureWay = resultWayIDs
                        .map(function (id) {
                    ...
                    if (bridgeOrTunnel === 'bridge') {
                        ...
                        var tunnelValue = 'yes';
                        if (
                            getFeatureType(structureWay, graph) === 'waterway'
                        ) {
            ...
            onClick: function (context) {
                ...
                context.perform(function actionConnectCrossingWays(graph) {
                    ...
                    edges.forEach(function (edge) {
                        ...
                        if (
                            (!nearby.node.hasInterestingTags() ||
                                nearby.node.isCrossing()) &&
                            nearby.distance < mergeThresholdInMeters
                        ) {
    ...
    function makeChangeLayerFix(higherOrLower) {
        return new validationIssueFix({
            ...
            title: t.append(
                'issues.fix.tag_this_as_' + higherOrLower + '.title',
            ),
            onClick: function (context) {
                var mode = context.mode();
                if (!mode || mode.id !== 'select') return;
                ...
                if (
                    !this.issue.entityIds.some(function (entityId) {

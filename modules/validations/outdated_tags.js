...
export function validationOutdatedTags() {
    ...
    function oldTagIssues(entity, graph) {
        ...
        const nsiDiff = nsiResult
            ? utilTagDiff(oldTags, nsiResult.newTags)
            : [];
        ...
        if (_dataDeprecated) {
            ...
            if (
                entity.type === 'way' &&
                entity.isClosed() &&
                entity.tags.traffic_calming === 'island' &&
                !entity.tags.highway
            ) {
        ...
        const deprecationDiffContext = Object.keys(oldTags)
            .filter((key) =>
                deprecatedTags?.some(
                    (deprecated) => deprecated.replace?.[key] !== undefined,
                ),
            )
        ...
        if (deprecationDiff.length) {
            const isOnlyAddingTags = !deprecationDiff.some(
                (d) => d.type === '-',
            );
            ...
            issues.push(
                new validationIssue({
                    ...
                    message: (context) => {
                        ...
                        const feature = utilDisplayLabel(
                            currEntity,
                            context.graph(),
                            /* verbose */ true,
                        );
                        ...
                        return t.append(
                            `issues.outdated_tags.${prefix}message`,
                            { feature },
                        );
                    },
                    reference: (selection) =>
                    ...
                    dynamicFixes: () => {
                        let fixes = [
                            new validationIssueFix({
                                title: t.append(
                                    'issues.fix.upgrade_tags.title',
                                ),
                                onClick: (context) => {
                                    context.perform(
                                        (graph) =>
                                            doUpgrade(graph, deprecationDiff),
                ...
                new validationIssue({
                    ...
                    message: (context) => {
                        ...
                        return isOnlyAddingTags
                            ? t.append(
                                  'issues.outdated_tags.noncanonical_brand.message_incomplete',
                                  { feature },
                              )
                            : t.append(
                                  'issues.outdated_tags.noncanonical_brand.message',
                                  { feature },
                              );
                    },
                    reference: (selection) =>
                        showReference(
                            selection,
                            t.append(
                                'issues.outdated_tags.noncanonical_brand.reference',
                            ),
                            nsiDiff,
                        ),
                    entityIds: [entity.id],
                    ...
                    dynamicFixes: () => {
                        let fixes = [
                            new validationIssueFix({
                                ...
                                onClick: (context) => {
                                    context.perform(
                                        (graph) => doUpgrade(graph, nsiDiff),
                                        t('issues.fix.upgrade_tags.annotation'),
                                    );
                                },
                            }),
                            new validationIssueFix({
                                title: t.append('issues.fix.tag_as_not.title', {
                                    name: nsiResult.matched.displayName,
                                }),
                                onClick: (context) => {
                                    context.perform(
                                        addNotTag,
                                        t('issues.fix.tag_as_not.annotation'),
                                    );
                                },
                            }),
                        ];
        ...
        function showReference(selection, reference, tagDiff) {
            ...
            enter.append('strong').call(t.append('issues.suggested'));

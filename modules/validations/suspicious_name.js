...
export function validationSuspiciousName(context) {
    ...
    function nameMatchesPresetName(name, preset) {
        ...
        return (
            ...
            preset.aliases().some((alias) => name === alias.toLowerCase())
        );
    }
    ...
    function isGenericName(name, tags, preset) {
        name = name.toLowerCase();
        return (
            nameMatchesRawTag(name, tags) ||
            nameMatchesPresetName(name, preset) ||
            isGenericMatchInNsi(tags)
        );
    }
    ...
    let validation = function checkGenericName(entity) {
        ...
        for (let key in tags) {
            ...
            if (isGenericName(value, tags, preset)) {
                ...
                issues.push(
                    makeGenericNameIssue(entity.id, key, value, langCode),
                );
            }
        }

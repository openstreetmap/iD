...
export function validationDisconnectedWay() {
    ...
    var validation = function checkDisconnectedWay(entity, graph) {
            ...
            new validationIssue({
                ...
                message: function (context) {
                    var entity =
                        ...
                        context.hasEntity(this.entityIds[0]);
                    var label =
                        entity && utilDisplayLabel(entity, context.graph());
                    return t.append(
                        'issues.disconnected_way.routable.message',
                        { count: this.entityIds.length, highway: label },
                    );
                },
        ...
        function makeFixes(context) {
            ...
            if (singleEntity) {
                if (singleEntity.type === 'way' && !singleEntity.isClosed()) {
                    ...
                    var startFix = makeContinueDrawingFixIfAllowed(
                        textDirection,
                        singleEntity.first(),
                        'start',
                    );
                    ...
                    var endFix = makeContinueDrawingFixIfAllowed(
                        textDirection,
                        singleEntity.last(),
                        'end',
                    );
        ...
        function routingIslandForEntity(entity) {
            ...
            function queueParentWays(node) {
                graph.parentWays(node).forEach(function (parentWay) {
                    if (
                        !routingIsland.has(parentWay) && // only check each feature once
        ...
        function isRoutableWay(way, ignoreInnerWays) {
            ...
            return graph.parentRelations(way).some(function (parentRelation) {
                ...
                if (
                    ...
                    (!ignoreInnerWays ||
                        parentRelation.memberById(way.id).role !== 'inner')
                )
                    return true;
        ...
        function makeContinueDrawingFixIfAllowed(
            textDirection,
            vertexID,
            whichEnd,
        ) {
            ...
            var useLeftContinue =
                (whichEnd === 'start' && textDirection === 'ltr') ||
                (whichEnd === 'end' && textDirection === 'rtl');

            return new validationIssueFix({
                icon:
                    'iD-operation-continue' + (useLeftContinue ? '-left' : ''),
                title: t.append(
                    'issues.fix.continue_from_' + whichEnd + '.title',
                ),
                entityIds: [vertexID],
                onClick: function (context) {
                    ...
                    if (
                        !context.editable() ||
                        !map.trimmedExtent().contains(vertex.loc)
                    ) {

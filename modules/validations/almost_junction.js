    ...
    const validation = function checkAlmostJunction(entity, graph) {
                ...
                new validationIssue({
                    ...
                    message: function (context) {
                        ...
                        if (this.entityIds[0] === this.entityIds[2]) {
                            return entity1
                                ? t.append(
                                      'issues.almost_junction.self.message',
                                      {
                            ...
                            const entity2 = context.hasEntity(
                                this.entityIds[2],
                            );
                            return entity1 && entity2
                                ? t.append('issues.almost_junction.message', {
                                      feature: utilDisplayLabel(
                                          entity1,
                                          context.graph(),
                                      ),
            ...
            let fixes = [
                new validationIssueFix({
                    ...
                    onClick: function (context) {
                        const annotation = t(
                            'issues.fix.connect_almost_junction.annotation',
                        );
                        ...
                        const nearEndNodes = findNearbyEndNodes(
                            endNode,
                            crossWay,
                        );
                        if (nearEndNodes.length > 0) {
                            const collinear = findSmallJoinAngle(
                                midNode,
                                endNode,
                                nearEndNodes,
                            );
                        ...
                        const edgeNodes = [
                            context.entity(targetEdge[0]),
                            context.entity(targetEdge[1]),
                        ];
                        const closestNodeInfo = geoSphericalClosestNode(
                            edgeNodes,
                            crossLoc,
                        );
                        ...
                        if (closestNodeInfo.distance < WELD_TH_METERS) {
                            context.perform(
                                actionMergeNodes(
                                    [closestNodeInfo.node.id, endNode.id],
                                    closestNodeInfo.node.loc,
                                ),
                                annotation,
                            );
        ...
        function showReference(selection) {
            selection
                ...
                .call(
                    t.append(
                        'issues.almost_junction.highway-highway.reference',
                    ),
                );
        }
        ...
        function findConnectableEndNodesByExtension(way) {
            ...
            indices.forEach((nodeIndex) => {
                ...
                testNodes[nodeIndex] = testNodes[nodeIndex].move(
                    connectionInfo.cross_loc,
                );
        ...
        function findSmallJoinAngle(midNode, tipNode, endNodes) {
            ...
            endNodes.forEach((endNode) => {
                const a1 =
                    geoAngle(midNode, tipNode, context.projection) + Math.PI;
                const a2 =
                    geoAngle(midNode, endNode, context.projection) + Math.PI;
      ...
      based on node joining threshold and extension distance */
        ...
        function canConnectWays(way, way2) {
            ...
            if (
                (hasTag(way.tags, 'bridge') || hasTag(way2.tags, 'bridge')) &&
                !(hasTag(way.tags, 'bridge') && hasTag(way2.tags, 'bridge'))
            )
                return false;
            if (
                (hasTag(way.tags, 'tunnel') || hasTag(way2.tags, 'tunnel')) &&
                !(hasTag(way.tags, 'tunnel') && hasTag(way2.tags, 'tunnel'))
            )
                return false;
        ...
        function canConnectByExtend(way, endNodeIdx) {
            ...
            const midNid =
                endNodeIdx === 0
                    ? way.nodes[1]
                    : way.nodes[way.nodes.length - 2]; // the other node of the edge
            ...
            for (let i = 0; i < segmentInfos.length; i++) {
                ...
                let crossLoc = geoLineIntersection(
                    [tipNode.loc, extTipLoc],
                    [nA.loc, nB.loc],
                );
                if (crossLoc) {
                    return {

...
export default {
    ...
    inferGeometry: function (tagMap) {
        ...
        var keyValueDoesNotImplyArea = function (key) {
            return (
                utilArrayIntersection(tagMap[key], Object.keys(_areaKeys[key]))
                    .length > 0
            );
        };
        var keyValueImpliesLine = function (key) {
            return (
                utilArrayIntersection(tagMap[key], Object.keys(_lineKeys[key]))
                    .length > 0
            );
        };
    ...
    addRule: function (selector) {
        var rule = {
            ...
            inferredGeometry: this.inferGeometry(
                this.buildTagMap(selector),
                this._areaKeys,
            ),
            ...
            findIssues: function (entity, graph, issues) {
                if (
                    this.geometryMatches(entity, graph) &&
                    this.matches(entity)
                ) {
                    var severity =
                        Object.keys(selector).indexOf('error') > -1
                            ? 'error'
                            : 'warning';

...
function filterAvailableLayers(photoContex) {
    ...
    const fromYear = fromDateString
        ? new Date(fromDateString).getFullYear()
        : 2016;
    ...
    return Array.from(_vegbilderCache.wfslayers.values()).filter(
        ({ layerInfo }) =>
            layerInfo.year >= fromYear &&
            (!toYear || layerInfo.year <= toYear) &&
            ((!layerInfo.is_sphere && showsFlat) ||
                (layerInfo.is_sphere && showsPano)),
    );
}
...
function loadWFSLayer(projection, cache, tiles) {
    ...
    Promise.all(
        tiles.map((tile) => loadTile(cache, cache.layerInfo.name, tile)),
...
function orderSequences(projection, cache) {
    ...
    const imageSequences = Array.from(grouped.values()).reduce(
        (imageSequences, imageGroup) => {
            imageGroup.sort((a, b) => {
            ...
            for (const [lastImage, image] of d3_pairs(imageGroup)) {
                ...
                if (
                    ...
                    image.captured_at.valueOf() -
                        lastImage.captured_at.valueOf() <=
                        20000
                ) {
...
export default {
    ...
    selectImage: function (context, key, keepOrientation) {
        ...
        const wrap = context
            .container()
            .select('.photoviewer .vegbilder-wrapper');
        ...
        attribution
            ...
            .attr(
                'href',
                `https://vegbilder.atlas.vegvesen.no/?year=${d.captured_at.getFullYear()}&lat=${d.loc[1]}&lng=${d.loc[0]}&view=image&imageId=${d.key}`,
            )
            .call(t.append('vegbilder.view_on'));

        _currentFrame = d.is_sphere ? _pannellumFrame : _planeFrame;

        _currentFrame.showPhotoFrame(wrap).selectPhoto(d, keepOrientation);
    ...
    showViewer: function (context) {
        const viewer = context.container().select('.photoviewer');
        const isHidden = viewer
            .selectAll('.photo-wrapper.vegbilder-wrapper.hide')
            .size();
    ...
    hideViewer: function (context) {
        ...
        context
            .container()
            .selectAll('.viewfield-group, .sequence')
    ...
    setStyles: function (context, hovered, reset) {
        ...
        const hoveredImageKeys =
            hoveredSequence?.images.map((d) => d.key) ?? [];
        ...
        const selectedImageKeys =
            selectedSequence?.images.map((d) => d.key) ?? [];

        // highlight sibling viewfields on either the selected or the hovered sequences
        const highlightedImageKeys = utilArrayUnion(
            hoveredImageKeys,
            selectedImageKeys,
        );
        ...
        context
            .container()
            .selectAll('.layer-vegbilder .viewfield-group')
            .classed(
                'highlighted',
                (d) => highlightedImageKeys.indexOf(d.key) !== -1,
            )
        ...
        context
            .container()
            .selectAll('.layer-vegbilder .sequence')
        ...
        context
            .container()
            .selectAll('.layer-vegbilder .viewfield-group .viewfield')

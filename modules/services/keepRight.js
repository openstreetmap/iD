...
const _krRuleset = [
    ...
    30,
    40, 50, 60, 70, 90, 100, 110, 120, 130, 150, 160, 170, 180, 190, 191, 192,
    193, 194, 195, 196, 197, 198, 200, 201, 202, 203, 204, 205, 206, 207, 208,
    210, 220, 230, 231, 232, 270, 280, 281, 282, 283, 284, 285, 290, 291, 292,
    293, 294, 295, 296, 297, 298, 300, 310, 311, 312, 313, 320, 350, 360, 370,
    380, 390, 400, 401, 402, 410, 411, 412, 413,
];
...
function tokenReplacements(d) {
    ...
    for (let i = 1; i < errorMatch.length; i++) {
        // skip first
        ...
        if (idType && capture) {
            ...
            if (_krData.localizeStrings[compare]) {
                ...
                capture = t(
                    'QA.keepRight.error_parts.' +
                        _krData.localizeStrings[compare],
                );
            } else {
...
function parseError(capture, idType) {
    ...
    if (_krData.localizeStrings[compare]) {
        // some replacement strings can be localized
        capture = t(
            'QA.keepRight.error_parts.' + _krData.localizeStrings[compare],
        );
    }
    ...
    function linkURL(d) {
        return {
            html: `<a class="kr_external_link" target="_blank" href="${d}">${d}</a>`,
        };
    }
    ...
    function parse231(capture) {
        ...
        items.forEach((item) => {
            ...
            if (match !== null && match.length > 2) {
                newList.push(
                    linkEntity('w' + match[1]) +
                        ' ' +
                        t('QA.keepRight.errorTypes.231.layer', {
                            layer: match[2],
                        }),
                );
            }
    ...
    function parse370(capture) {
        ...
        if (match && match.length) {
            return t('QA.keepRight.errorTypes.370.including_the_name', {
                name: match[1],
            });
        }
        return '';
...
export default {
    ...
    init() {
        fileFetcher.get('keepRight').then((d) => (_krData = d));
                ...
                .then((data) => {
                    ...
                    data.features.forEach((feature) => {
                        ...
                        switch (whichType) {
                            ...
                            case '300':
                                description =
                                    'This highway is missing a maxspeed tag';
                                break;
                            case '411':
                            case '412':
                            case '413':
    ...
    postUpdate(d, callback) {
        if (_cache.inflightPost[d.id]) {
            return callback(
                { message: 'Error update already inflight', status: -2 },
                d,
            );
        }
        ...
        d3_json(url, { signal: controller.signal }).finally(() => {

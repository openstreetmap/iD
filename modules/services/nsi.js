...
const notNames =
    /:(colou?r|type|forward|backward|left|right|etymology|pronunciation|wikipedia)$/i;
...
function setNsiSources() {
    const nsiVersion =
        packageJSON.dependencies['name-suggestion-index'] ||
        packageJSON.devDependencies['name-suggestion-index'];
...
function loadNsiPresets() {
    return Promise.all([
        ...
        Object.values(vals[0].presets).forEach(
            (preset) => (preset.suggestion = true),
        );
        ...
        Object.values(vals[0].presets).forEach((preset) => {
            if (preset.tags['brand:wikidata']) {
                preset.removeTags = {
                    'brand:wikipedia': '*',
                    ...(preset.removeTags || preset.addTags || preset.tags),
                };
            }
    ...
    return Promise.all([
        ...
        Object.keys(_nsi.data).forEach((tkv) => {
            ...
            items.forEach((item) => {
                ...
                const locationSetID = locationManager.locationSetID(
                    item.locationSet,
                );
                ...
                const fakeFeature = {
                    id: locationSetID,
                    properties: { id: locationSetID, area: 1 },
                };
        ...
        matcher.locationIndex = (bbox) => {
            const validHere = locationManager.locationSetsAt([
                bbox[0],
                bbox[1],
            ]);
            const results = [];
...
function gatherNames(tags) {
    ...
    if (t === 'transit') {
        patterns = {
            primary: /^network$/i,
            alternate:
                /^(operator|operator:\w+|network:\w+|\w+_name|\w+_name:\w+)$/i,
        };
    } else if (t === 'flags') {
        patterns = {
        ...
        patterns = {
            ...
            alternate:
                /^(brand|brand:\w+|operator|operator:\w+|\w+_name|\w+_name:\w+)/i,
        };
    } else if (t === 'operators') {
        testNameFragments = true;
        patterns = {
        ...
        patterns = {
            primary: /^(name|name:\w+)$/i,
            alternate:
                /^(brand|brand:\w+|network|network:\w+|operator|operator:\w+|\w+_name|\w+_name:\w+)/i,
        };
    }
...
function gatherTuples(tryKVs, tryNames) {
    ...
    ['primary', 'alternate'].forEach((whichName) => {
        ...
        const arr = Array.from(tryNames[whichName]).sort(
...
function _upgradeTags(tags, loc) {
    ...
    const foundQID =
        _nsi.qids.get(tags.wikidata) || _nsi.qids.get(tags.wikipedia);
    ...
    for (let i = 0; i < tuples.length; i++) {
        ...
        for (let j = 0; j < hits.length; j++) {
            ...
            if (
                ...
                !itemQID ||
                itemQID === notQID || // No `*:wikidata` or matched a `not:*:wikidata`
                    ...
                    if (nameHits.some((hit) => hit.itemID === itemID)) {
                        ...
                        if (branch) {
                            if (notBranches.test(branch)) {
                                ...
                                const branchHits = _nsi.matcher.match(
                                    k,
                                    v,
                                    branch,
                                    loc,
                                ...
                                if (branchHits && branchHits.length) {
                                    ...
                                    if (
                                        branchHits[0].match === 'primary' ||
                                        branchHits[0].match === 'alternate'
                                    ) {

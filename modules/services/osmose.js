...
export default {
    ...
    loadIssues(projection) {
        ...
        tiles.forEach((tile) => {
            ...
            let [x, y, z] = tile.xyz;
            let url =
                `${_osmoseUrlRoot}/issues/${z}/${x}/${y}.geojson?` +
                utilQsString(params);
            ...
            .map((itemType) => {
                ...
                const cacheData = (data) => {
                    ...
                    if (!cl) {
                        ...
                        console.log(
                            `Osmose strings request (${itemType}) had unexpected data`,
                        );
    ...
    postUpdate(issue, callback) {
        if (_cache.inflightPost[issue.id]) {
            return callback(
                { message: 'Issue update already inflight', status: -2 },
                issue,
            );
        }

...
export function modeSelect(context, selectedIDs) {
    ...
    function parentWaysIdsOfSelection(onlyCommonParents) {
        ...
        for (var i = 0; i < selectedIDs.length; i++) {
            ...
            parents = (
                onlyCommonParents ? utilArrayIntersection : utilArrayUnion
            )(parents, currParents);
            if (!parents.length) {
                return [];
            }
    ...
    function childNodeIdsOfSelection(onlyCommon) {
        ...
        for (var i = 0; i < selectedIDs.length; i++) {
            ...
            childs = (onlyCommon ? utilArrayIntersection : utilArrayUnion)(
                childs,
                currChilds,
            );
            if (!childs.length) {
    ...
    function parentWayIdForVertexNavigation() {
        ...
        if (
            _focusedParentWayId &&
            parentIds.indexOf(_focusedParentWayId) !== -1
        ) {
    ...
    mode.enter = function () {
        ...
        if (!_behaviors.length) {
            ...
            _behaviors = [
                ...
                _breatheBehavior,
                behaviorHover(context).on(
                    'hover',
                    context.ui().sidebar.hoverModeSelect,
                ),
                _selectBehavior,
        ...
        keybinding
            ...
            .on(uiCmd('⇧⌥↓'), nudgeSelection([0, 100]))
            .on(
                utilKeybinding.plusKeys.map((key) => uiCmd('⇧' + key)),
                scaleSelection(1.05),
            )
            .on(
                utilKeybinding.plusKeys.map((key) => uiCmd('⇧⌥' + key)),
                scaleSelection(Math.pow(1.05, 5)),
            )
            .on(
                utilKeybinding.minusKeys.map((key) => uiCmd('⇧' + key)),
                scaleSelection(1 / 1.05),
            )
            .on(
                utilKeybinding.minusKeys.map((key) => uiCmd('⇧⌥' + key)),
                scaleSelection(1 / Math.pow(1.05, 5)),
            )
        ...
        function nudgeSelection(delta) {
            return function () {
                ...
                if (moveOp.disabled()) {
                    ...
                    context.perform(
                        actionMove(selectedIDs, delta, context.projection),
                        moveOp.annotation(),
                    );
            ...
            return function () {
                ...
                function scalingDisabled() {
                    if (tooSmall()) {
                        ...
                        extent.percentContainedIn(context.map().extent()) < 0.8
                    ) {
                        return 'too_large';
                    } else if (
                        someMissing() ||
                        selectedIDs.some(incompleteRelation)
                    ) {
                    ...
                    function tooSmall() {
                        ...
                        return (
                            dLon < geoMetersToLon(1, extent[1][1]) &&
                    ...
                    function someMissing() {
                        ...
                        if (osm) {
                            let missing = nodes.filter(function (n) {
                ...
                if (disabled) {
                    ...
                    context
                        ...
                        .label(
                            t.append(
                                'operations.scale.' + disabled + '.' + multi,
                            ),
                        )();
                } else {
                    const pivot = context.projection(extent.center());
                    const annotation = t(
                        'operations.scale.annotation.' +
                            (isUp ? 'up' : 'down') +
                            '.feature',
        ...
        function didDoubleUp(d3_event, loc) {
            ...
            if (entity instanceof osmWay && target.classed('target')) {
                var choice = geoChooseEdge(
                    context.graph().childNodes(entity),
                    loc,
                    context.projection,
                );
                ...
                context.perform(
                    actionAddMidpoint(
                        { loc: entity.loc, edge: entity.edge },
                        osmNode(),
                    ),
                    t('operations.add.annotation.vertex'),
                );
        ...
        function selectElements() {
            ...
            if (_focusedParentWayId) {
                surface
                    .selectAll(utilEntitySelector([_focusedParentWayId]))
        ...
        function selectParent(d3_event) {
            ...
            var parentIds = _focusedParentWayId
                ? [_focusedParentWayId]
                : parentWaysIdsOfSelection(false);
    ...
    mode.exit = function () {
        ...
        if (
            ...
            (entity.members.length === 0 ||
                (entity.members.length === 1 && !entity.members[0].role))
        ) {
            // the user added this relation but didn't edit it at all, so just delete it
            var deleteAction = actionDeleteRelation(
                entity.id,
                true /* don't delete untagged members */,
            );
            context.perform(
                deleteAction,
                t('operations.delete.annotation.relation'),
            );
            context.validator().validate();
        }
    };

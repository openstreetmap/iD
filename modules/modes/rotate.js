...
export function modeRotate(context, entityIDs) {
    ...
    var annotation =
        entityIDs.length === 1
            ? t(
                  'operations.rotate.annotation.' +
                      context.graph().geometry(entityIDs[0]),
              )
            : t('operations.rotate.annotation.feature', {
                  n: entityIDs.length,
              });
    ...
    function doRotate(d3_event) {
        ...
        var currAngle = Math.atan2(
            currMouse[1] - _pivot[1],
            currMouse[0] - _pivot[0],
        );

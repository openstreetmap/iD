...
* __`background`__ - The value of the `id` property of the source in iD's
  ...
  custom backgrounds (if not already present) and selected. The bare id `custom`
  (from before multiple customs were supported) selects the migrated saved custom
  layer when present. Conversely, the URL reflects only the *currently selected*
  custom background, so it cannot share the full list of saved custom backgrounds
  at once.<br/>

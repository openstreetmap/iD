...
const presetsVersion =
    packageJson.devDependencies['@openstreetmap/id-tagging-schema'];
/* eslint-disable no-process-env */
const presetsUrl = (
    process.env.ID_PRESETS_CDN_URL ||
...
function buildData() {
    ...
    fs.writeFileSync(
        'data/territory_languages.json',
        prettyStringify(territoryLanguages, { maxLength: 9999 }),
    );
    ...
    fs.writeFileSync(
        'data/languages.json',
        prettyStringify(languageInfo, { maxLength: 200 }),
    );
    fs.writeFileSync(
        'dist/data/languages.min.json',
    ...
    let tasks = [
        ...
        Promise.all([
            ...
            .then(() =>
                ...
                fetch(
                    'https://raw.githubusercontent.com/openstreetmap/id-tagging-schema/interim/icons.json',
                ),
            )
            .then((response) => response.json())
            .then((cuttingEdgeIcons) => {
                cuttingEdgeIcons
                    .filter((icon) => /^fa[srb]-/.test(icon))
                    .forEach((icon) => faIcons.add(icon));
            })
    ...
    return (_currBuild = Promise.all(tasks)
        .then(() => {
            console.timeEnd(END);
            console.log('');
            _currBuild = null;
...
function generateTerritoryLanguages() {
    ...
    Object.keys(allRawInfo).forEach((territoryCode) => {
        ...
        territoryLanguages[territoryCode.toLowerCase()] = langCodes
            .sort((langCode1, langCode2) => {
                let popPercent1 = parseFloat(
                    territoryLangInfo[langCode1]._populationPercent,
                );
                let popPercent2 = parseFloat(
                    territoryLangInfo[langCode2]._populationPercent,
                );
                if (popPercent1 === popPercent2) {
                    return langCode1.localeCompare(langCode2, 'en', {
                        sensitivity: 'base',
                    });
                }
...
function writeEnJson() {
    ...
    const readImagery = fs.readFileSync(
        'node_modules/editor-layer-index/i18n/en.yaml',
        'utf8',
    );
    const readCommunity = fs.readFileSync(
        'node_modules/osm-community-index/i18n/en.yaml',
        'utf8',
    );
    const readManualImagery = fs.readFileSync(
    ...
    return Promise.all([
        ...
        props.forEach(function (prop) {
            if (enjson.en[prop]) {
                console.error(
                    `Error: Reserved property '${prop}' already exists in core strings`,
                );
                process.exit(1);
            }
        });
...
function writeFaIcons(faIcons) {
    Array.from(faIcons).forEach(function (key) {
        ...
        const def = fontawesome.findIconDefinition({
            prefix: prefix,
            iconName: name,
        });
        try {
            fs.writeFileSync(
                `svg/fontawesome/${key}.svg`,
                fontawesome.icon(def).html.toString(),
            );
        } catch (error) {
...
function fetchOrRequire(url) {
    if (url.startsWith('.')) {
        return import(url, { with: { type: 'json' } }).then((module) => ({
            json: () => module.default,
        }));
    } else {
        return fetch(url);

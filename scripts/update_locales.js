...
if (process.env.transifex_password) {
    ...
    // }
    transifexApi.setup({
        auth: JSON.parse(fs.readFileSync('./transifex.auth', 'utf8')).password,
    });
}
/* eslint-enable no-process-env */

const dataShortcuts = JSON.parse(
    fs.readFileSync('data/shortcuts.json', 'utf8'),
);
...
async function getResourceInfo(resourceId, callback) {
    try {
        ...
        console.error(
            `error while getting resource language stats collection for ${resourceId}`,
        );
        callback(err);
    }
}
function gotResourceInfo(err, results) {
    ...
    results.forEach((info) => {
        info.forEach((stat) => {
            let code = stat.relationships.language.data.id
                .substr(2)
                .replace(/_/g, '-');
            ...
            let coveragePart =
                stat.attributes[type] /
                stat.attributes.total_strings /
                results.length;

            if (coverageByLocaleCode[code] === undefined)
                coverageByLocaleCode[code] = 0;
            ...
            if (code === 'en') {
                ...
                getLanguageInfo(code, (err, info) => {
                    ...
                    if (coverage === undefined) {
                        console.log(
                            `Could not get language coverage for ${code}`,
                            coverageByLocaleCode,
                        );
                        process.exit(1);
                    }
...
function getLanguage(resourceId) {
    return async (code, callback) => {
        try {
            ...
            const url =
                await transifexApi.ResourceTranslationsAsyncDownload.download({
                    resource: {
                        data: {
                            type: 'resources',
                            id: `o:${transifexOrganization}:p:${transifexProject}:r:${resourceId}`,
                        },
                    },
                    language: { data: { type: 'languages', id: `l:${code}` } },
                    // fetch only reviewed strings for some languages
                    mode:
                        reviewedOnlyLangs.indexOf(code) !== -1
                            ? 'reviewed'
                            : 'default',
                });
            ...
            console.error(
                `error while getting translations for ${resourceId}, language ${code}`,
                err,
            );
            callback(err);
        }

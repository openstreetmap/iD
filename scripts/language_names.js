...
function getCLDROverrides() {
    ...
    return {
        ...
        pih: {
            nativeName: 'Pitkern–Norfuk',
            names: { en: 'Pitcairn-Norfolk', ty: 'Pitcairnais' },
        },
        ...
        xph: {
            ...
            names: { en: 'North Midlands Tasmanian' },
        },
...
function getLangNamesInNativeLang() {
    ...
    langDirectoryPaths.forEach((code) => {
        ...
        let languageObj = JSON.parse(fs.readFileSync(languagesPath, 'utf8'))
            .main[code];
    ...
    let englishNamesByCode = JSON.parse(
        fs.readFileSync(`${cldrMainDir}en/languages.json`, 'utf8'),
...
export function languageNamesInLanguageOf(code) {
    ...
    let translatedLangsByCode = JSON.parse(
        fs.readFileSync(languageFilePath, 'utf8'),
    ).main[code].localeDisplayNames.languages;
...
export function scriptNamesInLanguageOf(code) {
    ...
    let allTranslatedScriptsByCode = JSON.parse(
        fs.readFileSync(languageFilePath, 'utf8'),
    ).main[code].localeDisplayNames.scripts;

    let translatedScripts = {};
    referencedScripts.forEach((script) => {
        if (
            !allTranslatedScriptsByCode[script] ||
            script === allTranslatedScriptsByCode[script]
        )
            return;

...
const sources = JSON.parse(
    fs.readFileSync(
        require.resolve('editor-layer-index/imagery.geojson'),
        'utf8',
    ),
);
...
if (fs.existsSync('./data/manual_imagery.json')) {
    /** @type {any[]} */
    const manualImagery = JSON.parse(
        fs.readFileSync('./data/manual_imagery.json', 'utf8'),
    );
    // we can include additional imagery sources that aren't in the index
    sources.features = sources.features.filter(
        (source) =>
            !manualImagery.find(
                (manualSource) => manualSource.id === source.properties?.id,
            ),
    );
...
sources.features.forEach((feature) => {
    ...
    if (source.type === 'wms') {
        const projection =
            source.available_projections &&
            supportedWMSProjections.find(
                (p) => source.available_projections.indexOf(p) !== -1,
            );
        if (!projection) return;
        if (
            sources.features.some(
                (other) =>
                    other.properties?.name === source.name &&
                    other.properties?.type !== source.type,
            )
        )
            return;
    ...
    if (extent.min_zoom || extent.max_zoom) {
        im.zoomExtent = [extent.min_zoom || 0, extent.max_zoom || 22];
    }

...
var languages = ['de', 'en', 'es', 'fr', 'lv', 'tr'];
var langkeys = {};

eval(fs.readFileSync('./locale.js', 'utf8'));
for (var i = 0; i < languages.length; i++) {
    eval(fs.readFileSync('./' + languages[i] + '.js', 'utf8'));
    langkeys[languages[i]] = getKeys(locale[languages[i]]).sort();
}

// for (var i = 1; i < languages.length - 1; i++) {
// 
//     var changes = WordDiff.diff(
//         langkeys[languages[i]].join(','),
//         langkeys[languages[i + 1]].join(','));
// 
// 
//     console.warn('actual:' + '\n' + changes.del);
...
var allkeys = [];
_.forEach(langkeys, function(l) {
    allkeys = _.union(allkeys, l);
});
...
_.forEach(langkeys, function(l, k) {
    var missing = _.difference(allkeys, l);
    if (missing.length) {
        console.log('\n', k, 'is missing\n\n', _.difference(allkeys, l).join(','));
    }
});

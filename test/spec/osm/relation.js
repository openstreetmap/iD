describe('iD.osmRelation', function () {
    ...
    it('sets tags as specified', function () {
        expect(iD.osmRelation({ tags: { foo: 'bar' } }).tags).to.eql({
            foo: 'bar',
        });
    });
    ...
    describe('#geometry', function () {
        it("returns 'area' for multipolygons", function () {
            expect(
                iD
                    .osmRelation({ tags: { type: 'multipolygon' } })
                    .geometry(iD.coreGraph()),
            ).to.equal('area');
        });

        it("returns 'relation' for other relations", function () {
            expect(iD.osmRelation().geometry(iD.coreGraph())).to.equal(
                'relation',
            );
        });
    });
    ...
    describe('#isDegenerate', function () {
        ...
        it('returns false for a relation with members', function () {
            expect(
                iD
                    .osmRelation({ members: [{ id: 'a', role: 'inner' }] })
                    .isDegenerate(),
            ).to.equal(false);
        });
    });
    ...
    describe('#isRestriction', function () {
        it("returns true for 'restriction' type", function () {
            expect(
                iD
                    .osmRelation({ tags: { type: 'restriction' } })
                    .isRestriction(),
            ).to.be.true;
        });
        ...
        it("returns true for 'restriction:type' types", function () {
            expect(
                iD
                    .osmRelation({ tags: { type: 'restriction:bus' } })
                    .isRestriction(),
            ).to.be.true;
        });
    ...
    describe('#updateMember', function () {
        it('updates the properties of the relation member at the specified index', function () {
            ...
            expect(r.updateMember({ role: 'backward' }, 0).members).to.eql([
                { role: 'backward' },
            ]);
        });
    });
    ...
    describe('#removeMember', function () {
        it('removes the member at the specified index', function () {
            ...
            expect(r.removeMember(1).members).to.eql([
                { id: 'a' },
                { id: 'c' },
            ]);
        });
    ...
    describe('#replaceMember', function () {
        ...
        it('keeps duplicate members if `keepDuplicates = true`', function () {
            ...
            expect(
                r.replaceMember({ id: 'a' }, { id: 'b', type: 'node' }, true)
                    .members,
            ).to.eql([
                { id: 'b', role: 'b', type: 'node' },
                { id: 'b', role: 'b', type: 'node' },
    ...
    describe('#asJXON', function () {
        ...
        it('includes changeset if provided', function () {
            expect(
                iD.osmRelation().asJXON('1234').relation['@changeset'],
            ).to.equal('1234');
        });
    });

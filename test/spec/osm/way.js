describe('iD.osmWay', function () {
    ...
    describe('#contains', function () {
        it('returns true if the way contains the given node', function () {
            expect(iD.osmWay({ nodes: ['a', 'b', 'c'] }).contains('b')).to.be
                .true;
        });

        it('returns false if the way does not contain the given node', function () {
            expect(iD.osmWay({ nodes: ['a', 'b', 'c'] }).contains('d')).to.be
                .false;
        });
    });
    ...
    describe('#isClosed', function () {
        ...
        it('returns false when the way ends are not equal', function () {
            expect(iD.osmWay({ nodes: 'abc'.split('') }).isClosed()).to.be
                .false;
        });
    ...
    describe('#layer', function () {
        ...
        it('returns 0 when the way has a non numeric layer tag', function () {
            ...
            expect(iD.osmWay({ tags: { layer: 'Infinity' } }).layer()).to.equal(
                0,
            );
        ...
        it('returns 1 for location=overground', function () {
            expect(
                iD.osmWay({ tags: { location: 'overground' } }).layer(),
            ).to.equal(1);
        });

        it('returns -1 for covered=yes', function () {
            expect(iD.osmWay({ tags: { covered: 'yes' } }).layer()).to.equal(
                -1,
            );
        });
        ...
        it('returns -1 for location=underground', function () {
            expect(
                iD.osmWay({ tags: { location: 'underground' } }).layer(),
            ).to.equal(-1);
        });

        it('returns -10 for location=underwater', function () {
            expect(
                iD.osmWay({ tags: { location: 'underwater' } }).layer(),
            ).to.equal(-10);
        });
        ...
        it('returns 10 for power lines', function () {
            ...
            expect(
                iD.osmWay({ tags: { power: 'minor_line' } }).layer(),
            ).to.equal(10);
        });

        it('returns 10 for aerialways', function () {
            expect(
                iD.osmWay({ tags: { aerialway: 'cable_car' } }).layer(),
            ).to.equal(10);
        });
        ...
        it('returns -1 for cuttings', function () {
            expect(iD.osmWay({ tags: { cutting: 'yes' } }).layer()).to.equal(
                -1,
            );
        });
        ...
        it('returns -1 for waterways', function () {
            expect(
                iD.osmWay({ tags: { waterway: 'stream' } }).layer(),
            ).to.equal(-1);
        });

        it('returns -10 for pipelines', function () {
            expect(
                iD.osmWay({ tags: { man_made: 'pipeline' } }).layer(),
            ).to.equal(-10);
        });

        it('returns -10 for boundaries', function () {
            expect(
                iD.osmWay({ tags: { boundary: 'administrative' } }).layer(),
            ).to.equal(-10);
        });
    });
    ...
    describe('#isOneWay', function () {
        ...
        it('returns false when the way has tag oneway=no', function () {
            expect(
                iD.osmWay({ tags: { oneway: 'no' } }).isOneWay(),
                'oneway no',
            ).to.be.false;
            expect(iD.osmWay({ tags: { oneway: '0' } }).isOneWay(), 'oneway 0')
                .to.be.false;
        });

        it('returns true when the way has tag oneway=yes', function () {
            expect(
                iD.osmWay({ tags: { oneway: 'yes' } }).isOneWay(),
                'oneway yes',
            ).to.be.true;
            expect(
                iD.osmWay({ tags: { oneway: '-1' } }).isOneWay(),
                'oneway -1',
            ).to.be.true;
        });
        ...
        it('returns true when the way has tag oneway=reversible', function () {
            expect(
                iD.osmWay({ tags: { oneway: 'reversible' } }).isOneWay(),
                'oneway reversible',
            ).to.be.true;
        });
        ...
        it('returns true when the way has tag oneway=alternating', function () {
            expect(
                iD.osmWay({ tags: { oneway: 'alternating' } }).isOneWay(),
                'oneway alternating',
            ).to.be.true;
        });
        ...
        it('returns true when the way has implied oneway tag (waterway=river, waterway=stream, etc)', function () {
            expect(
                iD.osmWay({ tags: { waterway: 'river' } }).isOneWay(),
                'river',
            ).to.be.true;
            expect(
                iD.osmWay({ tags: { waterway: 'stream' } }).isOneWay(),
                'stream',
            ).to.be.true;
            expect(
                iD.osmWay({ tags: { highway: 'motorway' } }).isOneWay(),
                'motorway',
            ).to.be.true;
            expect(
                iD.osmWay({ tags: { junction: 'roundabout' } }).isOneWay(),
                'roundabout',
            ).to.be.true;
            expect(
                iD.osmWay({ tags: { junction: 'circular' } }).isOneWay(),
                'circular',
            ).to.be.true;
        });
        ...
        it('returns false when the way does not have implied oneway tag', function () {
            expect(
                iD.osmWay({ tags: { highway: 'motorway_link' } }).isOneWay(),
                'motorway_link',
            ).to.be.false;
            expect(
                iD.osmWay({ tags: { highway: 'trunk' } }).isOneWay(),
                'trunk',
            ).to.be.false;
            expect(
                iD.osmWay({ tags: { highway: 'trunk_link' } }).isOneWay(),
                'trunk_link',
            ).to.be.false;
            expect(
                iD.osmWay({ tags: { highway: 'primary' } }).isOneWay(),
                'primary',
            ).to.be.false;
            expect(
                iD.osmWay({ tags: { highway: 'primary_link' } }).isOneWay(),
                'primary_link',
            ).to.be.false;
            expect(
                iD.osmWay({ tags: { highway: 'secondary' } }).isOneWay(),
                'secondary',
            ).to.be.false;
            expect(
                iD.osmWay({ tags: { highway: 'secondary_link' } }).isOneWay(),
                'secondary_link',
            ).to.be.false;
            expect(
                iD.osmWay({ tags: { highway: 'tertiary' } }).isOneWay(),
                'tertiary',
            ).to.be.false;
            expect(
                iD.osmWay({ tags: { highway: 'tertiary_link' } }).isOneWay(),
                'tertiary_link',
            ).to.be.false;
            expect(
                iD.osmWay({ tags: { highway: 'unclassified' } }).isOneWay(),
                'unclassified',
            ).to.be.false;
            expect(
                iD.osmWay({ tags: { highway: 'residential' } }).isOneWay(),
                'residential',
            ).to.be.false;
            expect(
                iD.osmWay({ tags: { highway: 'living_street' } }).isOneWay(),
                'living_street',
            ).to.be.false;
            expect(
                iD.osmWay({ tags: { highway: 'service' } }).isOneWay(),
                'service',
            ).to.be.false;
            expect(
                iD.osmWay({ tags: { highway: 'track' } }).isOneWay(),
                'track',
            ).to.be.false;
            expect(iD.osmWay({ tags: { highway: 'path' } }).isOneWay(), 'path')
                .to.be.false;
        });
        ...
        it('returns false when oneway=no overrides implied oneway tag', function () {
            expect(
                iD
                    .osmWay({ tags: { junction: 'roundabout', oneway: 'no' } })
                    .isOneWay(),
                'roundabout',
            ).to.be.false;
            expect(
                iD
                    .osmWay({ tags: { junction: 'circular', oneway: 'no' } })
                    .isOneWay(),
                'circular',
            ).to.be.false;
            expect(
                iD
                    .osmWay({ tags: { highway: 'motorway', oneway: 'no' } })
                    .isOneWay(),
                'motorway',
            ).to.be.false;
        });
    ...
    describe('#sidednessIdentifier', function () {
        it('returns tag when the tag has implied sidedness', function () {
            expect(
                iD.osmWay({ tags: { natural: 'cliff' } }).sidednessIdentifier(),
            ).to.eql('natural');
            expect(
                iD
                    .osmWay({ tags: { natural: 'coastline' } })
                    .sidednessIdentifier(),
            ).to.eql('coastline');
            expect(
                iD
                    .osmWay({ tags: { barrier: 'retaining_wall' } })
                    .sidednessIdentifier(),
            ).to.eql('barrier');
            expect(
                iD.osmWay({ tags: { barrier: 'kerb' } }).sidednessIdentifier(),
            ).to.eql('barrier');
            expect(
                iD
                    .osmWay({ tags: { barrier: 'guard_rail' } })
                    .sidednessIdentifier(),
            ).to.eql('barrier');
            expect(
                iD
                    .osmWay({ tags: { barrier: 'city_wall' } })
                    .sidednessIdentifier(),
            ).to.eql('barrier');
            expect(
                iD
                    .osmWay({ tags: { man_made: 'embankment' } })
                    .sidednessIdentifier(),
            ).to.eql('man_made');
            expect(
                iD
                    .osmWay({ tags: { 'abandoned:barrier': 'guard_rail' } })
                    .sidednessIdentifier(),
            ).to.eql('barrier');
        });
        ...
        it('returns null when tag does not have implied sidedness', function () {
            expect(
                iD.osmWay({ tags: { natural: 'ridge' } }).sidednessIdentifier(),
            ).to.be.null;
            expect(
                iD.osmWay({ tags: { barrier: 'fence' } }).sidednessIdentifier(),
            ).to.be.null;
            expect(
                iD.osmWay({ tags: { man_made: 'dyke' } }).sidednessIdentifier(),
            ).to.be.null;
            expect(
                iD
                    .osmWay({ tags: { highway: 'motorway' } })
                    .sidednessIdentifier(),
            ).to.be.null;
            expect(
                iD
                    .osmWay({ tags: { 'demolished:highway': 'motorway' } })
                    .sidednessIdentifier(),
            ).to.be.null;
            expect(
                iD
                    .osmWay({ tags: { 'not:natural': 'cliff' } })
                    .sidednessIdentifier(),
            ).to.be.null;
        });
    });
    ...
    describe('#isSided', function () {
        ...
        it('returns false when the way has two_sided=yes', function () {
            expect(iD.osmWay({ tags: { two_sided: 'yes' } }).isSided()).to.be
                .false;
        });

        it('returns true when the tag has implied sidedness', function () {
            expect(iD.osmWay({ tags: { natural: 'cliff' } }).isSided()).to.be
                .true;
            expect(iD.osmWay({ tags: { natural: 'coastline' } }).isSided()).to
                .be.true;
            expect(iD.osmWay({ tags: { barrier: 'retaining_wall' } }).isSided())
                .to.be.true;
            expect(iD.osmWay({ tags: { barrier: 'kerb' } }).isSided()).to.be
                .true;
            expect(iD.osmWay({ tags: { barrier: 'guard_rail' } }).isSided()).to
                .be.true;
            expect(iD.osmWay({ tags: { barrier: 'city_wall' } }).isSided()).to
                .be.true;
            expect(iD.osmWay({ tags: { man_made: 'embankment' } }).isSided()).to
                .be.true;
        });
        ...
        it('returns false when two_sided=yes overrides tag with implied sidedness', function () {
            expect(
                iD
                    .osmWay({ tags: { natural: 'cliff', two_sided: 'yes' } })
                    .isSided(),
            ).to.be.false;
            expect(
                iD
            ...
            expect(
                iD
                    .osmWay({ tags: { barrier: 'kerb', two_sided: 'yes' } })
                    .isSided(),
            ).to.be.false;
            expect(
                iD
        ...
        it('returns true when two_sided=no is on tag with implied sidedness', function () {
            expect(
                iD
                    .osmWay({ tags: { natural: 'cliff', two_sided: 'no' } })
                    .isSided(),
            ).to.be.true;
            expect(
                iD
                    .osmWay({ tags: { natural: 'coastline', two_sided: 'no' } })
                    .isSided(),
            ).to.be.true;
            expect(
                iD
                    .osmWay({
                        tags: { barrier: 'retaining_wall', two_sided: 'no' },
                    })
                    .isSided(),
            ).to.be.true;
            expect(
                iD
                    .osmWay({ tags: { barrier: 'kerb', two_sided: 'no' } })
                    .isSided(),
            ).to.be.true;
            expect(
                iD
                    .osmWay({
                        tags: { barrier: 'guard_rail', two_sided: 'no' },
                    })
                    .isSided(),
            ).to.be.true;
            expect(
                iD
                    .osmWay({ tags: { barrier: 'city_wall', two_sided: 'no' } })
                    .isSided(),
            ).to.be.true;
            expect(
                iD
                    .osmWay({
                        tags: { man_made: 'embankment', two_sided: 'no' },
                    })
                    .isSided(),
            ).to.be.true;
        });
        ...
        it('returns false when the tag does not have implied sidedness', function () {
            expect(iD.osmWay({ tags: { natural: 'ridge' } }).isSided()).to.be
                .false;
            expect(iD.osmWay({ tags: { barrier: 'fence' } }).isSided()).to.be
                .false;
            expect(iD.osmWay({ tags: { man_made: 'dyke' } }).isSided()).to.be
                .false;
            expect(iD.osmWay({ tags: { highway: 'motorway' } }).isSided()).to.be
                .false;
        });
    });
    ...
    describe('#isArea', function () {
        ...
        it('returns true if the way has tag area=yes', function () {
            expect(iD.osmWay({ tags: { area: 'yes' } }).isArea()).to.equal(
                true,
            );
        });
        ...
        it('returns true if the way is closed and has a key in iD.osmAreaKeys', function () {
            expect(
                iD
                    .osmWay({ nodes: ['n1', 'n1'], tags: { building: 'yes' } })
                    .isArea(),
            ).to.equal(true);
        });
        ...
        it('returns true for some highway and railway exceptions', function () {
            ...
            expect(
                iD
                    .osmWay({ nodes: ['n1', 'n1'], tags: { railway: 'wash' } })
                    .isArea(),
                'railway=wash',
            ).to.equal(true);
        });
        ...
        it('returns false if the way is closed and has no keys in iD.osmAreaKeys', function () {
            expect(
                iD.osmWay({ nodes: ['n1', 'n1'], tags: { a: 'b' } }).isArea(),
            ).to.equal(false);
        });
    ...
    describe('#isDegenerate', function () {
        ...
        it('returns true for a circular way with only one unique node', function () {
            expect(iD.osmWay({ nodes: ['a', 'a'] }).isDegenerate()).to.equal(
                true,
            );
        });

        it('returns false for a linear way with two or more nodes', function () {
            expect(iD.osmWay({ nodes: ['a', 'b'] }).isDegenerate()).to.equal(
                false,
            );
        });
        ...
        it('returns true for a linear way that doubles back on itself', function () {
            expect(
                iD.osmWay({ nodes: ['a', 'b', 'a'] }).isDegenerate(),
            ).to.equal(true);
        });

        it('returns true for an area with zero, one, or two unique nodes', function () {
            expect(
                iD.osmWay({ tags: { area: 'yes' }, nodes: [] }).isDegenerate(),
            ).to.equal(true);
            expect(
                iD
                    .osmWay({ tags: { area: 'yes' }, nodes: ['a', 'a'] })
                    .isDegenerate(),
            ).to.equal(true);
            expect(
                iD
                    .osmWay({ tags: { area: 'yes' }, nodes: ['a', 'b', 'a'] })
                    .isDegenerate(),
            ).to.equal(true);
        });
    ...
    describe('#areAdjacent', function () {
        ...
        it('returns false for non-adjacent nodes in the way', function () {
            expect(
                iD.osmWay({ nodes: ['a', 'b', 'c'] }).areAdjacent('a', 'c'),
            ).to.equal(false);
        });
    ...
    describe('#geometry', function () {
        ...
        it("returns 'area' when the way is an area", function () {
            expect(
                iD.osmWay({ tags: { area: 'yes' } }).geometry(iD.coreGraph()),
            ).to.equal('area');
        });
    });
    ...
    describe('#addNode', function () {
        ...
        it('throws RangeError if index outside of array range for linear way', function () {
            var w = iD.osmWay({ nodes: 'ab'.split('') });
            expect(w.addNode.bind(w, 'c', 3)).to.throw(
                RangeError,
                /out of range 0\.\.2/,
                'over range',
            );
            expect(w.addNode.bind(w, 'c', -1)).to.throw(
                RangeError,
                /out of range 0\.\.2/,
                'under range',
            );
    ...
    describe('#updateNode', function () {
        it('throws RangeError if empty way', function () {
            var w = iD.osmWay();
            expect(w.updateNode.bind(w, 'd', 0)).to.throw(
                RangeError,
                /out of range 0\.\.-1/,
            );
        });
        ...
        it('updates an internal node on a circular way at a positive index', function () {
            var w = iD.osmWay({ nodes: 'aba'.split('') });
            expect(w.updateNode('d', 1).nodes.join('')).to.eql(
                'ada',
                'circular',
            );
        });
    ...
    describe('#removeNode', function () {
        ...
        it('removes multiply occurring nodes', function () {
            ...
            expect(w1.removeNode('b').nodes.join('')).to.eql(
                'ac',
                'multiple remove, linear',
            );
            var w2 = iD.osmWay({ nodes: 'abcba'.split('') });
            expect(w2.removeNode('b').nodes.join('')).to.eql(
                'aca',
                'multiple remove, circular',
            );
        });
    ...
    describe('#asJXON', function () {
        it('converts a way to jxon', function () {
            ...
            expect(node.asJXON()).to.eql({
                way: {
                    ...
                    tag: [
                        { keyAttributes: { k: 'highway', v: 'residential' } },
                    ],
                },
            });
        });
        ...
        it('includes changeset if provided', function () {
            expect(iD.osmWay().asJXON('1234').way['@changeset']).to.equal(
                '1234',
            );
        });
    });

describe('iD.geo - vector', function () {
    describe('geoVecEqual', function () {
        ...
        it('tests vectors for equality within epsilon', function () {
            expect(iD.geoVecEqual([1, 2], [1.0000001, 2.0000001], 1e-5)).to.be
                .true;
            expect(iD.geoVecEqual([1, 2], [1.0000001, 2.0000001], 1e-8)).to.be
                .false;
        });
    });
    ...
    describe('geoVecNormalize', function () {
        it('gets unit vectors', function () {
            ...
            expect(iD.geoVecNormalize([-5, 0])).to.eql([-1, 0]);
            expect(iD.geoVecNormalize([1, 1])[0]).to.be.closeTo(
                Math.sqrt(2) / 2,
                1e-6,
            );
            expect(iD.geoVecNormalize([1, 1])[1]).to.be.closeTo(
                Math.sqrt(2) / 2,
                1e-6,
            );
        });
    ...
    describe('geoVecAngle', function () {
        it('returns angle between a and b', function () {
            ...
            expect(iD.geoVecAngle([0, 0], [0, 1])).to.be.closeTo(
                Math.PI / 2,
                1e-6,
            );
            expect(iD.geoVecAngle([0, 0], [-1, 0])).to.be.closeTo(
                Math.PI,
                1e-6,
            );
            expect(iD.geoVecAngle([0, 0], [0, -1])).to.be.closeTo(
    ...
    describe('geoVecNormalizedDot', function () {
        ...
        it('normalized dot product of 45 degrees', function () {
            var a = [0, 2];
            var b = [2, 2];
            expect(iD.geoVecNormalizedDot(a, b)).to.be.closeTo(
                Math.sqrt(2) / 2,
                1e-6,
            );
        });

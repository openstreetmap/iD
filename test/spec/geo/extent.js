describe('iD.geoExtent', function () {
    describe('constructor', function () {
        it('defaults to infinitely empty extent', function () {
            expect(iD.geoExtent().equals([[Infinity, Infinity], [-Infinity, -Infinity]])).toBeTruthy();
        });

        it('constructs via a point', function () {
            var p = [0, 0];
            expect(iD.geoExtent(p).equals([p, p])).toBeTruthy();
        });

        it('constructs via two points', function () {
            var min = [0, 0],
                max = [5, 10];
            expect(iD.geoExtent(min, max).equals([min, max])).toBeTruthy();
        });
        ...
        it('constructs via an iD.geoExtent', function () {
            ...
            expect(iD.geoExtent(extent).equals(extent)).toBeTruthy();
        });
    ...
    describe('#extend', function () {
        ...
        it('returns the minimal extent containing self and the given point', function () {
            expect(iD.geoExtent().extend([0, 0]).equals([[0, 0], [0, 0]])).toBeTruthy();
            expect(iD.geoExtent([0, 0], [0, 0]).extend([5, 10]).equals([[0, 0], [5, 10]])).toBeTruthy();
        });

        it('returns the minimal extent containing self and the given extent', function () {
            expect(iD.geoExtent().extend([[0, 0], [5, 10]]).equals([[0, 0], [5, 10]])).toBeTruthy();
            expect(iD.geoExtent([0, 0], [0, 0]).extend([[4, -1], [5, 10]]).equals([[0, -1], [5, 10]])).toBeTruthy();
        });
    });
    ...
    describe('#contains', function () {
        ...
        it('returns false for a point outside self', function () {
            expect(iD.geoExtent([0, 0], [5, 5]).contains([6, 6])).toBe(false);
        });

        it('returns true for an extent contained by self', function () {
            expect(iD.geoExtent([0, 0], [5, 5]).contains([[1, 1], [2, 2]])).toBe(true);
            expect(iD.geoExtent([1, 1], [2, 2]).contains([[0, 0], [5, 5]])).toBe(false);
        });

        it('returns false for an extent partially contained by self', function () {
            expect(iD.geoExtent([0, 0], [5, 5]).contains([[1, 1], [6, 6]])).toBe(false);
            expect(iD.geoExtent([1, 1], [6, 6]).contains([[0, 0], [5, 5]])).toBe(false);
        });

        it('returns false for an extent not intersected by self', function () {
            expect(iD.geoExtent([0, 0], [5, 5]).contains([[6, 6], [7, 7]])).toBe(false);
            expect(iD.geoExtent([[6, 6], [7, 7]]).contains([[0, 0], [5, 5]])).toBe(false);
        });
    });
    ...
    describe('#intersects', function () {
        ...
        it('returns false for a point outside self', function () {
            expect(iD.geoExtent([0, 0], [5, 5]).intersects([6, 6])).toBe(false);
        });
        ...
        it('returns false for an extent not intersected by self', function () {
            expect(iD.geoExtent([0, 0], [5, 5]).intersects([[6, 6], [7, 7]])).toBe(false);
            expect(iD.geoExtent([[6, 6], [7, 7]]).intersects([[0, 0], [5, 5]])).toBe(false);
        });
    });
    ...
    describe('#split', function () {
        it('splits into four parts', function () {
            ...
            expect(splits[0].equals([[0, 10], [2.5, 15]])).toBeTruthy();
            expect(splits[1].equals([[2.5, 10], [5, 15]])).toBeTruthy();
            expect(splits[2].equals([[2.5, 15], [5, 20]])).toBeTruthy();
            expect(splits[3].equals([[0, 15], [2.5, 20]])).toBeTruthy();
        });
    });

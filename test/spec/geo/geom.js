describe('iD.geo - geometry', function() {
    ...
    describe('geoEdgeEqual', function() {
        ...
        it('returns true for equal edges along same direction', function() {
            expect(iD.geoEdgeEqual(['a', 'b'], ['a', 'b'])).toBe(true);
        });

        it('returns true for equal edges along opposite direction', function() {
            expect(iD.geoEdgeEqual(['a', 'b'], ['b', 'a'])).toBe(true);
        });
    });
    ...
    describe('geoChooseEdge', function() {
        ...
        it('skips the given nodeID at end of way', function() {
            ...
            var nodes = [
                new iD.osmNode({id: 'a', loc: a}),
                new iD.osmNode({id: 'b', loc: b}),
                new iD.osmNode({id: 'c', loc: c}),
                new iD.osmNode({id: 'd', loc: d}),
    ...
    describe('geoHasLineIntersections', function() {
        ...
        it('returns false if no activeID', function() {
            ...
            var b = new iD.osmNode({id: 'b', loc: [4, 2]});
        ...
        it('returns false if there are no intersections', function() {
            ...
            var c = new iD.osmNode({id: 'c', loc: [4, 4]});
            var d = new iD.osmNode({id: 'd', loc: [2, 4]});
            var e = new iD.osmNode({id: 'e', loc: [0, 0]});
            var f = new iD.osmNode({id: 'f', loc: [8, 0]});
            var g = new iD.osmNode({id: 'g', loc: [8, 8]});
        ...
        it('returns true if the activeID is causing intersections', function() {
            ...
            //  h --------- g
            var a = new iD.osmNode({id: 'a', loc: [2, 2]});
            var b = new iD.osmNode({id: 'b', loc: [10, 2]});
            var c = new iD.osmNode({id: 'c', loc: [4, 4]});
            var d = new iD.osmNode({id: 'd', loc: [2, 4]});
            var e = new iD.osmNode({id: 'e', loc: [0, 0]});
            var f = new iD.osmNode({id: 'f', loc: [8, 0]});
    ...
    describe('geoHasSelfIntersections', function() {
        ...
        it('returns false if no activeID', function() {
            ...
            var b = new iD.osmNode({id: 'b', loc: [2, 0]});
        ...
        it('returns true if there are self intersections without a junction (closed way)', function() {
            ...
            //  d     b
            var a = new iD.osmNode({id: 'a', loc: [0, 0]});
            var b = new iD.osmNode({id: 'b', loc: [2, 2]});
            var c = new iD.osmNode({id: 'c', loc: [2, 0]});
            var d = new iD.osmNode({id: 'd', loc: [0, 2]});
            var nodes = [a, b, c, d, a];
            expect(iD.geoHasSelfIntersections(nodes, 'a')).toBe(true);
            expect(iD.geoHasSelfIntersections(nodes, 'b')).toBe(true);
            expect(iD.geoHasSelfIntersections(nodes, 'c')).toBe(true);
            expect(iD.geoHasSelfIntersections(nodes, 'd')).toBe(true);
        });
    ...
    describe('geoPointInPolygon', function() {
        it('says a point in a polygon is on a polygon', function() {
            ...
            var point = [0.5, 0.5];
            expect(iD.geoPointInPolygon(point, poly)).toBe(true);
        });
    ...
    describe('geoPolygonContainsPolygon', function() {
        it('says a polygon in a polygon is in', function() {
            ...
            expect(iD.geoPolygonContainsPolygon(outer, inner)).toBe(true);
        });
    ...
    describe('geoPolygonIntersectsPolygon', function() {
        it('returns true when outer polygon fully contains inner', function() {
            var outer = [[0, 0], [0, 3], [3, 3], [3, 0], [0, 0]];
            var inner = [[1, 1], [1, 2], [2, 2], [2, 1], [1, 1]];
            expect(iD.geoPolygonIntersectsPolygon(outer, inner)).toBe(true);
        });
        ...
        it('returns true when outer polygon partially contains inner (no vertices contained - strict test)', function() {
            var outer = [[0, 0], [0, 3], [3, 3], [3, 0], [0, 0]];
            var inner = [[1, -1], [1, 4], [2, 4], [2, -1], [1, -1]];
            expect(iD.geoPolygonIntersectsPolygon(outer, inner, true)).toBe(true);
        });

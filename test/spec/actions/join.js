describe('iD.actionJoin', function () {
    describe('#disabled', function () {
        ...
        it("returns 'not_eligible' for non-line geometries", function () {
            var graph = iD.coreGraph([iD.osmNode({ id: 'a', loc: [0, 0] })]);

            expect(iD.actionJoin(['a']).disabled(graph)).to.equal(
                'not_eligible',
            );
        });
        ...
        it("returns 'not_adjacent' for ways that don't share the necessary nodes", function () {
            ...
            expect(iD.actionJoin(['-', '=']).disabled(graph)).to.equal(
                'not_adjacent',
            );
        });
        ...
        ['restriction', 'connectivity'].forEach(function (type) {
            ...
            it(
                'returns falsy in situations where a ' +
                    type +
                    "relation wouldn't be damaged (a)",
                function () {
                    // a --> b ==> c
                    // |
                    // d
                    ...
                    expect(iD.actionJoin(['-', '=']).disabled(graph)).not.to.be
                        .ok;
                },
            );
            ...
            it(
                'returns falsy in situations where a ' +
                    type +
                    "restriction wouldn't be damaged (b)",
                function () {
                    //       d
                    //       |
                    // a --> b ==> c
    ...
    it('merges the number of steps', function () {
        let graph = iD.coreGraph([
            ...
            iD.osmWay({
                id: '=',
                nodes: ['b', 'c'],
                tags: { highway: 'steps', step_count: '30' },
            }),
        ]);
        graph = iD.actionJoin(['-', '='])(graph);

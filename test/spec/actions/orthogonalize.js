describe('iD.actionOrthogonalize', function () {
    ...
    describe('closed paths', function () {
        ...
        it('only moves nodes which are near right or near straight', function () {
            ...
            var diff = iD.coreDifference(
                graph,
                iD.actionOrthogonalize('-', projection)(graph),
            );
            expect(Object.keys(diff.changes()).sort()).to.eql([
                'a',
                'b',
                'c',
                'f',
        ...
        it('preserves the shape of skinny quads', function () {
            ...
            for (var i = 0; i < tests.length; i++) {
                ...
                var initialWidth = iD.geoSphericalDistance(
                    graph.entity('a').loc,
                    graph.entity('b').loc,
                );
                graph = iD.actionOrthogonalize('-', projection)(graph);
                var finalWidth = iD.geoSphericalDistance(
                    graph.entity('a').loc,
                    graph.entity('b').loc,
                );
                expect(finalWidth / initialWidth).within(0.9, 1.1);
            }
        });
    });
    ...
    describe('vertices', function () {
        it('orthogonalizes a single vertex in a quad', function () {
            ...
            var diff = iD.coreDifference(
                graph,
                iD.actionOrthogonalize('-', projection, 'b')(graph),
            );
    ...
    describe('#disabled', function () {
        describe('closed paths', function () {
            it('returns "square_enough" for a perfect quad', function () {
                ...
                var result = iD
                    .actionOrthogonalize('-', projection)
                    .disabled(graph);
        ...
        describe('vertex-only', function () {
            it('returns "square_enough" for a vertex in a perfect quad', function () {
                ...
                var result = iD
                    .actionOrthogonalize('-', projection, 'b')
                    .disabled(graph);
                expect(result).to.eql('square_enough');
            });

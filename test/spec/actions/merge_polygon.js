describe('iD.actionMergePolygon', function () {
    ...
    function find(relation, id) {
        return relation.members.find(function (m) {
            return m.id === id;
        });
    }
    ...
    it('merges multipolygon tags', function () {
        var graph = iD.coreGraph([
            iD.osmRelation({
                id: 'r1',
                tags: { type: 'multipolygon', a: 'a' },
            }),
            iD.osmRelation({
                id: 'r2',
                tags: { type: 'multipolygon', b: 'b' },
            }),
        ]);
    ...
    it('merges tags from closed outer ways', function () {
        graph = graph.replace(
            graph.entity('w0').update({ tags: { building: 'yes' } }),
        );
    ...
    it('merges no tags from unclosed outer ways', function () {
        graph = graph.replace(
            graph.entity('w3').update({ tags: { natural: 'water' } }),
        );
    ...
    it('merges no tags from inner ways', function () {
        graph = graph.replace(
            graph.entity('w1').update({ tags: { natural: 'water' } }),
        );
        graph = iD.actionMergePolygon(['w0', 'w1'], 'r')(graph);
    ...
    it("doesn't copy area tags from ways", function () {
        graph = graph.replace(
            graph.entity('w0').update({ tags: { area: 'yes' } }),
        );
        graph = iD.actionMergePolygon(['w0', 'w1'], 'r')(graph);
        var r = graph.entity('r');

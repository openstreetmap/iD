describe('iD.actionCircularize', function () {
    ...
    function isCircular(id, graph) {
        var points = graph.childNodes(graph.entity(id)).map(function (n) {
    ...
    function angle(point1, point2, center) {
        ...
        return (
            (180 / Math.PI) *
            Math.acos(vector1[0] * vector2[0] + vector1[1] * vector2[1])
        );
    }
    ...
    it('creates circle respecting min-angle limit', function () {
        ...
        expect(isCircular('-', graph)).to.be.ok;
        points = graph.childNodes(graph.entity('-')).map(function (n) {
            return projection(n.loc);
        });
        centroid = d3.polygonCentroid(points);
        ...
        expect(angle(points[points.length - 1], points[0], centroid)).to.be.lte(
            20,
        );
    });
    ...
    it('adds new nodes on shared way wound in opposite direction', function () {
        ...
        expect(
            intersection(graph.entity('-').nodes, graph.entity('=').nodes)
                .length,
        ).to.eql(3);

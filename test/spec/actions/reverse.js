describe('iD.actionReverse', function () {
    ...
    describe('reverses directional tags on nodes', function () {
        it('reverses relative directions', function () {
            ...
            expect(graph.entity(node1.id).tags).to.eql({
                direction: 'backward',
            });
        });
        ...
        it('reverses relative directions for arbitrary direction tags', function () {
            var node1 = iD.osmNode({
                tags: { 'traffic_sign:direction': 'forward' },
            });
            var graph = iD.actionReverse(node1.id)(iD.coreGraph([node1]));
            expect(graph.entity(node1.id).tags).to.eql({
    ...
    describe('reverses oneway', function () {
        ...
        it('reverses oneway tags if reverseOneway: true is provided', function () {
            ...
            expect(
                iD
                    .actionReverse('yes', { reverseOneway: true })(graph)
                    .entity('yes').tags,
            ).to.eql({ oneway: '-1' }, 'yes');
            expect(
                iD
                    .actionReverse('no', { reverseOneway: true })(graph)
                    .entity('no').tags,
            ).to.eql({ oneway: 'no' }, 'no');
            expect(
                iD
                    .actionReverse('1', { reverseOneway: true })(graph)
                    .entity('1').tags,
            ).to.eql({ oneway: '-1' }, '1');
            expect(
                iD
        ...
        it('ignores other oneway tags', function () {
            ...
            expect(
                iD
                    .actionReverse('alternating', {
                        reverseOneway: true,
                    })(graph)
                    .entity('alternating').tags,
            ).to.eql({ oneway: 'alternating' }, 'alternating');
            expect(
                iD
                    .actionReverse('reversible', { reverseOneway: true })(graph)
                    .entity('reversible').tags,
            ).to.eql({ oneway: 'reversible' }, 'reversible');
            expect(
                iD
                    .actionReverse('dummy', { reverseOneway: true })(graph)
                    .entity('dummy').tags,
            ).to.eql({ oneway: 'dummy' }, 'dummy');
        });
    });
    ...
    describe('reverses directional keys on ways', function () {
        ...
        it('transforms *:right:*=* ⟺ *:left:*=*', function () {
            var way = iD.osmWay({
                tags: { 'cycleway:right:surface': 'paved' },
            });
            var graph = iD.coreGraph([way]);

            graph = iD.actionReverse(way.id)(graph);
            expect(graph.entity(way.id).tags).to.eql({
                'cycleway:left:surface': 'paved',
            });
    ...
    describe('reverses directional values on ways', function () {
        it('transforms *=up ⟺ *=down', function () {
            ...
            expect(
                iD.actionReverse('inclineU')(graph).entity('inclineU').tags,
            ).to.eql({ incline: 'down' }, 'inclineU');
            expect(
                iD.actionReverse('directionU')(graph).entity('directionU').tags,
            ).to.eql({ direction: 'down' }, 'directionU');

            expect(
                iD.actionReverse('inclineD')(graph).entity('inclineD').tags,
            ).to.eql({ incline: 'up' }, 'inclineD');
            expect(
                iD.actionReverse('directionD')(graph).entity('directionD').tags,
            ).to.eql({ direction: 'up' }, 'directionD');
        });
        ...
        it('skips *=up ⟺ *=down for ignored tags', function () {
            ...
            expect(iD.actionReverse('name')(graph).entity('name').tags).to.eql(
                { name: 'up' },
                'name',
            );
            expect(iD.actionReverse('note')(graph).entity('note').tags).to.eql(
                { note: 'up' },
                'note',
            );
            expect(iD.actionReverse('ref')(graph).entity('ref').tags).to.eql(
                { ref: 'down' },
                'ref',
            );
            expect(
                iD.actionReverse('description')(graph).entity('description')
                    .tags,
            ).to.eql({ description: 'down' }, 'description');
        });
        ...
        it('transforms *=forward ⟺ *=backward', function () {
            var graph = iD.coreGraph([
                ...
                iD.osmWay({
                    id: 'trolley_wireB',
                    tags: { trolley_wire: 'backward' },
                }),
            ]);

            expect(
                iD.actionReverse('conveyingF')(graph).entity('conveyingF').tags,
            ).to.eql({ conveying: 'backward' }, 'conveyingF');
            expect(
                iD.actionReverse('directionF')(graph).entity('directionF').tags,
            ).to.eql({ direction: 'backward' }, 'directionF');
            expect(
                iD.actionReverse('priorityF')(graph).entity('priorityF').tags,
            ).to.eql({ priority: 'backward' }, 'priorityF');
            expect(
                iD.actionReverse('trolley_wireF')(graph).entity('trolley_wireF')
                    .tags,
            ).to.eql({ trolley_wire: 'backward' }, 'trolley_wireF');

            expect(
                iD.actionReverse('conveyingB')(graph).entity('conveyingB').tags,
            ).to.eql({ conveying: 'forward' }, 'conveyingB');
            expect(
                iD.actionReverse('directionB')(graph).entity('directionB').tags,
            ).to.eql({ direction: 'forward' }, 'directionB');
            expect(
                iD.actionReverse('priorityB')(graph).entity('priorityB').tags,
            ).to.eql({ priority: 'forward' }, 'priorityB');
            expect(
                iD.actionReverse('trolley_wireB')(graph).entity('trolley_wireB')
                    .tags,
            ).to.eql({ trolley_wire: 'forward' }, 'trolley_wireB');
        });
        ...
        it('drops "s" from forwards/backwards when reversing', function () {
            var graph = iD.coreGraph([
                iD.osmWay({
                    id: 'conveyingF',
                    tags: { conveying: 'forwards' },
                }),
                iD.osmWay({
                    id: 'conveyingB',
                    tags: { conveying: 'backwards' },
                }),
            ]);
        ...
        it('skips *=forward ⟺ *=backward for ignored tags', function () {
            ...
            expect(
                iD.actionReverse('description')(graph).entity('description')
                    .tags,
            ).to.eql({ description: 'backwards' }, 'description');
        });
        ...
        it('transforms *=right ⟺ *=left', function () {
            ...
            expect(
                iD.actionReverse('sidewalkR')(graph).entity('sidewalkR').tags,
            ).to.eql({ sidewalk: 'left' }, 'sidewalkR');
            expect(
                iD.actionReverse('sidewalkL')(graph).entity('sidewalkL').tags,
            ).to.eql({ sidewalk: 'right' }, 'sidewalkL');
        });
        ...
        it('skips *=right ⟺ *=left for ignored tags', function () {
            ...
            expect(
                iD.actionReverse('description')(graph).entity('description')
                    .tags,
            ).to.eql({ description: 'left' }, 'description');
        });
    });
    ...
    describe('reverses relation roles', function () {
        it('transforms role=forward ⟺ role=backward in member relations', function () {
            var graph = iD.coreGraph([
                ...
                iD.osmRelation({
                    id: 'forward',
                    members: [{ type: 'way', id: 'w1', role: 'forward' }],
                }),
                iD.osmRelation({
                    id: 'backward',
                    members: [{ type: 'way', id: 'w1', role: 'backward' }],
                }),
            ]);

            expect(
                iD.actionReverse('w1')(graph).entity('forward').members[0].role,
            ).to.eql('backward', 'forward');
            expect(
                iD.actionReverse('w1')(graph).entity('backward').members[0]
                    .role,
            ).to.eql('forward', 'backward');
        });
        ...
        it('drops "s" from forwards/backwards when reversing', function () {
            var graph = iD.coreGraph([
                ...
                iD.osmRelation({
                    id: 'forwards',
                    members: [{ type: 'way', id: 'w1', role: 'forwards' }],
                }),
                iD.osmRelation({
                    id: 'backwards',
                    members: [{ type: 'way', id: 'w1', role: 'backwards' }],
                }),
            ]);

            expect(
                iD.actionReverse('w1')(graph).entity('forwards').members[0]
                    .role,
            ).to.eql('backward', 'forwards');
            expect(
                iD.actionReverse('w1')(graph).entity('backwards').members[0]
                    .role,
            ).to.eql('forward', 'backwards');
        });
        ...
        it("doesn't transform role=north ⟺ role=south in member relations", function () {
            var graph = iD.coreGraph([
                ...
                iD.osmRelation({
                    id: 'north',
                    members: [{ type: 'way', id: 'w1', role: 'north' }],
                }),
                iD.osmRelation({
                    id: 'south',
                    members: [{ type: 'way', id: 'w1', role: 'south' }],
                }),
            ]);

            expect(
                iD.actionReverse('w1')(graph).entity('north').members[0].role,
            ).to.eql('north', 'north');
            expect(
                iD.actionReverse('w1')(graph).entity('south').members[0].role,
            ).to.eql('south', 'south');
        });
        ...
        it("doesn't transform role=east ⟺ role=west in member relations", function () {
            var graph = iD.coreGraph([
                ...
                iD.osmRelation({
                    id: 'east',
                    members: [{ type: 'way', id: 'w1', role: 'east' }],
                }),
                iD.osmRelation({
                    id: 'west',
                    members: [{ type: 'way', id: 'w1', role: 'west' }],
                }),
            ]);

            expect(
                iD.actionReverse('w1')(graph).entity('east').members[0].role,
            ).to.eql('east', 'east');
            expect(
                iD.actionReverse('w1')(graph).entity('west').members[0].role,
            ).to.eql('west', 'west');
        });
        ...
        it('ignores directionless roles in member relations', function () {
            var graph = iD.coreGraph([
                ...
                iD.osmRelation({
                    id: 'ignore',
                    members: [{ type: 'way', id: 'w1', role: 'ignore' }],
                }),
                iD.osmRelation({
                    id: 'empty',
                    members: [{ type: 'way', id: 'w1', role: '' }],
                }),
            ]);

            expect(
                iD.actionReverse('w1')(graph).entity('ignore').members[0].role,
            ).to.eql('ignore', 'ignore');
            expect(
                iD.actionReverse('w1')(graph).entity('empty').members[0].role,
            ).to.eql('', 'empty');
        });
    });
    ...
    describe('reverses directional values on childnodes', function () {
        ...
        it('reverses the direction of a backward facing stop sign on the way', function () {
            var node1 = iD.osmNode();
            var node2 = iD.osmNode({
                tags: { direction: 'backward', highway: 'stop' },
            });
            var node3 = iD.osmNode();
            var way = iD.osmWay({ nodes: [node1.id, node2.id, node3.id] });
            var graph = iD.actionReverse(way.id)(
        ...
        it('reverses the direction of a left facing stop sign on the way', function () {
            var node1 = iD.osmNode();
            var node2 = iD.osmNode({
                tags: { direction: 'left', highway: 'stop' },
            });
            var node3 = iD.osmNode();
            var way = iD.osmWay({ nodes: [node1.id, node2.id, node3.id] });
            var graph = iD.actionReverse(way.id)(
        ...
        it('reverses the direction of a right facing stop sign on the way', function () {
            var node1 = iD.osmNode();
            var node2 = iD.osmNode({
                tags: { direction: 'right', highway: 'stop' },
            });
            var node3 = iD.osmNode();
            var way = iD.osmWay({ nodes: [node1.id, node2.id, node3.id] });
            var graph = iD.actionReverse(way.id)(
        ...
        it('ignores directions other than forward or backward on attached stop sign during a reverse', function () {
            var node1 = iD.osmNode();
            var node2 = iD.osmNode({
                tags: { direction: 'empty', highway: 'stop' },
            });
            var node3 = iD.osmNode();
            var way = iD.osmWay({ nodes: [node1.id, node2.id, node3.id] });
            var graph = iD.actionReverse(way.id)(
    ...
    describe('reverses directional keys on childnodes', function () {
        it('reverses the direction of a forward facing traffic sign on the way', function () {
            var node1 = iD.osmNode();
            var node2 = iD.osmNode({
                tags: { 'traffic_sign:forward': 'stop' },
            });
            var node3 = iD.osmNode();
            var way = iD.osmWay({ nodes: [node1.id, node2.id, node3.id] });
            var graph = iD.actionReverse(way.id)(
        ...
        it('reverses the direction of a backward facing stop sign on the way', function () {
            var node1 = iD.osmNode();
            var node2 = iD.osmNode({
                tags: { 'traffic_sign:backward': 'stop' },
            });
            var node3 = iD.osmNode();
            var way = iD.osmWay({ nodes: [node1.id, node2.id, node3.id] });
            var graph = iD.actionReverse(way.id)(
        ...
        it('reverses the direction of a right facing traffic_signals on the way', function () {
            ...
            var node2 = iD.osmNode({
                tags: {
                    'traffic_signals:direction': 'right',
                    highway: 'traffic_signals',
                },
            });
            var node3 = iD.osmNode();
        ...
        it('ignores directions other than forward or backward on attached traffic_signals during a reverse', function () {
            ...
            var node2 = iD.osmNode({
                tags: {
                    'traffic_signals:direction': 'empty',
                    highway: 'traffic_signals',
                },
            });
            var node3 = iD.osmNode();
    ...
    describe('does not reverse values which are relative to another reversed tag', function () {
        ...
        it('preserves the turn directions of a multi-lane road', function () {
            var way = iD.osmWay({
                tags: { 'turn:lanes': 'through|through|right' },
            });
            var graph = iD.actionReverse(way.id)(iD.coreGraph([way]));
            var target = graph.entity(way.id);
        ...
        it('preserves the turn direction of each direction with multiple lanes', function () {
            var way = iD.osmWay({
                tags: {
                    ...
                    'turn:lanes:backward': 'through|through|left',
                },
            });
            var graph = iD.actionReverse(way.id)(iD.coreGraph([way]));
            var target = graph.entity(way.id);
            expect(target.tags['turn:lanes:backward']).to.eql('through|right');
            expect(target.tags['turn:lanes:forward']).to.eql(
                'through|through|left',
            );
        });

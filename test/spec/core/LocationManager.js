describe('LocationManager', () => {
    ...
    describe('#mergeCustomGeoJSON', () => {
        it('merges geojson into lococation-conflation cache', () => {
            ...
            expect(locationManager.loco()._cache['colorado.geojson']).to.be.eql(
                colorado,
            );
        });
    });
    ...
    describe('#locationSetID', () => {
        it('calculates a locationSetID for a locationSet', () => {
            expect(
                locationManager.locationSetID({ include: ['usa'] }),
            ).to.be.eql('+[Q30]');
        });

        it('falls back to the world locationSetID in case of errors', () => {
            expect(locationManager.locationSetID({ foo: 'bar' })).to.be.eql(
                '+[Q2]',
            );
            expect(
                locationManager.locationSetID({ include: ['fake.geojson'] }),
            ).to.be.eql('+[Q2]');
        });
    });
    ...
    describe('#locationSetsAt', () => {
        ...
        it('returns valid locationSets at a given lon,lat', async () => {
            ...
            expect(result1)
                .to.be.an('object')
                .that.has.all.keys('+[Q2]', '+[Q30]', '+[colorado.geojson]');
            const result2 = locationManager.locationSetsAt([-74.481, 40.797]); // Morristown
            expect(result2)
                .to.be.an('object')
                .that.has.all.keys('+[Q2]', '+[Q30]');
            const result3 = locationManager.locationSetsAt([13.575, 41.207]); // Gaeta
            expect(result3).to.be.an('object').that.has.all.keys('+[Q2]');
        });
    });

...
describe('iD.serviceTaginfo', function () {
    ...
    describe('#multikeys', function () {
        ...
        it('excludes multikeys with extra colons', async () => {
            ...
            expect(callback).to.have.been.calledWith(null, [
                {
                    title: 'service:bicycle:retail',
                    value: 'service:bicycle:retail',
                },
            ]);
        });
    ...
    describe('#values', function () {
        ...
        it('includes popular values', async () => {
            fetchMock.mock(/\/key\/values/, {
                body:
                    '{"data":[{"value":"parking","description":"A place for parking cars", "count":1000},' +
        ...
        it('does not get values for extremely unpopular keys', async () => {
            fetchMock.mock(/\/key\/values/, {
                body:
                    '{"data":[{"value":"Rue Pasteur","description":"", "count":3},' +
        ...
        it('excludes values with capital letters and some punctuation', async () => {
            fetchMock.mock(/\/key\/values/, {
                body:
                    '{"data":[{"value":"parking","description":"A place for parking cars", "count":2000},' +
        ...
        it('includes network values with capital letters and some punctuation', async () => {
            fetchMock.mock(/\/key\/values/, {
                body:
                    '{"data":[{"value":"US:TX:FM","description":"Farm to Market Roads in the U.S. state of Texas.", "count":34000},' +
            ...
            expect(callback).to.have.been.calledWith(null, [
                {
                    value: 'US:TX:FM',
                    title: 'Farm to Market Roads in the U.S. state of Texas.',
                },
                {
                    value: 'US:KY',
                    title: 'Primary and secondary state highways in the U.S. state of Kentucky.',
                },
                { value: 'US:US', title: 'U.S. routes in the United States.' },
                {
                    value: 'US:I',
                    title: 'Interstate highways in the United States.',
                },
                {
                    value: 'US:MD',
                    title: 'State highways in the U.S. state of Maryland.',
                },
            ]);
        });
    ...
    describe('#roles', function () {
        it('calls the given callback with the results of the roles query', async () => {
            ...
            var callback = sinon.spy();
            taginfo.roles(
                { rtype: 'route', query: 's', geometry: 'relation' },
                callback,
            );
            ...
            expect(parseQueryString(fetchMock.calls()[0][0])).to.eql({
                ...
                page: '1',
                rp: '25',
                sortname: 'count_relation_members',
                sortorder: 'desc',
                lang: 'en',
            });
            expect(callback).to.have.been.calledWith(null, [

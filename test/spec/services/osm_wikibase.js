...
describe('iD.serviceOsmWikibase', function () {
                ...
                P25: [
                    {
                        mainsnak: {
                            ...
                            datatype: 'wikibase-item',
                            datavalue: {
                                value: { 'entity-type': 'item', id: 'Q4679' },
                                type: 'wikibase-entityid',
                            },
                        },
                        type: 'statement',
                ...
                P28: [
                    {
                        mainsnak: {
                            ...
                            datatype: 'string',
                            datavalue: {
                                value: 'Mapping-Features-Parking-Lot.png',
                                type: 'string',
                            },
                        },
                        type: 'statement',
    ...
    describe('#getEntity', function () {
        it('calls the given callback with the results of the getEntity data item query', async () => {
            ...
            wikibase.getEntity(
                { key: 'amenity', value: 'parking', langCodes: ['fr'] },
                callback,
            );

            await setTimeout(50);
            expect(
                parseQueryString(fetchMock.calls(/action=wbgetentities/)[0][0]),
            ).to.eql({
    ...
    it('creates correct sitelinks', function () {
        ...
        expect(wikibase.toSitelink('amenity or_not_')).to.eql(
            'Key:amenity or not',
        );
        expect(wikibase.toSitelink('amenity', 'parking')).to.eql(
            'Tag:amenity=parking',
        );
        expect(wikibase.toSitelink(' amenity_', '_parking_')).to.eql(
            'Tag: amenity = parking',
        );
        expect(wikibase.toSitelink('amenity or_not', '_park ing_')).to.eql(
            'Tag:amenity or not= park ing',
        );
    });
    ...
    it('gets correct value from entity', function () {
        ...
        expect(wikibase.claimToValue(tagData(), 'P4', 'en')).to.eql(
            'Primary image.jpg',
        );
    ...
    describe('linkifyWikiText', () => {
        it('handles normal text', () => {
            ...
            d3.select(main).call(
                iD.serviceOsmWikibase.linkifyWikiText('hello'),
            );
        ...
        it('prevents XSS attacks', () => {
            const main = document.createElement('main');
            d3.select(main).call(
                iD.serviceOsmWikibase.linkifyWikiText(
                    '123 <script>bad</script> 456',
                ),
            );

            expect(main.innerHTML).toBe(
                '<span>123 &lt;script&gt;bad&lt;/script&gt; 456</span>',
            );
        ...
        it('linkifies the tag: and key: syntax', () => {
            ...
            d3.select(main).call(
                iD.serviceOsmWikibase.linkifyWikiText(
                    'use tag:natural=water with key:water instead',
                ),
            );
            ...
            expect(main.textContent).toBe(
                'use natural=water with water=* instead',
            );
        });
        ...
        it('works if the string is 100% a link', () => {
            const main = document.createElement('main');
            d3.select(main).call(
                iD.serviceOsmWikibase.linkifyWikiText('tag:natural=water'),
            );

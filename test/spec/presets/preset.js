describe('iD.presetPreset', function() {
    ...
    describe('#fields', function() {
        it('has no fields by default', function() {
            const preset = iD.presetPreset('test', {});
    ...
    describe('#matchGeometry', function() {
        it('returns false if it doesn\'t match', function() {
            const preset = iD.presetPreset('test', {geometry: ['line']});
            expect(preset.matchGeometry('point')).to.equal(false);
        });

        it('returns true if it does match', function() {
            const preset = iD.presetPreset('test', {geometry: ['point', 'line']});
    ...
    describe('#matchScore', function() {
        it('returns -1 if preset does not match tags', function() {
            const preset = iD.presetPreset('test', {tags: {foo: 'bar'}});
            const entity = iD.osmWay({tags: {highway: 'motorway'}});
            expect(preset.matchScore(entity.tags)).to.equal(-1);
        });

        it('returns the value of the matchScore property when matched', function() {
            const preset = iD.presetPreset('test', {tags: {highway: 'motorway'}, matchScore: 0.2});
            const entity = iD.osmWay({tags: {highway: 'motorway'}});
            expect(preset.matchScore(entity.tags)).to.equal(0.2);
        });

        it('defaults to the number of matched tags', function() {
            let preset = iD.presetPreset('test', {tags: {highway: 'residential'}});
            let entity = iD.osmWay({tags: {highway: 'residential'}});
            expect(preset.matchScore(entity.tags)).to.equal(1);
        ...
        it('counts * as a match for any value with score 0.5', function() {
            const preset = iD.presetPreset('test', {tags: {building: '*'}});
            const entity = iD.osmWay({tags: {building: 'yep'}});
            expect(preset.matchScore(entity.tags)).to.equal(0.5);
        });

        it('boosts matchScore for additional matches in addTags', function() {
            const presetSupercenter = iD.presetPreset('shop/supermarket/walmart_supercenter', {
                tags: { 'brand:wikidata': 'Q483551', 'shop': 'supermarket' },
                addTags: { 'name': 'Walmart Supercenter' }
            });
            const presetMarket = iD.presetPreset('shop/supermarket/walmart_market', {
                tags: { 'brand:wikidata': 'Q483551', 'shop': 'supermarket' },
                addTags: { 'name': 'Walmart Neighborhood Market' }
            });

            const supercenter = iD.osmWay({ tags: {
                'brand:wikidata': 'Q483551',
                'shop': 'supermarket',
                'name': 'Walmart Supercenter'
            }});
            const market = iD.osmWay({ tags: {
                'brand:wikidata': 'Q483551',
                'shop': 'supermarket',
    ...
    describe('#setTags', function() {
        ...
        it('adds default tags of fields with matching geometry', function() {
            ...
            const field = iD.presetField('field', {key: 'building', geometry: 'area', default: 'yes'});
            const preset = iD.presetPreset('test', {fields: ['field']}, isAddable, {field: field});
        ...
        describe('for a preset with no tag in areaKeys', function() {
            const preset = iD.presetPreset('test', {geometry: ['line', 'area'], tags: {name: 'testname', highway: 'pedestrian'}});
        ...
        describe('for a preset with a tag in areaKeys', function() {
            it('doesn\'t add area=yes automatically', function() {
                const preset = iD.presetPreset('test', {geometry: ['area'], tags: {name: 'testname', building: 'yes'}});
                expect(preset.setTags({}, 'area')).to.eql({name: 'testname', building: 'yes'});
            });

            it('does add area=yes if asked to', function() {
                const preset = iD.presetPreset('test', {geometry: ['area'], tags: {name: 'testname', area: 'yes'}});
    ...
    describe('#unsetTags', function() {
        ...
        it('removes area=yes', function() {
            const preset = iD.presetPreset('test', {tags: {highway: 'pedestrian'}});
        ...
        it('preserves tags that are not listed in removeTags', function() {
            const preset = iD.presetPreset('test', {tags: {a: 'b'}, removeTags: {}});
        ...
        it('preserves "ignoringKeys" tags', function() {
            const preset = iD.presetPreset('test', {tags: {a: 'a', b: 'b'}});
            expect(preset.unsetTags({a: 'a', b: 'b'}, 'area', ['b'])).to.eql({b: 'b'});
        });

        it('preserves "ignoringKeys" tags from default fields', function() {
            const field = iD.presetField('field', {key: 'b', geometry: 'area', default: 'default'});
            const preset = iD.presetPreset('test', {fields: ['field']}, true, {field: field});
            expect(preset.unsetTags({b: 'default'}, 'area', ['b'])).to.eql({b: 'default'});
        });

        it('uses tags from addTags if removeTags is not defined', function() {
            const preset = iD.presetPreset('test', {tags: {a: 'b'}, addTags: {remove: 'me'}});
    ...
    describe('#addable', function() {
        it('sets/gets addability of preset', function() {
            const preset = iD.presetPreset('test', {}, false);
    ...
    describe('#references', function() {
        it('references name, aliases and terms of another preset', function() {
            const allPresets = {};
            const other = iD.presetPreset('other', {}, undefined, undefined, allPresets);
            const preset = iD.presetPreset('test', {name: '{other}'}, undefined, undefined, allPresets);

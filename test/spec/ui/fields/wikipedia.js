...
describe('iD.uiFieldWikipedia', function () {
    ...
    function changeTags(changed) {
        ...
        for (var k in changed) {
            if (changed.hasOwnProperty(k)) {
                ...
                if (
                    tags[k] !== v &&
                    (v !== undefined || tags.hasOwnProperty(k))
                ) {
                    tags[k] = v;
                    didChange = true;
                }
    ...
    it('recognizes lang:title format', async () => {
        ...
        expect(iD.utilGetSetValue(selection.selectAll('.wiki-lang'))).to.equal(
            'English',
        );
        expect(iD.utilGetSetValue(selection.selectAll('.wiki-title'))).to.equal(
            'Title',
        );
    });
    ...
    it('sets language, value', async () => {
        var wikipedia = iD
            .uiFieldWikipedia(field, context)
            .entityIDs([entity.id]);
        await setTimeout(20);
    ...
    it('recognizes pasted URLs', async () => {
        ...
        iD.utilGetSetValue(
            selection.selectAll('.wiki-title'),
    ...
    describe('encodePath', function () {
        it('returns an encoded URI component that contains the title with spaces replaced by underscores', () => {
            var wikipedia = iD
                .uiFieldWikipedia(field, context)
                .entityIDs([entity.id]);
            expect(wikipedia.encodePath('? (film)', undefined)).to.equal(
                '%3F_(film)',
            );
        });
        ...
        it('returns an encoded URI component that includes an anchor fragment', () => {
            ...
            expect(
                wikipedia.encodePath('? (film)', 'Themes and style'),
            ).to.equal('%3F_(film)#Themes_and_style');
        });
    });
    ...
    describe('encodeURIAnchorFragment', function () {
        it('returns an encoded URI anchor fragment', () => {
            ...
            expect(wikipedia.encodeURIAnchorFragment('Theme?')).to.equal(
                '#Theme%3F',
            );
        });
        ...
        it('replaces all whitespace characters with underscore', () => {
            var wikipedia = iD
                ...
                .entityIDs([entity.id]);
            expect(
                wikipedia.encodeURIAnchorFragment('Themes And Styles'),
            ).to.equal('#Themes_And_Styles');
        });
        ...
        it('encodes % characters, does not replace them with a dot', () => {
            var wikipedia = iD
                ...
                .entityIDs([entity.id]);
            expect(
                wikipedia.encodeURIAnchorFragment('Is%this_100% correct'),
            ).to.equal('#Is%25this_100%25_correct');
        });
        ...
        it('encodes characters that are URI encoded characters', () => {
            var wikipedia = iD
                ...
                .entityIDs([entity.id]);
            expect(
                wikipedia.encodeURIAnchorFragment('Section %20%25'),
            ).to.equal('#Section_%2520%2525');
        });
    });

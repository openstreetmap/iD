...
describe('iD.uiFieldLocalized', function () {
    ...
    it('adds a blank set of fields when the + button is clicked', async () => {
        ...
        expect(selection.selectAll('.localized-lang').nodes().length).to.equal(
            1,
        );
        expect(selection.selectAll('.localized-value').nodes().length).to.equal(
            1,
        );
    });
    ...
    it('has a lang attribute on an existing multilingual name field', async () => {
        ...
        await setTimeout(20);
        selection.call(localized);
        expect(selection.selectAll('.localized-value').attr('lang')).to.eql(
            'de',
        );
    });
});

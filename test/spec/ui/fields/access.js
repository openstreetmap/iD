describe('iD.uiFieldAccess', function () {
    ...
    it('creates inputs for a variety of modes of access', function () {
        ...
        expect(
            selection.selectAll('.preset-access-motor_vehicle').size(),
        ).to.equal(1);
        expect(selection.selectAll('.preset-access-bicycle').size()).to.equal(
            1,
        );
    ...
    it('does not include "yes", "designated", "dismount" options for general access (#934), (#2213)', function () {
        var access = iD.uiFieldAccess(field, context);
        var options = access.options('access').map(function (v) {
            return v.value;
        });
    ...
    it('does include a "dismount" option for bicycles (#2726)', function () {
        ...
        options = access.options('bicycle').map(function (v) {
            return v.value;
        });
        expect(options).to.include('dismount');

        options = access.options('foot').map(function (v) {
            return v.value;
        });
        expect(options).not.to.include('dismount');
    });
    ...
    it('sets foot placeholder to "yes" for steps and pedestrian', function () {
        ...
        expect(
            selection
                .selectAll('.preset-input-access-foot')
                .attr('placeholder'),
        ).to.equal('yes');
    ...
    it('sets bicycle placeholder to "designated" for cycleways', function () {
        ...
        expect(
            selection
                .selectAll('.preset-input-access-bicycle')
                .attr('placeholder'),
        ).to.equal('designated');
    });
    ...
    it('sets horse placeholder to "designated" for bridleways', function () {
        ...
        expect(
            selection
                .selectAll('.preset-input-access-horse')
                .attr('placeholder'),
        ).to.equal('designated');
    });
    ...
    it('sets motor_vehicle placeholder to "no" for footways, steps, pedestrian, cycleway, bridleway, and path', function () {
        ...
        [
            ...
            expect(
                selection
                    .selectAll('.preset-input-access-motor_vehicle')
                    .attr('placeholder'),
            ).to.equal('no');
        });
    });
    ...
    it('sets correct placeholder on a multi selection', function () {
        ...
        var tags = {
            highway: 'primary',
            foot: ['yes', 'no'],
            bicycle: ['no', undefined],
            vehicle: ['no', undefined],
        };
        ...
        expect(
            selection
                .selectAll('.preset-input-access-foot')
                .attr('placeholder'),
        ).to.equal(iD.localizer.t('inspector.multiple_values'));
        expect(
            selection
                .selectAll('.preset-input-access-bicycle')
                .attr('placeholder'),

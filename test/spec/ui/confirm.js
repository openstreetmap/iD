...
describe('iD.uiConfirm', function () {
    ...
    beforeEach(function () {
        elem = d3.select('body').append('div').attr('class', 'confirm-wrap');
    });

    afterEach(function () {
        d3.select('.confirm-wrap').remove();
    });
    ...
    it('has a header section', function () {
        var selection = iD.uiConfirm(elem);
        expect(selection.selectAll('div.content div.header').size()).to.equal(
            1,
        );
    });
    ...
    it('has a message section', function () {
        var selection = iD.uiConfirm(elem);
        expect(
            selection.selectAll('div.content div.message-text').size(),
        ).to.equal(1);
    });

    it('has a buttons section', function () {
        var selection = iD.uiConfirm(elem);
        expect(selection.selectAll('div.content div.buttons').size()).to.equal(
            1,
        );
    });
    ...
    it('can have an ok button added to it', function () {
        ...
        expect(
            selection.selectAll('div.content div.buttons button.action').size(),
        ).to.equal(1);
    });
    ...
    it('can be dismissed by clicking the ok button', async () => {
        var selection = iD.uiConfirm(elem).okButton();
        happen.click(
            selection.select('div.content div.buttons button.action').node(),
        );
        await setTimeout(275);
        d3.timerFlush();
        expect(selection.node().parentNode).to.be.null;

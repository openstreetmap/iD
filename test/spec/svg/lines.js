describe('iD.svgLines', function () {
    ...
    it('adds tag classes', function () {
        ...
        expect(surface.select('.line').classed('tag-highway-residential')).to.be
            .true;
    });
    ...
    it('adds stroke classes for the tags of the parent relation of multipolygon members', function () {
        ...
        expect(surface.select('.stroke').classed('tag-natural-wood')).to.be
            .true;
    });
    ...
    describe('z-indexing', function () {
        var graph = iD.coreGraph([
            ...
            iD.osmWay({
                id: 'lo',
                tags: { highway: 'residential', layer: '0' },
                nodes: ['a', 'b'],
            }),
            iD.osmWay({
                id: 'hi',
        ...
        it('stacks higher lines above lower ones in a single render', function () {
            surface.call(
                ...
                graph,
                [graph.entity('lo'), graph.entity('hi')],
                none,
            );
        ...
        it('stacks higher lines above lower ones in a single render (reverse)', function () {
            surface.call(
                iD.svgLines(projection, context),
                graph,
                [graph.entity('hi'), graph.entity('lo')],
                none,
            );
    ...
    it('rounds layers down to the nearest whole number for rendering', () => {
        const graph = iD.coreGraph([
            ...
            iD.osmWay({
                id: 'w1',
                tags: { highway: 'residential', layer: '-2.5' },
                nodes: ['a', 'b'],
            }),
        ]);
        surface.call(
        ...
        const layerGroup = surface.select('path.w1').nodes()[0]
    ...
    describe('oneway-markers', function () {
        it('has marker layer for oneway ways', function () {
            ...
            expect(
                selection.nodes()[0].attributes['marker-mid'].nodeValue,
            ).to.eql('url(#ideditor-oneway-marker-black)');
            expect(
                selection.nodes()[1].attributes['marker-mid'].nodeValue,
            ).to.eql('url(#ideditor-oneway-marker-black)');
            expect(
                selection.nodes()[2].attributes['marker-mid'].nodeValue,
            ).to.eql('url(#ideditor-oneway-marker-black)');
        });
        ...
        it('has two marker layers for alternating oneway ways', function () {
            ...
            var e_a = iD.osmWay({
                id: 'explicit-alternating',
                tags: { highway: 'residential', oneway: 'alternating' },
                nodes: [a.id, b.id],
            });
        ...
        it('has no marker layer for oneway=no ways', function () {
            ...
            var e_no = iD.osmWay({
                id: 'explicit-no-oneway',
                tags: { highway: 'residential', oneway: 'no' },
                nodes: [a.id, b.id],
            });
            var i_no = iD.osmWay({
                id: 'implied-no-oneway',
                tags: { highway: 'residential' },
                nodes: [a.id, c.id],
            });
    ...
    describe('sided-markers', function () {
        it('has marker layer for sided way', function () {
            ...
            expect(
                selection.nodes()[0].attributes['marker-mid'].nodeValue,
            ).to.eql('url(#ideditor-sided-marker-natural)');
            expect(
                selection.nodes()[1].attributes['marker-mid'].nodeValue,
            ).to.eql('url(#ideditor-sided-marker-coastline)');
            expect(
                selection.nodes()[2].attributes['marker-mid'].nodeValue,
            ).to.eql('url(#ideditor-sided-marker-barrier)');
            expect(
                selection.nodes()[3].attributes['marker-mid'].nodeValue,
            ).to.eql('url(#ideditor-sided-marker-man_made)');
        });

describe('iD.svgTagClasses', function () {
    ...
    it('adds classes for primary tag key and key-value', function() {
        selection
            .datum(iD.entityEntity({tags: {highway: 'primary'}}))
            .call(iD.svgTagClasses());
    ...
    it('adds only one primary tag', function() {
        selection
            .datum(iD.entityEntity({tags: {highway: 'primary', railway: 'rail'}}))
            .call(iD.svgTagClasses());
        expect(selection.attr('class')).to.equal('tag-highway tag-highway-primary');
    });
    ...
    it('orders primary tags', function() {
        selection
            .datum(iD.entityEntity({tags: {railway: 'rail', highway: 'primary'}}))
            .call(iD.svgTagClasses());
        expect(selection.attr('class')).to.equal('tag-highway tag-highway-primary');
    });
    ...
    it('adds status tag when status in primary value (`railway=abandoned`)', function() {
        selection
            .datum(iD.entityEntity({tags: {railway: 'abandoned'}}))
            .call(iD.svgTagClasses());
    ...
    it('adds status tag when status in key and value matches "yes" (railway=rail + abandoned=yes)', function() {
        selection
            .datum(iD.entityEntity({tags: {railway: 'rail', abandoned: 'yes'}}))
            .call(iD.svgTagClasses());
    ...
    it('adds status tag when status in key and value matches primary (railway=rail + abandoned=railway)', function() {
        selection
            .datum(iD.entityEntity({tags: {railway: 'rail', abandoned: 'railway'}}))
            .call(iD.svgTagClasses());
        expect(selection.attr('class')).to.equal('tag-railway tag-railway-rail tag-status tag-status-abandoned');
    });
    ...
    it('adds primary and status tag when status in key and no primary (abandoned=railway)', function() {
        selection
            .datum(iD.entityEntity({tags: {abandoned: 'railway'}}))
            .call(iD.svgTagClasses());
        expect(selection.attr('class')).to.equal('tag-railway tag-status tag-status-abandoned');
    });
    ...
    it('does not add status tag for different primary tag (highway=path + abandoned=railway)', function() {
        selection
            .datum(iD.entityEntity({tags: {highway: 'path', abandoned: 'railway'}}))
            .call(iD.svgTagClasses());
    ...
    it('adds secondary tags', function() {
        selection
            .datum(iD.entityEntity({tags: {highway: 'primary', bridge: 'yes'}}))
            .call(iD.svgTagClasses());
    ...
    it('adds no bridge=no tags', function() {
        selection
            .datum(iD.entityEntity({tags: {bridge: 'no'}}))
            .call(iD.svgTagClasses());
        expect(selection.attr('class')).to.equal(null);
    });
    ...
    it('adds tag-unpaved for highway=track with no surface tagging', function() {
        selection
            .datum(iD.entityEntity({tags: {highway: 'track'}}))
            .call(iD.svgTagClasses());
    ...
    it('does not add tag-unpaved for highway=track with explicit paved surface tagging', function() {
        selection
            .datum(iD.entityEntity({tags: {highway: 'track', surface: 'asphalt'}}))
            .call(iD.svgTagClasses());
        expect(selection.classed('tag-unpaved')).to.be.false;

        selection
            .datum(iD.entityEntity({tags: {highway: 'track', tracktype: 'grade1'}}))
            .call(iD.svgTagClasses());
        expect(selection.classed('tag-unpaved')).to.be.false;
    });
    ...
    it('adds tag-unpaved for highway=track with explicit unpaved surface tagging', function() {
        selection
            .datum(iD.entityEntity({tags: {highway: 'track', surface: 'dirt'}}))
            .call(iD.svgTagClasses());
        expect(selection.classed('tag-unpaved')).to.be.true;

        selection
            .datum(iD.entityEntity({tags: {highway: 'track', tracktype: 'grade3'}}))
            .call(iD.svgTagClasses());
        expect(selection.classed('tag-unpaved')).to.be.true;
    });
    ...
    it('does not add tag-unpaved for other highway types with no surface tagging', function() {
        selection
            .datum(iD.entityEntity({tags: {highway: 'tertiary'}}))
            .call(iD.svgTagClasses());
        expect(selection.classed('tag-unpaved')).to.be.false;

        selection
            .datum(iD.entityEntity({tags: {highway: 'foo'}}))
            .call(iD.svgTagClasses());
        expect(selection.classed('tag-unpaved')).to.be.false;
    });
    ...
    it('does not add tag-unpaved for other highway types with explicit paved surface tagging', function() {
        selection
            .datum(iD.entityEntity({tags: {highway: 'tertiary', surface: 'asphalt'}}))
            .call(iD.svgTagClasses());
        expect(selection.classed('tag-unpaved')).to.be.false;

        selection
            .datum(iD.entityEntity({tags: {highway: 'foo', tracktype: 'grade1'}}))
            .call(iD.svgTagClasses());
        expect(selection.classed('tag-unpaved')).to.be.false;
    });
    ...
    it('does not add tag-unpaved for other aeroway types with explicit paved surface tagging', function() {
        selection
            .datum(iD.entityEntity({tags: {aeroway: 'taxiway', surface: 'asphalt'}}))
            .call(iD.svgTagClasses());
        expect(selection.classed('tag-unpaved')).to.be.false;

        selection
            .datum(iD.entityEntity({tags: {aeroway: 'runway', surface: 'paved'}}))
            .call(iD.svgTagClasses());
        expect(selection.classed('tag-unpaved')).to.be.false;
    });
    ...
    it('adds tag-unpaved for other highway types with explicit unpaved surface tagging', function() {
        selection
            .datum(iD.entityEntity({tags: {highway: 'tertiary', surface: 'dirt'}}))
            .call(iD.svgTagClasses());
        expect(selection.classed('tag-unpaved')).to.be.true;

        selection
            .datum(iD.entityEntity({tags: {highway: 'foo', tracktype: 'grade3'}}))
            .call(iD.svgTagClasses());
        expect(selection.classed('tag-unpaved')).to.be.true;
    });
    ...
    it('adds tag-unpaved for other aeroway types with explicit unpaved surface tagging', function() {
        selection
            .datum(iD.entityEntity({tags: {aeroway: 'taxiway', surface: 'dirt'}}))
            .call(iD.svgTagClasses());
        expect(selection.classed('tag-unpaved')).to.be.true;

        selection
            .datum(iD.entityEntity({tags: {aeroway: 'runway', surface: 'unpaved'}}))
            .call(iD.svgTagClasses());
        expect(selection.classed('tag-unpaved')).to.be.true;
    });
    ...
    it('does not add tag-unpaved for non-highways/aeroways', function() {
        selection
            .datum(iD.entityEntity({tags: {railway: 'abandoned', surface: 'gravel'}}))
            .call(iD.svgTagClasses());
        expect(selection.classed('tag-unpaved')).to.be.false;

        selection
            .datum(iD.entityEntity({tags: {amenity: 'parking', surface: 'dirt'}}))
            .call(iD.svgTagClasses());
        expect(selection.classed('tag-unpaved')).to.be.false;
    });
    ...
    it('adds tag-wikidata if entity has a wikidata tag', function() {
        selection
            .datum(iD.entityEntity({ tags: { wikidata: 'Q18275868' } }))
            .call(iD.svgTagClasses());
    ...
    it('adds tag-wikidata if entity has a brand:wikidata tag', function() {
        selection
            .datum(iD.entityEntity({ tags: { 'brand:wikidata': 'Q18275868' } }))
            .call(iD.svgTagClasses());
        expect(selection.classed('tag-wikidata')).to.be.true;
    });
    ...
    it('stroke overrides: renders areas with barriers as lines', function() {
        selection
            ...
            .datum(iD.entityEntity({tags: {landuse: 'residential', barrier: 'hedge'}}))
            .call(iD.svgTagClasses());

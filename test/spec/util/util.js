describe('iD.util', function () {
    ...
    it('utilTagText', function () {
        ...
        expect(iD.utilTagText({ tags: { foo: 'bar', two: 'three' } })).to.eql(
            'foo=bar, two=three',
        );
    });
    ...
    describe('utilStringQs', function () {
        it('splits a parameter string into k=v pairs', function () {
            ...
            expect(iD.utilStringQs('foo=bar&one=2')).to.eql({
                foo: 'bar',
                one: '2',
            });
    ...
    it('utilQsString', function () {
        ...
        expect(iD.utilQsString({ foo: 'bar baz' })).to.be.oneOf([
            'foo=bar%20baz',
            'foo=bar+baz',
        ]);
    ...
    describe('utilUnicodeCharsCount', function () {
        ...
        it('counts emoji', function () {
            ...
            expect(iD.utilUnicodeCharsCount('😎😬😆😵😴😄🙂🤔')).to.be.oneOf([
                8, 16,
            ]);
        });
    });
    ...
    describe('utilUnicodeCharsTruncated', function () {
        ...
        it('truncates Hindi text with combining marks', function () {
            ...
            expect(iD.utilUnicodeCharsTruncated('अनुच्छेद', 8)).to.eql(
                'अनुच्छेद',
            );
            expect(iD.utilUnicodeCharsTruncated('अनुच्छेद', 255)).to.eql(
                'अनुच्छेद',
            );
        });
        it('truncates demonic multiple combining marks', function () {
            ...
            expect(iD.utilUnicodeCharsTruncated('Z͑ͫ̓ͪ̂ͫ̽͏̴̙̤̞͉͚̯̞̠͍A̴̵̜̰͔ͫ͗͢L̠ͨͧͩ͘G̴̻͈͍͔̹̑͗̎̅͛́Ǫ̵̹̻̝̳͂̌̌͘!͖̬̰̙̗̿̋ͥͥ̂ͣ̐́́͜͞', 255)).to.eql(
        ...
        it('truncates emoji', function () {
            ...
            expect(iD.utilUnicodeCharsTruncated('🏳️‍🌈', 2)).to.be.oneOf([
                '🏳️',
                '\ud83c\udff3',
            ]);
            ...
            expect(iD.utilUnicodeCharsTruncated('👩‍❤️‍💋‍👩', 6)).to.be.oneOf([
                '👩‍❤️‍💋',
                '👩‍❤️‍',
            ]);
            expect(iD.utilUnicodeCharsTruncated('😎😬😆😵😴😄🙂🤔', 0)).to.eql(
            ...
            expect(
                iD.utilUnicodeCharsTruncated('😎😬😆😵😴😄🙂🤔', 4),
            ).to.be.oneOf(['😎😬😆😵', '😎😬']);
            expect(
                iD.utilUnicodeCharsTruncated('😎😬😆😵😴😄🙂🤔', 8),
            ).to.be.oneOf(['😎😬😆😵😴😄🙂🤔', '😎😬😆😵']);
            expect(iD.utilUnicodeCharsTruncated('😎😬😆😵😴😄🙂🤔', 16)).to.eql(
                '😎😬😆😵😴😄🙂🤔',
            );
            expect(
                iD.utilUnicodeCharsTruncated('😎😬😆😵😴😄🙂🤔', 255),
            ).to.eql('😎😬😆😵😴😄🙂🤔');
        });
    });
    ...
    describe('utilDisplayName', function () {
        it('returns the name if tagged with a name', function () {
            expect(
                iD.utilDisplayName({ tags: { name: 'East Coast Greenway' } }),
            ).to.eql('East Coast Greenway');
        });
        it('returns just the name for non-routes', function () {
            expect(
                iD.utilDisplayName({
                    tags: { name: 'Abyssinian Room', ref: '260-115' },
                }),
            ).to.eql('Abyssinian Room');
        });
        it('returns just the name for route with PTv2-formatted names', function () {
            expect(
                iD.utilDisplayName({
                    tags: {
                        name: 'NORTA 2: French Market → Canal at Bourbon',
                        network: 'NORTA',
                        ref: '2',
                        from: 'French Market',
                        to: 'Canal at Bourbon',
            ...
            ).to.eql('NORTA 2: French Market → Canal at Bourbon');
            expect(
                iD.utilDisplayName({
                    tags: {
                        name: 'VTA 64A: McKee & White => San Jose Diridon => Ohlone/Chynoweth',
                        network: 'VTA',
                        ref: '64A',
                        from: 'McKee & White',
                        to: 'Ohlone/Chynoweth',
                        via: 'San Jose Diridon',
                    },
                }),
            ).to.eql(
                'VTA 64A: McKee & White => San Jose Diridon => Ohlone/Chynoweth',
            );
            expect(
                iD.utilDisplayName({
                    tags: {
                        name: 'Bus 224: Downtown Garland Station -> Lake Ray Hubbard TC -> Downtown Dallas',
                        route: 'bus',
                        ref: '224',
                        from: 'Downtown Garland Station',
                        to: 'Downtown Dallas',
                        via: 'Lake Ray Hubbard TC',
                    },
                }),
            ).to.eql(
                'Bus 224: Downtown Garland Station -> Lake Ray Hubbard TC -> Downtown Dallas',
            );
        });
        it('suppresses the network tag if the hideNetwork argument is true', function () {
            ...
            ).to.eql('25L: Lynfield Express');
            expect(
                iD.utilDisplayName(
                    { tags: { network: 'SORTA', ref: '3X' } },
                    true,
                ),
            ).to.eql('3X');
            expect(
        ...
        it('distinguishes unnamed features by network or cycle_network', function () {
            expect(
                iD.utilDisplayName({ tags: { network: 'SORTA', ref: '3X' } }),
            ).to.eql('SORTA 3X');
            expect(
                iD.utilDisplayName({
                    tags: { network: 'ncn', cycle_network: 'US:US', ref: '76' },
                }),
            ).to.eql('US:US 76');
        });
        it('distinguishes unnamed routes by direction', function () {
            ...
            ).to.eql('Marguerite X anticlockwise');
        });
        it('distinguishes unnamed routes by waypoints', function () {
            ...
            ).to.eql('VTA Green from Old Ironsides to Winchester');
            ...
            expect(
                iD.utilDisplayName({
                    tags: {
                        ...
                        from: 'Antioch',
                        to: 'Millbrae',
                        via: 'Pittsburg/Bay Point;San Francisco International Airport',
                        route: 'subway',
                    },
                }),
            ).to.eql(
                'BART Yellow from Antioch to Millbrae via Pittsburg/Bay Point;San Francisco International Airport',
            );
        });
        it('can use alternative name tags', () => {
            expect(iD.utilDisplayName({ tags: { loc_ref: 'A' } })).to.eql('A');
            expect(
                iD.utilDisplayName({ tags: { 'seamark:name': 'Bean Rock' } }),
            ).to.eql('Bean Rock');

            expect(
                iD.utilDisplayName({
                    tags: { highway: 'milestone', distance: '12' },
                }),
            ).to.eql('12');
            ...
            expect(
                iD.utilDisplayName({
                    tags: { railway: 'milestone', 'railway:position': '12' },
                }),
            ).to.eql('12');
            expect(
                iD.utilDisplayName({ tags: { 'railway:position': '12' } }),
            ).to.eql(''); // `railway:position` is not used as a name on other features
        });
        it('prefers standard tags over alternative names', () => {
            expect(
                iD.utilDisplayName({ tags: { name: '1', official_name: '2' } }),
            ).to.eql('1');
            expect(
                iD.utilDisplayName({ tags: { ref: '1', loc_ref: '2' } }),
            ).to.eql('1');
            expect(
                iD.utilDisplayName({
                    tags: { ref: '1', network: 'AT', loc_ref: '2' },
                }),
            ).to.eql('AT 1');
        });
        it('distinguishes named features by name', function () {
            expect(
                iD.utilDisplayName({
                    tags: { name: 'Ohio Turnpike', route: 'road' },
                }),
            ).to.eql('Ohio Turnpike');
            expect(
                iD.utilDisplayName({
                    tags: {
                        name: 'Lynfield Express',
            ...
            ).to.eql('AT 25L: Lynfield Express');
        });
        it('distinguishes named features by network or cycle_network', function () {
            ...
            ).to.eql('US:TX:NTTA Dallas North Tollway');
        });
        it('distinguishes named features by ref', function () {
            ...
            ).to.eql('US:TX:NTTA DNT: Dallas North Tollway');
        });
        it('distinguishes named features by direction', function () {
            ...
            ).to.eql('US:TX:NTTA Dallas North Tollway south');
        });
        it('distinguishes named features by waypoints', function () {
            expect(
                iD.utilDisplayName({
                    tags: {
                        name: 'Kings Island Express',
                        network: 'SORTA',
                        ref: '71X',
                        from: 'Sycamore & Court',
                        to: 'Fields Ertel & Royal Point',
                        route: 'bus',
                    },
                }),
            ).to.eql(
                'SORTA 71X: Kings Island Express from Sycamore & Court to Fields Ertel & Royal Point',
            );
            expect(
                iD.utilDisplayName({
                    tags: {
                ...
                'Caltrain Local from San Francisco to Tamien via College Park',
            );
        });
        it('uses addr:housename', () => {
            expect(
                iD.utilDisplayName({
                    tags: { 'addr:housename': 'Siglap House' },
                }),
            ).to.eql('Siglap House');
        });
        ...
        it('uses the street address as a last resort', () => {
            expect(
                iD.utilDisplayName({
                    tags: {
                        'addr:housenumber': '1',
                        'addr:place': 'Motutapu Island',
                    },
                }),
            ).to.eql('1 Motutapu Island');
        });
            ...
            expect(
                iD.utilDisplayName({
                    tags: {
                        'addr:unit': 'Flat 1',
                        'addr:housenumber': '30',
                        'addr:street': 'Madden Street',
                    },
                }),
            ).to.eql('Flat 1, 30 Madden Street');
        });
        it('uses just addr:housenumber if it is the only addr: tag present', () => {
            expect(
                iD.utilDisplayName({ tags: { 'addr:housenumber': '32' } }),
            ).to.eql('32');
        });
    ...
    describe('utilOldestID', function () {
        ...
        it('returns the oldest database ID', function () {
            expect(iD.utilOldestID(['w100', 'w-1', 'a', 'w-300', 'w2'])).to.eql(
                'w2',
            );
        });
        it('returns the oldest editor ID if no database IDs', function () {
            expect(
                iD.utilOldestID(['w-100', 'w-1', 'a', 'w-300', 'w-2']),
            ).to.eql('w-1');
        });
